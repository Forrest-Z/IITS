...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared(
      "GetNearestCenterPolylinePoint3d_client");  // CHANGE
  rclcpp::Client<fead_interfaces::srv::GetNearestCenterPolylinePoint3d>::
      SharedPtr client =  // CHANGE
      node->create_client<
          fead_interfaces::srv::GetNearestCenterPolylinePoint3d>(
          "GetNearestCenterPolylinePoint3d");  // CHANGE
  ...
  if (rclcpp::spin_until_future_complete(node, result) ==
    ...
    RCLCPP_ERROR(
        ...
        "Failed to call get_nearest_center_polyline_point3d");  // CHANGE
  }
