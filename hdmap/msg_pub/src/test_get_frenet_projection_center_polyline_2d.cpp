...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared(
      "GetFrenetProjectionCenterPolyline2d_client");  // CHANGE
  rclcpp::Client<fead_interfaces::srv::GetFrenetProjectionCenterPolyline2d>::
      SharedPtr client =  // CHANGE
      node->create_client<
          fead_interfaces::srv::GetFrenetProjectionCenterPolyline2d>(
          "GetFrenetProjectionCenterPolyline2d");  // CHANGE
  ...
  if (rclcpp::spin_until_future_complete(node, result) ==
    ...
    RCLCPP_ERROR(
        ...
        "Failed to call get_frenet_projection_center_polyline2d");  // CHANGE
  }
