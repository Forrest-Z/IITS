...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<rclcpp::Node> node =
      rclcpp::Node::make_shared("AccessLeftLane_client");  // CHANGE
  rclcpp::Client<fead_interfaces::srv::AccessLeftLane>::SharedPtr
      client =  // CHANGE
      node->create_client<fead_interfaces::srv::AccessLeftLane>(
          "AccessLeftLane");  // CHANGE

  auto request = std::make_shared<
      fead_interfaces::srv::AccessLeftLane::Request>();  // CHANGE
  ...
  if (rclcpp::spin_until_future_complete(node, result) ==
    ...
    if (result.get()->flag) {
      RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Left Waypoint: %.4f %.4f %.4f",
                  result.get()->waypoint.x, result.get()->waypoint.y,
                  result.get()->waypoint.z);
    }
