...
struct CenterLine
{
  ...
  using ElevationProfileVector = std::vector<ElevationProfile>;
  using LateralProfileVector = std::vector<LateralProfile>;
  ...
  ElevationProfileVector elevationProfileVector;
  LateralProfileVector lateralProfileVector;

  double length{0.};

  /**
   * @brief Evalues the center line at the given position along the curve and returns a DirectedPoint
   * which contains the position and the tangent vector.
   * If applyLateralOffset is true, it will apply a lateral offset as defined by the lateral offset vector.
   */
  ...
  /**
   * @brief Calculates the center line offset at the given position along the curve.
   */
  double calculateOffset(double s) const;

  template <typename ProfileVector>
  double applyProfileVector(double s, ProfileVector pv) const;
