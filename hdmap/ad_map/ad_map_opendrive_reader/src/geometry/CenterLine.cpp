...
geometry::DirectedPoint CenterLine::eval(double s,
  ...
  for (auto it = geometry.rbegin(); it != geometry.rend(); it++) {
    if (s >= (*it)->GetStartOffset()) {
      ...
      // apply profiles
      directedPoint.ApplyLateralOffset(
          applyProfileVector(s, lateralProfileVector));
      directedPoint.ApplyElevationOffset(
          applyProfileVector(s, elevationProfileVector));
...
double CenterLine::applyProfileVector(double s, ProfileVector pv) const {
  if (s < 0.) {
    return calculateOffset(0.);
  }
  ...
  for (auto it = pv.rbegin(); it != pv.rend(); it++) {
    if (s >= it->start_position) {
      auto poly = boost::array<double, 4>{{it->elevation, it->slope,
                                           it->vertical_curvature,
                                           it->curvature_change}};

      return boost::math::tools::evaluate_polynomial(poly,
                                                     s - it->start_position);
    }
  }
...
bool generateCenterLine(RoadInformation &roadInfo, CenterLine &centerLine) {
  ...
  centerLine.elevationProfileVector = roadInfo.road_profiles.elevation_profile;
  centerLine.lateralProfileVector = roadInfo.road_profiles.lateral_profile;

  // Add geometry information
  for (auto &geometry_attribute : roadInfo.geometry_attributes) {
    Point start(geometry_attribute->start_position_x,
                geometry_attribute->start_position_y, 0.0);
