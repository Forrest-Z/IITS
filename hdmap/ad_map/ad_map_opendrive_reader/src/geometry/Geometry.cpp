...
DirectedPoint::DirectedPoint() : location(0, 0, 0), tangent(0) {}
...
void DirectedPoint::ApplyElevationOffset(double elevation_offset) {
  location.z += elevation_offset;
}
...
const DirectedPoint GeometryPoly3::PosFromDist(const double dist) const {
  ...
  DirectedPoint point(x0 + x, y0 + y, z, _heading + theta);
  return point;
}
