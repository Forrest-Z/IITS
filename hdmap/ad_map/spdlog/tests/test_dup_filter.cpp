...
TEST_CASE("dup_filter_test1", "[dup_filter_sink]") {
  ...
  for (int i = 0; i < 10; i++) {
    dup_sink.log(
        spdlog::details::log_msg{"test", spdlog::level::info, "message1"});
  }
...
TEST_CASE("dup_filter_test3", "[dup_filter_sink]") {
  ...
  for (int i = 0; i < 10; i++) {
    dup_sink.log(
        spdlog::details::log_msg{"test", spdlog::level::info, "message1"});
    dup_sink.log(
        spdlog::details::log_msg{"test", spdlog::level::info, "message2"});
  }
...
TEST_CASE("dup_filter_test4", "[dup_filter_sink]") {
  ...
  dup_sink.log(
      spdlog::details::log_msg{"test", spdlog::level::info, "message"});
