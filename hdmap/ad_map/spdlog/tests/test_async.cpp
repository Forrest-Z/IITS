...
TEST_CASE("basic async test ", "[async]") {
  ...
  size_t messages = 256;
  {
    ...
    auto logger = std::make_shared<async_logger>("as", test_sink, tp,
...
TEST_CASE("discard policy ", "[async]") {
  ...
  auto logger = std::make_shared<async_logger>(
      "as", test_sink, tp, async_overflow_policy::overrun_oldest);
  for (size_t i = 0; i < messages; i++) {
...
TEST_CASE("to_file", "[async]") {
  ...
  std::string filename = "test_logs/async_test.log";
  {
    ...
    auto tp =
        std::make_shared<spdlog::details::thread_pool>(messages, tp_threads);
    auto logger = std::make_shared<spdlog::async_logger>(
        "as", std::move(file_sink), std::move(tp));
