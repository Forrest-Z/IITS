...
TEST_CASE("simple_file_logger", "[simple_logger]]") {
  ...
  auto logger =
      spdlog::create<spdlog::sinks::basic_file_sink_mt>("logger", filename);
...
TEST_CASE("flush_on", "[flush_on]]") {
  ...
  REQUIRE(
      ...
      std::string("Should not be flushed\nTest message 1\nTest message 2\n"));
...
TEST_CASE("rotating_file_logger2", "[rotating_logger]]") {
...

  {
    ...
    auto logger =
        spdlog::rotating_logger_mt("logger", basename, max_size, 2, true);
    for (int i = 0; i < 10; ++i) {
      logger->info("Test message {}", i);
    }
