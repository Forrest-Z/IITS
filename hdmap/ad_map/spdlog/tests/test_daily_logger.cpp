...
TEST_CASE("daily_logger with dateonly calculator", "[daily_logger]") {
  ...
  fmt::format_to(w, "{}_{:04d}-{:02d}-{:02d}", basename, tm.tm_year + 1900,
...
struct custom_daily_file_name_calculator {
  static spdlog::filename_t calc_filename(const spdlog::filename_t &basename,
                                          const tm &now_tm) {
    spdlog::memory_buf_t w;
    fmt::format_to(w, "{}{:04d}{:02d}{:02d}", basename, now_tm.tm_year + 1900,
...
TEST_CASE("daily_logger with custom calculator", "[daily_logger]") {
  ...
  std::tm tm = spdlog::details::os::localtime();
  spdlog::memory_buf_t w;
  fmt::format_to(w, "{}{:04d}{:02d}{:02d}", basename, tm.tm_year + 1900,
                 tm.tm_mon + 1, tm.tm_mday);
...
TEST_CASE("rotating_file_sink::calc_filename1", "[rotating_file_sink]]") {
  auto filename =
      spdlog::sinks::rotating_file_sink_st::calc_filename("rotated.txt", 3);
  REQUIRE(filename == "rotated.3.txt");
}

TEST_CASE("rotating_file_sink::calc_filename2", "[rotating_file_sink]]") {
  auto filename =
      spdlog::sinks::rotating_file_sink_st::calc_filename("rotated", 3);
  REQUIRE(filename == "rotated.3");
}

TEST_CASE("rotating_file_sink::calc_filename3", "[rotating_file_sink]]") {
  auto filename =
      spdlog::sinks::rotating_file_sink_st::calc_filename("rotated.txt", 0);
...
TEST_CASE("daily_file_sink::daily_filename_calculator", "[daily_file_sink]]") {
  ...
  auto filename = spdlog::sinks::daily_filename_calculator::calc_filename(
      "daily.txt", spdlog::details::os::localtime());
...
static void test_rotate(int days_to_run, uint16_t max_days,
