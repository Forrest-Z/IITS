...
static std::string log_to_str(const std::string &msg, const Args &... args) {
  ...
  oss_logger.set_formatter(std::unique_ptr<spdlog::formatter>(
      new spdlog::pattern_formatter(args...)));
...
TEST_CASE("custom eol", "[pattern_formatter]") {
  ...
  std::string eol = ";)";
  REQUIRE(log_to_str(msg, "%v", spdlog::pattern_time_type::local, ";)") ==
          msg + eol);
}
...
TEST_CASE("level", "[pattern_formatter]") {
  REQUIRE(log_to_str("Some message", "[%l] %v",
...
TEST_CASE("name", "[pattern_formatter]") {
  REQUIRE(log_to_str("Some message", "[%n] %v",
                     spdlog::pattern_time_type::local,
...
TEST_CASE("date MM/DD/YY ", "[pattern_formatter]") {
  ...
  oss << std::setfill('0') << std::setw(2) << now_tm.tm_mon + 1 << "/"
      << std::setw(2) << now_tm.tm_mday << "/" << std::setw(2)
...
TEST_CASE("level_left_padded", "[pattern_formatter]") {
  REQUIRE(log_to_str("Some message", "[%8l] %v",
                     spdlog::pattern_time_type::local,
                     "\n") == "[    info] Some message\n");
  REQUIRE(log_to_str("Some message", "[%8!l] %v",
                     spdlog::pattern_time_type::local,
                     "\n") == "[    info] Some message\n");
}
...
TEST_CASE("short level_left_padded", "[pattern_formatter]") {
  REQUIRE(log_to_str("Some message", "[%3L] %v",
                     spdlog::pattern_time_type::local,
                     "\n") == "[  I] Some message\n");
  REQUIRE(log_to_str("Some message", "[%3!L] %v",
                     spdlog::pattern_time_type::local,
                     "\n") == "[  I] Some message\n");
}
...
TEST_CASE("left_padded_short", "[pattern_formatter]") {
  REQUIRE(log_to_str("Some message", "[%3n] %v",
                     spdlog::pattern_time_type::local,
                     "\n") == "[pattern_tester] Some message\n");
  REQUIRE(log_to_str("Some message", "[%3!n] %v",
                     spdlog::pattern_time_type::local,
                     "\n") == "[pat] Some message\n");
}
...
TEST_CASE("left_padded_huge", "[pattern_formatter]") {
  REQUIRE(log_to_str("Some message", "[%-300n] %v",
                     spdlog::pattern_time_type::local, "\n") ==
          "[pattern_tester                                                  ] "
          "Some message\n");

  REQUIRE(log_to_str("Some message", "[%-300!n] %v",
                     spdlog::pattern_time_type::local, "\n") ==
          "[pattern_tester                                                  ] "
          "Some message\n");
}
...
TEST_CASE("left_padded_max", "[pattern_formatter]") {
  REQUIRE(log_to_str("Some message", "[%-64n] %v",
                     spdlog::pattern_time_type::local, "\n") ==
          "[pattern_tester                                                  ] "
          "Some message\n");

  REQUIRE(log_to_str("Some message", "[%-64!n] %v",
                     spdlog::pattern_time_type::local, "\n") ==
          "[pattern_tester                                                  ] "
          "Some message\n");
}
...
TEST_CASE("paddinng_truncate_funcname", "[pattern_formatter]") {
  ...
  auto formatter = std::unique_ptr<spdlog::formatter>(
      new spdlog::pattern_formatter(pattern));
  test_sink.set_formatter(std::move(formatter));

  spdlog::details::log_msg msg1{spdlog::source_loc{"ignored", 1, "func"},
                                "test_logger", spdlog::level::info, "message"};
  test_sink.log(msg1);

  spdlog::details::log_msg msg2{spdlog::source_loc{"ignored", 1, "function"},
                                "test_logger", spdlog::level::info, "message"};
  test_sink.log(msg2);
...
TEST_CASE("clone-formatter", "[pattern_formatter]") {
  auto formatter_1 =
      std::make_shared<spdlog::pattern_formatter>("%D %X [%] [%n] %v");
  auto formatter_2 = formatter_1->clone();
  std::string logger_name = "test";
  spdlog::details::log_msg msg(logger_name, spdlog::level::info,
                               "some message");
...
TEST_CASE("short filename formatter-1", "[pattern_formatter]") {
  ...
  spdlog::details::log_msg msg(source_loc, "logger-name", spdlog::level::info,
                               "Hello");
  formatter.format(msg, formatted);
