...
void verify_file(const char *filename, int expected_count) {
  ...
  if (count != expected_count) {
    spdlog::error("Test failed. {} has {:n} lines instead of {:n}", filename,
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (argc == 1) {
      spdlog::info("Usage: {} <message_count> <threads> <q_size> <iterations>",
                   argv[0]);
      return 0;
    }
    ...
    spdlog::info("Queue memory : {:n} x {} = {:n} KB ", queue_size, slot_size,
...
void bench_mt(int howmany, std::shared_ptr<spdlog::logger> logger,
              int thread_count) {
  ...
  for (int t = 0; t < thread_count; ++t) {
    if (t == 0 && msgs_per_thread_mod)
      threads.push_back(std::thread(thread_fun, logger,
