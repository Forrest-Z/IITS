...
void bench_threaded_logging(int threads, int iters) {
  ...
  auto basic_mt =
      spdlog::basic_logger_mt("basic_mt", "logs/basic_mt.log", true);
  bench_mt(iters, std::move(basic_mt), threads);
  auto basic_mt_tracing = spdlog::basic_logger_mt("basic_mt/backtrace-on",
                                                  "logs/basic_mt.log", true);
  ...
  auto rotating_mt = spdlog::rotating_logger_mt(
      "rotating_mt", "logs/rotating_mt.log", file_size, rotating_files);
  bench_mt(iters, std::move(rotating_mt), threads);
  auto rotating_mt_tracing = spdlog::rotating_logger_mt(
      "rotating_mt/backtrace-on", "logs/rotating_mt.log", file_size,
      rotating_files);
  ...
  auto daily_mt_tracing =
      spdlog::daily_logger_mt("daily_mt/backtrace-on", "logs/daily_mt.log");
  ...
  auto empty_logger_tracing =
      std::make_shared<spdlog::logger>("level-off/backtrace-on");
...
void bench_single_threaded(int iters) {
  ...
  auto basic_st =
      spdlog::basic_logger_st("basic_st", "logs/basic_st.log", true);
  bench(iters, std::move(basic_st));

  auto basic_st_tracing = spdlog::basic_logger_st("basic_st/backtrace-on",
                                                  "logs/basic_st.log", true);
  bench(iters, std::move(basic_st_tracing));

  spdlog::info("");
  auto rotating_st = spdlog::rotating_logger_st(
      "rotating_st", "logs/rotating_st.log", file_size, rotating_files);
  bench(iters, std::move(rotating_st));
  auto rotating_st_tracing = spdlog::rotating_logger_st(
      "rotating_st/backtrace-on", "logs/rotating_st.log", file_size,
      rotating_files);
  ...
  auto daily_st_tracing =
      spdlog::daily_logger_st("daily_st/backtrace-on", "logs/daily_st.log");
...
void bench(int howmany, std::shared_ptr<spdlog::logger> log) {
  ...
  spdlog::info("{:<30} Elapsed: {:0.2f} secs {:>16n}/sec", log->name(), delta_d,
               int(howmany / delta_d));
