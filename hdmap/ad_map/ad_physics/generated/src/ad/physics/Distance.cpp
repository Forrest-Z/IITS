...
::ad::physics::DistanceSquared Distance::operator*(
    const Distance &other) const {
  ensureValid();
  other.ensureValid();
  ::ad::physics::DistanceSquared const result(mDistance * other.mDistance);
  result.ensureValid();  // LCOV_EXCL_BR_LINE On correct definition of
                         // squarerooted type, this cannot not happen
  return result;
}
