...
TEST(AccelerationTests, minIsDefinedAsExpected) {
  ...
  EXPECT_DOUBLE_EQ(::ad::physics::Acceleration::cMinValue,
                   static_cast<double>(::ad::physics::Acceleration::getMin()));
}

TEST(AccelerationTests, maxIsDefinedAsExpected) {
  EXPECT_DOUBLE_EQ(1e3, ::ad::physics::Acceleration::cMaxValue);
  EXPECT_DOUBLE_EQ(::ad::physics::Acceleration::cMaxValue,
                   static_cast<double>(::ad::physics::Acceleration::getMax()));
}
...
TEST(AccelerationTests, aboveMaxIsInvalid) {
  ::ad::physics::Acceleration value(::ad::physics::Acceleration::cMaxValue *
                                    1.1);
  EXPECT_FALSE(value.isValid());
}
...
TEST(AccelerationTestsStd, fabsIsWorkingCorrectly) {
  EXPECT_DOUBLE_EQ(
      0., static_cast<double>(std::fabs(::ad::physics::Acceleration(-0.))));
  EXPECT_DOUBLE_EQ(
      1., static_cast<double>(std::fabs(::ad::physics::Acceleration(-1.))));
  EXPECT_DOUBLE_EQ(::ad::physics::Acceleration::cPrecisionValue,
                   static_cast<double>(std::fabs(::ad::physics::Acceleration(
                       ::ad::physics::Acceleration::cPrecisionValue))));
...
TEST(AccelerationTests, copyConstructionFromValidValue) {
  ::ad::physics::Acceleration const validValue(
      ::ad::physics::Acceleration::cMinValue);
  ::ad::physics::Acceleration value(validValue);
  EXPECT_TRUE(value.isValid());
...
TEST(AccelerationTests, moveConstructionFromValidValue) {
  ::ad::physics::Acceleration validValue(
      ::ad::physics::Acceleration::cMinValue);
...
TEST(AccelerationTests, arithmeticOperatorsThrowOnInvalid) {
  ::ad::physics::Acceleration const minimalValue(
      ::ad::physics::Acceleration::cMinValue);
  ::ad::physics::Acceleration const maximalValue(
      ::ad::physics::Acceleration::cMaxValue);
  ::ad::physics::Acceleration const invalidValue;
  ...
  EXPECT_THROW(invalidValue / maximalValue, std::out_of_range);
  EXPECT_THROW(maximalValue / invalidValue, std::out_of_range);
  EXPECT_THROW(maximalValue / ::ad::physics::Acceleration(0.0),
               std::out_of_range);
...
TEST(AccelerationTests, comparisonOperatorsRespectPrecision) {
  double const precisionValueTimesTen =
      ::ad::physics::Acceleration::cPrecisionValue * 10.;
  ::ad::physics::Acceleration value;
  if (::ad::physics::Acceleration::cMinValue > precisionValueTimesTen) {
    value = ::ad::physics::Acceleration(::ad::physics::Acceleration::cMinValue +
                                        precisionValueTimesTen);
  } else if (::ad::physics::Acceleration::cMaxValue < precisionValueTimesTen) {
    value = ::ad::physics::Acceleration(::ad::physics::Acceleration::cMaxValue -
                                        precisionValueTimesTen);
  } else {
    value = ::ad::physics::Acceleration(precisionValueTimesTen);
  }
  ::ad::physics::Acceleration const sameValue = value;
  ::ad::physics::Acceleration const slightlyBiggerValue(
      static_cast<double>(value) +
      ::ad::physics::Acceleration::cPrecisionValue * 0.9);
  ::ad::physics::Acceleration const slightlySmallerValue(
      static_cast<double>(value) -
      ::ad::physics::Acceleration::cPrecisionValue * 0.9);
  ::ad::physics::Acceleration const actuallyBiggerValue(
      static_cast<double>(value) +
      ::ad::physics::Acceleration::cPrecisionValue * 1.1);
  ::ad::physics::Acceleration const actuallySmallerValue(
      static_cast<double>(value) -
      ::ad::physics::Acceleration::cPrecisionValue * 1.1);
