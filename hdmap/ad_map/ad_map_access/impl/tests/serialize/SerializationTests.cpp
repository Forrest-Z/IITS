...
struct SerializationTest : public FactoryTest {
  ...
  bool compareStores(access::Store const &left, access::Store const &right) {
    if (left.isValid() && right.isValid()) {
      bool equal = (left.getMetaData() == right.getMetaData()) &&
                   (left.getPartitions() == right.getPartitions()) &&
                   (left.getLanes() == right.getLanes()) &&
                   (left.getLandmarks() == right.getLandmarks());
      ...
      for (auto landmarkId : left.getLandmarks()) {
        equal = equal &&
                *left.getLandmarkPtr(landmarkId) ==
                    *right.getLandmarkPtr(landmarkId);
      }
      return equal;
    } else {
      return false;
  ...
  template <typename SerializationFile>
  void TestSerializationFile(const char *test_file_name) {
    size_t version_major_write =
        ::ad::map::serialize::SerializerFileCRC32::VERSION_MAJOR;
    size_t version_minor_write =
        ::ad::map::serialize::SerializerFileCRC32::VERSION_MINOR;

    SerializationFile serializer(true);
    ASSERT_TRUE(serializer.open(test_file_name, version_major_write,
                                version_minor_write));
    ...
    ASSERT_TRUE(deserializer.open(test_file_name, version_major_read,
                                  version_minor_read));
...
TEST_F(SerializationTest, TestSerializationFileCRC32) {
  TestSerializationFile<SerializerFileCRC32>(
      "test_files/test_serialization_crc32.adm");
}
...
TEST_F(SerializationTest, TestMapVersion) {
  ...
  ASSERT_THROW(deserializer.open("test_files/testmap_wrong_version.adm",
  ...
  serializer.open("test_files/test_map_version_correct.adm", version_major,
                  version_minor);
  store->save(serializer);
  serializer.close();

  SerializerFileCRC32 deserializer_correct(false);
  ASSERT_TRUE(deserializer_correct.open(
      "test_files/test_map_version_correct.adm", version_major, version_minor));
  ASSERT_TRUE(store->load(deserializer_correct));
  ASSERT_TRUE(deserializer_correct.close());

  // check the version number
  ASSERT_EQ(::ad::map::serialize::SerializerFileCRC32::VERSION_MAJOR,
            version_major);
  ASSERT_EQ(::ad::map::serialize::SerializerFileCRC32::VERSION_MINOR,
            version_minor);
}
...
TEST_F(SerializationTest, BadBranch) {
  ...
  ASSERT_TRUE(serializerW.open("test_files/test_map_version_correct.adm",
                               version_major, version_minor));
  ASSERT_FALSE(serializerW.open("test_files/test_map_version_correct.adm",
                                version_major, version_minor));
  ...
  ASSERT_TRUE(serializerR.open("test_files/test_map_version_correct.adm",
                               version_major, version_minor));
  ASSERT_FALSE(serializerR.open("test_files/test_map_version_correct.adm",
                                version_major, version_minor));
