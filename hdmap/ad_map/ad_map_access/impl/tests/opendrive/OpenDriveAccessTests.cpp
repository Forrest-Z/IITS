...
struct OpenDriveAccessTests : ::testing::Test {
  ...
  void checkEdgePoints(lane::LaneId laneId, point::ECEFEdge const &edge) {
    ...
    for (auto pointIter = edge.begin(); pointIter != edge.end(); pointIter++) {
      ...
      if (nextPointIter != edge.end()) {
        ...
        EXPECT_NE(pointDistance, physics::Distance(0.))
            << static_cast<uint64_t>(laneId);
      }
    }
  }
  ...
  void checkEdgeContacts(lane::Lane const &lane) {
    if (lane::isRouteable(lane)) {
      for (auto successorContact :
           lane::getContactLanes(lane, lane::ContactLocation::SUCCESSOR)) {
        auto successorLane = lane::getLane(successorContact.toLane);
        if (successorLane.direction == lane.direction) {
          EXPECT_EQ(lane.edgeLeft.ecefEdge.back(),
                    successorLane.edgeLeft.ecefEdge.front())
              << static_cast<uint64_t>(lane.id)
              << " succ: " << static_cast<uint64_t>(successorLane.id);
          EXPECT_EQ(lane.edgeRight.ecefEdge.back(),
                    successorLane.edgeRight.ecefEdge.front())
              << static_cast<uint64_t>(lane.id)
              << " succ: " << static_cast<uint64_t>(successorLane.id);
        } else {
          EXPECT_EQ(lane.edgeLeft.ecefEdge.back(),
                    successorLane.edgeRight.ecefEdge.back())
              << static_cast<uint64_t>(lane.id)
              << " succ: " << static_cast<uint64_t>(successorLane.id);
          EXPECT_EQ(lane.edgeRight.ecefEdge.back(),
                    successorLane.edgeLeft.ecefEdge.back())
              << static_cast<uint64_t>(lane.id)
              << " succ: " << static_cast<uint64_t>(successorLane.id);
        }
      }
      for (auto predecessorContact :
           lane::getContactLanes(lane, lane::ContactLocation::PREDECESSOR)) {
        auto predecessorLane = lane::getLane(predecessorContact.toLane);
        if (predecessorLane.direction == lane.direction) {
          EXPECT_EQ(lane.edgeLeft.ecefEdge.front(),
                    predecessorLane.edgeLeft.ecefEdge.back())
              << static_cast<uint64_t>(lane.id)
              << " pre: " << static_cast<uint64_t>(predecessorLane.id);
          EXPECT_EQ(lane.edgeRight.ecefEdge.front(),
                    predecessorLane.edgeRight.ecefEdge.back())
              << static_cast<uint64_t>(lane.id)
              << " pre: " << static_cast<uint64_t>(predecessorLane.id);
        } else {
          EXPECT_EQ(lane.edgeLeft.ecefEdge.front(),
                    predecessorLane.edgeRight.ecefEdge.front())
              << static_cast<uint64_t>(lane.id)
              << " pre: " << static_cast<uint64_t>(predecessorLane.id);
          EXPECT_EQ(lane.edgeRight.ecefEdge.front(),
                    predecessorLane.edgeLeft.ecefEdge.front())
              << static_cast<uint64_t>(lane.id)
              << " pre: " << static_cast<uint64_t>(predecessorLane.id);
        }
      }
      for (auto leftContact :
           lane::getContactLanes(lane, lane::ContactLocation::LEFT)) {
        auto leftLane = lane::getLane(leftContact.toLane);
        EXPECT_EQ(lane.edgeLeft.ecefEdge.front(),
                  leftLane.edgeRight.ecefEdge.front())
            << static_cast<uint64_t>(lane.id)
            << " left: " << static_cast<uint64_t>(leftLane.id);
        EXPECT_EQ(lane.edgeLeft.ecefEdge.back(),
                  leftLane.edgeRight.ecefEdge.back())
            << static_cast<uint64_t>(lane.id)
            << " left: " << static_cast<uint64_t>(leftLane.id);
      }
      for (auto rightContact :
           lane::getContactLanes(lane, lane::ContactLocation::RIGHT)) {
        auto rightLane = lane::getLane(rightContact.toLane);
        EXPECT_EQ(lane.edgeRight.ecefEdge.front(),
                  rightLane.edgeLeft.ecefEdge.front())
            << static_cast<uint64_t>(lane.id)
            << " right: " << static_cast<uint64_t>(rightLane.id);
        EXPECT_EQ(lane.edgeRight.ecefEdge.back(),
                  rightLane.edgeLeft.ecefEdge.back())
            << static_cast<uint64_t>(lane.id)
            << " right: " << static_cast<uint64_t>(rightLane.id);
      }
    }
  ...
  void checkMapMatching(lane::Lane const &lane) {
    ...
    auto ecefPoint0 = lane::getParametricPoint(
        lane, physics::ParametricValue(0.), physics::ParametricValue(.5));
    auto enuPoint0 = point::toENU(ecefPoint0);
    auto mapMatchedPositions0 = mapMatching.getMapMatchedPositions(
        enuPoint0, physics::Distance(5.), physics::Probability(0.01));
    auto ecefPoint1 = lane::getParametricPoint(
        lane, physics::ParametricValue(1.), physics::ParametricValue(.5));
    auto enuPoint1 = point::toENU(ecefPoint1);
    auto mapMatchedPositions1 = mapMatching.getMapMatchedPositions(
        enuPoint1, physics::Distance(5.), physics::Probability(0.01));
    auto contactLanes = lane::getContactLanes(
        lane,
        {lane::ContactLocation::LEFT, lane::ContactLocation::RIGHT,
         lane::ContactLocation::SUCCESSOR, lane::ContactLocation::PREDECESSOR});
    ...
    mapMatching.addHeadingHint(point::createENUHeading(0.),
                               access::getENUReferencePoint());
    for (auto paramLon = physics::ParametricValue(0.);
         paramLon <= physics::ParametricValue(1.);
         paramLon += physics::ParametricValue(0.1)) {
      for (auto paramLat = physics::ParametricValue(0.);
           paramLat <= physics::ParametricValue(1.);
        ...
        EXPECT_NO_THROW(
            auto mapMatchedPositions = mapMatching.getMapMatchedPositions(
...
TEST_F(OpenDriveAccessTests, read_map) {
  ...
  size_t versionMajorWrite =
      ::ad::map::serialize::SerializerFileCRC32::VERSION_MAJOR;
  size_t versionMinorWrite =
      ::ad::map::serialize::SerializerFileCRC32::VERSION_MINOR;
  serializer.open("test_files/Town01.adm", versionMajorWrite,
                  versionMinorWrite);
...
TEST_F(OpenDriveAccessTests, defaultIntersectionType) {
  ...
  access::cleanup();
  ASSERT_TRUE(
      access::init("test_files/bad/Town01copyPriorityToRightAndStraight.txt"));
  access::cleanup();
...
TEST_F(OpenDriveAccessTests, branch) {
  ...
  ASSERT_FALSE(
      factory.createAdMap(std::string("test_files/bad/NoneExists.xodr"), 2.0,
                          intersection::IntersectionType::HasWay));

  access::cleanup();
  ASSERT_FALSE(factory.createAdMap(std::string("test_files/bad/TownEmpty.xodr"),
                                   2.0,
                                   intersection::IntersectionType::HasWay));
}
...
TEST_F(OpenDriveAccessTests, DataTypeConversion) {
  ...
  ASSERT_EQ(landmark::TrafficSignType::LANES_MERGING,
            toTrafficSignType(120, 0));

  ASSERT_EQ(landmark::TrafficSignType::CAUTION_PEDESTRIAN,
            toTrafficSignType(133, 0));

  ASSERT_EQ(landmark::TrafficSignType::CAUTION_BICYCLE,
            toTrafficSignType(138, 0));
  ...
  ASSERT_EQ(landmark::TrafficSignType::ACCESS_FORBIDDEN,
            toTrafficSignType(250, 0));
  ASSERT_EQ(landmark::TrafficSignType::ACCESS_FORBIDDEN_MOTORVEHICLES,
            toTrafficSignType(251, 0));
  ASSERT_EQ(landmark::TrafficSignType::ACCESS_FORBIDDEN_TRUCKS,
            toTrafficSignType(253, 0));
  ASSERT_EQ(landmark::TrafficSignType::ACCESS_FORBIDDEN_BICYCLE,
            toTrafficSignType(254, 0));
  ASSERT_EQ(landmark::TrafficSignType::ACCESS_FORBIDDEN_WEIGHT,
            toTrafficSignType(263, 0));
  ASSERT_EQ(landmark::TrafficSignType::ACCESS_FORBIDDEN_WIDTH,
            toTrafficSignType(264, 0));
  ASSERT_EQ(landmark::TrafficSignType::ACCESS_FORBIDDEN_HEIGHT,
            toTrafficSignType(265, 0));
  ASSERT_EQ(landmark::TrafficSignType::ACCESS_FORBIDDEN_WRONG_DIR,
            toTrafficSignType(267, 0));

  ASSERT_EQ(landmark::TrafficSignType::MAX_SPEED, toTrafficSignType(274, -1));
  ASSERT_EQ(landmark::TrafficSignType::SPEED_ZONE_30_BEGIN,
            toTrafficSignType(274, 1));
  ASSERT_EQ(landmark::TrafficSignType::SPEED_ZONE_30_END,
            toTrafficSignType(274, 2));
  ASSERT_EQ(landmark::TrafficSignType::MAX_SPEED, toTrafficSignType(274, 3));

  ASSERT_EQ(landmark::TrafficSignType::HAS_WAY_NEXT_INTERSECTION,
            toTrafficSignType(301, 0));
  ...
  ASSERT_EQ(landmark::TrafficSignType::MOTORVEHICLE_BEGIN,
            toTrafficSignType(331, 0));
  ...
  ASSERT_EQ(landmark::TrafficLightType::SOLID_RED_YELLOW_GREEN,
            toTrafficLightType(1000001, 0));
  ASSERT_EQ(landmark::TrafficLightType::PEDESTRIAN_RED_GREEN,
            toTrafficLightType(1000002, 0));
  ASSERT_EQ(landmark::TrafficLightType::BIKE_PEDESTRIAN_RED_GREEN,
            toTrafficLightType(1000007, 0));
  ASSERT_EQ(landmark::TrafficLightType::UNKNOWN,
            toTrafficLightType(1000008, 0));
  ASSERT_EQ(landmark::TrafficLightType::SOLID_RED_YELLOW,
            toTrafficLightType(1000009, 0));
  ASSERT_EQ(landmark::TrafficLightType::UNKNOWN,
            toTrafficLightType(1000010, 0));
  ASSERT_EQ(landmark::TrafficLightType::LEFT_RED_YELLOW_GREEN,
            toTrafficLightType(1000011, 10));
  ASSERT_EQ(landmark::TrafficLightType::RIGHT_RED_YELLOW_GREEN,
            toTrafficLightType(1000011, 20));
  ASSERT_EQ(landmark::TrafficLightType::STRAIGHT_RED_YELLOW_GREEN,
            toTrafficLightType(1000011, 30));
  ASSERT_EQ(landmark::TrafficLightType::LEFT_STRAIGHT_RED_YELLOW_GREEN,
            toTrafficLightType(1000011, 40));
  ASSERT_EQ(landmark::TrafficLightType::RIGHT_STRAIGHT_RED_YELLOW_GREEN,
            toTrafficLightType(1000011, 50));
  ASSERT_EQ(landmark::TrafficLightType::UNKNOWN,
            toTrafficLightType(1000011, 60));
  ASSERT_EQ(landmark::TrafficLightType::UNKNOWN,
            toTrafficLightType(1000012, 0));
  ASSERT_EQ(landmark::TrafficLightType::BIKE_RED_GREEN,
            toTrafficLightType(1000013, 0));
  ASSERT_EQ(landmark::TrafficLightType::UNKNOWN,
            toTrafficLightType(1000014, 0));
  ASSERT_EQ(landmark::TrafficLightType::UNKNOWN,
            toTrafficLightType(1000015, 0));
  ASSERT_EQ(landmark::TrafficLightType::UNKNOWN,
            toTrafficLightType(1000016, 0));

  ::opendrive::SignalReference signalReference;
  signalReference.parametricPosition = 0.5;
  ASSERT_EQ(lane::ContactLocation::SUCCESSOR,
            toContactLocation(signalReference, true));
  signalReference.parametricPosition = 0.4;
  ASSERT_EQ(lane::ContactLocation::PREDECESSOR,
            toContactLocation(signalReference, true));
  signalReference.inLaneOrientation = true;
  ASSERT_EQ(lane::ContactLocation::SUCCESSOR,
            toContactLocation(signalReference, false));
