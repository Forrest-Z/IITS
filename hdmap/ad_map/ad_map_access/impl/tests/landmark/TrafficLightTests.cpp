...
landmark::ENULandmark make_landmark(uint64_t id, double x, double y,
                                    double heading) {
...
TEST_F(TrafficLightTests, validate_trafficLights_access) {
  ...
  auto const laneGeoPosition =
      point::createGeoPoint(point::Longitude(8.4365541),
                            point::Latitude(49.0159383), point::Altitude(0.));
  auto const laneId = lane::uniqueLaneId(laneGeoPosition);
  access::setENUReferencePoint(laneGeoPosition);

  EXPECT_THROW(landmark::getVisibleTrafficLights(lane::LaneId(1000)),
               std::invalid_argument);

  auto const mapTrafficLights = landmark::getVisibleTrafficLights(laneId);

  std::vector<landmark::ENULandmark> expectedLandmarks{
      make_landmark(21, -21.4540, -40.1775, -1.1057),
  ...
  for (auto mapTrafficLightId : mapTrafficLights) {
    ...
    do {
      // we cannot compare the traffic light id's since these may change in the
      // map, but the positions
      // have to be constant
      if ((std::fabs(static_cast<double>(expectedIter->position.x -
                                         mapTrafficLight.position.x)) <=
           maxError) &&
          (std::fabs(static_cast<double>(expectedIter->position.y -
                                         mapTrafficLight.position.y)) <=
           maxError) &&
          (std::fabs(static_cast<double>(expectedIter->position.z -
                                         mapTrafficLight.position.z)) <=
           maxError) &&
          (fmod(static_cast<double>(expectedIter->heading -
                                    mapTrafficLight.heading),
                2 * M_PI) <= maxError)) {
        break;
      }
      expectedIter++;
