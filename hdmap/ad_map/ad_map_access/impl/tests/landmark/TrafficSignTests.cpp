...
TEST_F(TrafficSignTests, generate_traffic_signs) {
  ...
  for (std::size_t signIndex = 0u;
       signIndex < landmark::knownTrafficSigns.size(); ++signIndex) {
    landmark::LandmarkId landmarkId(100 + signIndex);
    ECEFPoint position =
        createECEFPoint(8.44 + static_cast<double>(signIndex) * 0.000002,
                        49.000002, 115.);  // approx. every 2 meter
    ECEFPoint orientation = createECEFPoint(
        8.4399 + static_cast<double>(signIndex) * 0.000002, 49.000002, 115.);
    ECEFEdge boundingBoxGeometry{position, orientation};
    Geometry bounding_box = createGeometry(boundingBoxGeometry, true);
    EXPECT_TRUE(factory.addTrafficSign(
        mPartitionId, landmarkId, landmark::knownTrafficSigns[signIndex],
        position, orientation, bounding_box, std::to_string(signIndex)));
    EXPECT_TRUE(factory.add(mLaneId, landmarkId));
  }

  EXPECT_EQ(landmark::getLandmarks().size(),
            landmark::knownTrafficSigns.size());
  ...
  serializer.open("test_files/test_traffic_signs.adm", version_major,
                  version_minor);
