...
TEST_F(LandmarkOperationTests, TestLandmark) {
  GeoPoint point = createGeoPoint(Longitude(8.44038283714),
                                  Latitude(49.0199362465), Altitude(115.));
  ECEFPoint position = toECEF(point);

  // setup a landmark
  landmark::LandmarkId landmarkId(1234);
  GeoPoint orientation =
      createGeoPoint(Longitude(8.44038283714), Latitude(49.0), Altitude(115.));
  ...
  EXPECT_TRUE(mFactory->addLandmark(mPartitionId, landmarkId,
                                    landmark::LandmarkType::UNKNOWN, position,
                                    orientationECEF, boundingBox));
  ...
  point = createGeoPoint(Longitude(8.44038283714), Latitude(51.0199362465),
                         Altitude(115.));
  EXPECT_THROW(landmark::uniqueLandmarkId(point), std::invalid_argument);
...
TEST_F(LandmarkOperationTests, TestLandmark_Orientation) {
  GeoPoint point =
      createGeoPoint(Longitude(8.4), Latitude(49.1), Altitude(115.));
  ECEFPoint position = toECEF(point);

  ECEFPoint orientation1 =
      toECEF(createGeoPoint(Longitude(8.4), Latitude(49.2), Altitude(115.)));
  ECEFPoint orientation2 =
      toECEF(createGeoPoint(Longitude(8.4), Latitude(49.), Altitude(115.)));
  ECEFPoint orientation3 =
      toECEF(createGeoPoint(Longitude(8.5), Latitude(49.1), Altitude(115.)));
  ECEFPoint orientation4 =
      toECEF(createGeoPoint(Longitude(8.3), Latitude(49.1), Altitude(115.)));

  // setup a landmark
  Geometry boundingBox = createGeometry({orientation1, orientation4}, true);
  EXPECT_TRUE(mFactory->addLandmark(mPartitionId, landmark::LandmarkId(1),
                                    landmark::LandmarkType::UNKNOWN, position,
                                    orientation1, boundingBox));

  EXPECT_TRUE(mFactory->addLandmark(mPartitionId, landmark::LandmarkId(2),
                                    landmark::LandmarkType::UNKNOWN, position,
                                    orientation2, boundingBox));

  EXPECT_TRUE(mFactory->addLandmark(mPartitionId, landmark::LandmarkId(3),
                                    landmark::LandmarkType::UNKNOWN, position,
                                    orientation3, boundingBox));

  EXPECT_TRUE(mFactory->addLandmark(mPartitionId, landmark::LandmarkId(4),
                                    landmark::LandmarkType::UNKNOWN, position,
                                    orientation4, boundingBox));

  EXPECT_NEAR(double(landmark::getENUHeading(
                  landmark::getLandmark(landmark::LandmarkId(1)))),
              M_PI / 2., 1e-3);
  EXPECT_NEAR(double(landmark::getENUHeading(
                  landmark::getLandmark(landmark::LandmarkId(2)))),
              -M_PI / 2., 1e-3);
  EXPECT_NEAR(double(landmark::getENUHeading(
                  landmark::getLandmark(landmark::LandmarkId(3)))),
              0., 1e-3);
  EXPECT_NEAR(double(landmark::getENUHeading(
                  landmark::getLandmark(landmark::LandmarkId(4)))),
              M_PI, 1e-3);
}
...
TEST_F(LandmarkOperationTests, TestTrafficLight) {
  ...
  EXPECT_TRUE(mFactory->addTrafficLight(
      mPartitionId, landmarkId,
      landmark::TrafficLightType::SOLID_RED_YELLOW_GREEN, position, orientation,
      bounding_box));
  EXPECT_EQ(landmark::getLandmarks().size(), 1u);
  ...
  lane::ContactTypeList traffic_light_contact(
  ...
  EXPECT_TRUE(mFactory->add(mLaneId, x12, succ, traffic_light_contact,
                            no_restrictions, landmarkId));
  // invalid contact, type: traffic light and no traffic light id
  EXPECT_FALSE_NO_LOG(
      mFactory->add(x12, x13, succ, traffic_light_contact, no_restrictions));
  // invalid contact, type: free and valid traffic light id
  EXPECT_FALSE_NO_LOG(
      mFactory->add(x12, x13, succ, free_contact, no_restrictions, landmarkId));
  ...
  EXPECT_TRUE(traffic_light.trafficLightType ==
...
TEST_F(LandmarkOperationTests, TestTrafficSign) {
  ...
  EXPECT_TRUE(mFactory->addTrafficSign(
      mPartitionId, landmarkId, landmark::TrafficSignType::MAX_SPEED, position,
      orientation, bounding_box, text));
  EXPECT_EQ(landmark::getLandmarks().size(), 1u);
  ...
  EXPECT_THROW_NO_LOG(landmark::getLandmarkPtr(landmark::LandmarkId(1000)),
