...
struct AdMapMatchingTest : ::testing::Test {
  ...
  void compareMapMatching(int line, point::GeoPoint const &geoPoint,
                          std::size_t const expectedNumberOfMatches);

  void addRouteHint() {
    // with route hint straigth through Albert-Nestler-Str
    auto hintPoint =
        point::createGeoPoint(point::Longitude(8.4401803),
                              point::Latitude(49.0191987), point::Altitude(0.));
    auto mapMatchingResults = mMapMatching->getMapMatchedPositions(
        hintPoint, physics::Distance(0.01), mMinProbabilty);
    ...
    laneSegment.laneInterval.laneId =
        mapMatchingResults.front().lanePoint.paraPoint.laneId;
   ...
   *  Ada-Lovel.
   *
   * All points for matching (mTestPoints) should be along the lane from south
   * to north
  ...
  std::vector<std::pair<point::GeoPoint, size_t>> mTestPoints{
      std::make_pair(point::createGeoPoint(point::Longitude(8.4400665),
                                           point::Latitude(49.0192005),
                                           point::Altitude(0.)),
                     0u),
      std::make_pair(point::createGeoPoint(point::Longitude(8.4401882),
                                           point::Latitude(49.0191939),
                                           point::Altitude(0.)),
                     1u),
      std::make_pair(point::createGeoPoint(point::Longitude(8.4401510),
                                           point::Latitude(49.0191792),
                                           point::Altitude(0.)),
                     2u),
      std::make_pair(point::createGeoPoint(point::Longitude(8.4401742),
                                           point::Latitude(49.0192009),
                                           point::Altitude(0.)),
                     3u),
      // approaching middle of the intersection
      std::make_pair(point::createGeoPoint(point::Longitude(8.4401540),
                                           point::Latitude(49.0192082),
                                           point::Altitude(0.)),
                     4u)};
...
void AdMapMatchingTest::compareMapMatching(
    int line, point::GeoPoint const &geoPoint,
    std::size_t const expectedNumberOfMatches) {
  ...
  auto mapMatchingResults = mMapMatching->getMapMatchedPositions(
      geoPoint, searchDist, mMinProbabilty);

  for (auto mapMatchingResult : mapMatchingResults) {
    TestResult result;
    result.mapMatchedPosition = mapMatchingResult;
    result.laneDirection =
        lane::getLane(result.mapMatchedPosition.lanePoint.paraPoint.laneId)
            .direction;
    testResults.push_back(result);
  }

  ASSERT_EQ(expectedNumberOfMatches, testResults.size())
      << " compareMapMatching called from " << line << "\n";

  if ((mRouteHint.roadSegments.size() > 0u) && (testResults.size() > 0u)) {
    ASSERT_EQ(1u, mRouteHint.roadSegments.size())
        << " compareMapMatching called from " << line << "\n";
    ASSERT_EQ(1u, mRouteHint.roadSegments[0].drivableLaneSegments.size())
        << " compareMapMatching called from " << line << "\n";
    ASSERT_LT(0u, testResults.size()) << " compareMapMatching called from "
                                      << line << "\n";
    ASSERT_EQ(
        mRouteHint.roadSegments[0].drivableLaneSegments[0].laneInterval.laneId,
...
TEST_F(AdMapMatchingTest, isLanePartOfRouteHints) {
  ...
  EXPECT_TRUE(
      mMapMatching->isLanePartOfRouteHints(mRouteHint.roadSegments.front()
...
TEST_F(AdMapMatchingTest, laneOperation) {
  ...
  EXECUTE_NO_LOG(mapMatchingResults = mMapMatching->findLanes(
                     point::ECEFPoint(), physics::Distance(0.01)));
  ASSERT_EQ(mapMatchingResults.size(), 0u);
  mapMatchingResults =
      mMapMatching->findLanes(toECEF(hintPoint), physics::Distance());
  ASSERT_EQ(mapMatchingResults.size(), 0u);
  mapMatchingResults =
      mMapMatching->findLanes(toECEF(hintPoint), physics::Distance(0.01));
  ASSERT_EQ(mapMatchingResults.size(), 1u);
  ASSERT_NEAR(double(ad::map::lane::calcWidth(toENU(hintPoint))), 3.0039,
              0.0001);
