...
TEST_F(AdMapBoundingBoxMapMatchingTest, box_within_single_lane) {
  ...
  auto centerMapMatched = mapMatching.getMapMatchedPositions(
      mObjectPosition, mSamplingDistance, mMinProbabilty);
  ...
  auto result =
      mapMatching.getMapMatchedBoundingBox(mObjectPosition, mSamplingDistance);
  ...
  for (auto centerMatch : centerMapMatched) {
    if (centerMatch.type == match::MapMatchedPositionType::LANE_IN) {
      ASSERT_EQ(centerMatch.lanePoint.paraPoint.laneId,
                result.laneOccupiedRegions.front().laneId);
  ...
  result
      .referencePointPositions[int32_t(match::ObjectReferencePoints::RearLeft)]
      .clear();
  result
      .referencePointPositions[int32_t(match::ObjectReferencePoints::RearRight)]
      .clear();
  result
      .referencePointPositions[int32_t(match::ObjectReferencePoints::FrontLeft)]
      .clear();
  result
  ...
  LaneOccupiedRegionList occRegion =
      mapMatching.getLaneOccupiedRegions(enuObjs, mSamplingDistance);
  ...
  for (auto centerMatch : centerMapMatched) {
    if (centerMatch.type == match::MapMatchedPositionType::LANE_IN) {
      ...
      ASSERT_EQ(centerMatch.lanePoint.paraPoint.laneId,
                occRegion.front().laneId);
    }
  }

  physics::Distance dis =
      signedDistanceToLane(occRegion.front().laneId, centerMapMatched);
...
TEST_F(AdMapBoundingBoxMapMatchingTest, rotated_box_within_two_lateral_lanes) {
  ...
  auto searchFront = std::find_if(
      result.laneOccupiedRegions.begin(), result.laneOccupiedRegions.end(),
      [&centerMapMatched](match::LaneOccupiedRegion const &other) {
        return other.laneId ==
               centerMapMatched.front().lanePoint.paraPoint.laneId;
      });
  ASSERT_TRUE(searchFront != std::end(result.laneOccupiedRegions));

  auto searchBack = std::find_if(
      result.laneOccupiedRegions.begin(), result.laneOccupiedRegions.end(),
      [&centerMapMatched](match::LaneOccupiedRegion const &other) {
        return other.laneId ==
  ...
  auto laneContactRelation = lane::getDirectNeighborhoodRelation(
      centerMapMatched.front().lanePoint.paraPoint.laneId,
      centerMapMatched.back().lanePoint.paraPoint.laneId);
  ...
  for (auto const &occupiedRegion : result.laneOccupiedRegions) {
    ...
    vehicleWidth += lane::getLane(occupiedRegion.laneId).width *
                    (occupiedRegion.lateralRange.maximum -
                     occupiedRegion.lateralRange.minimum);
  }
...
TEST_F(AdMapBoundingBoxMapMatchingTest, box_covering_three_lanes_longitudinal) {
  auto objectCenter = point::toENU(point::createGeoPoint(
      point::Longitude(8.439497056), point::Latitude(49.018312553),
      point::Altitude(0.)));

  match::AdMapMatching mapMatching;
  auto centerMapMatched = mapMatching.getMapMatchedPositions(
      objectCenter, mSamplingDistance, mMinProbabilty);
  ...
  objectPosition.heading =
      point::createENUHeading(static_cast<double>(heading));
  ...
  auto result =
      mapMatching.getMapMatchedBoundingBox(objectPosition, mSamplingDistance);
  ...
  for (auto const &occupiedRegion : result.laneOccupiedRegions) {
    ...
    if (occupiedRegion.laneId == centerLaneId) {
      ...
      ASSERT_EQ(physics::ParametricValue(0.),
                occupiedRegion.longitudinalRange.minimum);
      ASSERT_EQ(physics::ParametricValue(1.),
                occupiedRegion.longitudinalRange.maximum);
    } else if (occupiedRegion.longitudinalRange.minimum ==
               physics::ParametricValue(0.)) {
      // other lane not fully covered, but definitely some parts of it
      ASSERT_NE(physics::ParametricValue(0.),
                occupiedRegion.longitudinalRange.maximum);
      ASSERT_NE(physics::ParametricValue(1.),
                occupiedRegion.longitudinalRange.maximum);
    } else {
      // other lane not fully covered, but definitely some parts of it
      ASSERT_EQ(physics::ParametricValue(1.),
...
TEST_F(AdMapBoundingBoxMapMatchingTest,
       box_not_touching_second_lane_within_sampling_distance) {
  auto objectCenter = point::toENU(
      point::createGeoPoint(point::Longitude(8.4394653),
                            point::Latitude(49.0182735), point::Altitude(0.)));
  ...
  auto objectPosition = mObjectPosition;
  objectPosition.heading =
      mapMatching.getLaneENUHeading(centerMapMatched.front());
  objectPosition.centerPoint = objectCenter;
  objectPosition.dimension.width = physics::Distance(2.5);
  objectPosition.dimension.length = physics::Distance(6.);
