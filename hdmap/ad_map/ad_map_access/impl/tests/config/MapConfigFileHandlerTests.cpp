...
TEST(MapConfigFileHandler, readValidConfig) {
  ...
  // the configFileName is a fully qualified path, check that it is correctly
  // returned
...
TEST(MapConfigFileHandler, map_config_invalid_map_location) {
  ad::map::config::MapConfigFileHandler configHandler{};
  ASSERT_FALSE_NO_LOG(configHandler.readConfig(
      "test_files/map_config_invalid_map_location.txt"));
}

TEST(MapConfigFileHandler, map_config_map_location_out_of_tree) {
  ad::map::config::MapConfigFileHandler configHandler{};
  ASSERT_FALSE_NO_LOG(configHandler.readConfig(
      "test_files/map_config_map_location_out_of_tree.txt"));
}

TEST(MapConfigFileHandler, invalid_default_enu_reference_config) {
  ad::map::config::MapConfigFileHandler configHandler{};
  ASSERT_FALSE_NO_LOG(configHandler.readConfig(
      "test_files/map_config_invalid_default_enu_reference.txt"));
...
TEST(MapConfigFileHandler, valid_default_enu_reference_config) {
  ...
  EXPECT_EQ(ad::map::point::Latitude(49.0080515),
            configHandler.defaultEnuReference().latitude);
  EXPECT_EQ(ad::map::point::Longitude(8.442078),
            configHandler.defaultEnuReference().longitude);
  EXPECT_EQ(ad::map::point::Altitude(0.),
            configHandler.defaultEnuReference().altitude);
}
...
TEST(MapConfigFileHandler, bad_branch) {
  ...
  ASSERT_FALSE(
      configHandler.readConfig("test_files/bad/opendrive.overlap.bad.txt"));
}
