...
struct CoordianteTransformTest : ::testing::Test {
  ...
  virtual void SetUp() {
    access::cleanup();
    GeoPoint enu_origin =
        createGeoPoint(Longitude(-78.12), Latitude(43.21), Altitude(120));
  ...
  void TestCT(const TF1 &tf1, const TF2 &tf2) {
  ...
  template <typename TF1, typename TF2>
  void TestCT2(const TF1 &tf1, const TF2 &tf2) {
    TestCT(tf1, tf2);
    TestCT(tf2, tf1);
  }
...
TEST_F(CoordianteTransformTest, Geo2ENU) {
  ...
  GeoPoint geo =
      createGeoPoint(Longitude(-78.120123), Latitude(43.21009), Altitude(124));
...
TEST_F(CoordianteTransformTest, ECEF2ENU) {
  ...
  ECEFPoint ecef = createECEFPoint(958506.00937351841, -4556367.3723087218,
                                   4344627.1621201029);  // Exact
  TestCT2(ecef, enu);
}
...
TEST_F(CoordianteTransformTest, ECEFGeo) {
  {
    ...
    GeoPoint geo = createGeoPoint(Longitude(-121.935549), Latitude(37.401336),
                                  Altitude(123.456));
    ECEFPoint ecef = createECEFPoint(-2683524.7374057802, -4305306.3074802049,
                                     3852945.8669066750);
    TestCT2(geo, ecef);
  }
  ...
  {
    // Position of tree from http://www.dirsig.org/docs/new/coordinates.html
    GeoPoint geo = createGeoPoint(Longitude(-78.120123), Latitude(43.21009),
                                  Altitude(124));
    // ECEFPoint ecef = createECEFPoint(958506.011, -4556367.375, 4344627.164);
    // // Original values
    ECEFPoint ecef = createECEFPoint(958506.00937351841, -4556367.3723087218,
                                     4344627.1621201029);  // Exact
    TestCT2(geo, ecef);
...
TEST_F(CoordianteTransformTest, Distances) {
  {
    ...
    // Position of Intel Office
    GeoPoint geo1 =
        createGeoPoint(Longitude(-121.935549), Latitude(37.401336), h);
    // Position of San Jose Fire Department Station 29
    GeoPoint geo2 =
        createGeoPoint(Longitude(-121.933886), Latitude(37.401112), h);
    // Position of San Jose Fire Department Station 29 + altitude difference
    GeoPoint geo3 =
        createGeoPoint(Longitude(-121.933886), Latitude(37.401112), h + dh);
...
TEST_F(CoordianteTransformTest, GetRef) {
  ...
  // Position of Intel Office
  GeoPoint pt1_geo = createGeoPoint(Longitude(-121.935549), Latitude(37.401336),
                                    Altitude(123.456));
  // Position of San Jose Fire Department Station 29
  GeoPoint pt2_geo = createGeoPoint(Longitude(-121.933886), Latitude(37.401112),
                                    Altitude(123.456));
...
TEST_F(CoordianteTransformTest, Branch) {
  ...
  EXPECT_THROW_NO_LOG(coordinateTransform.Geo2ENU(geo_pt_invalid),
                      std::invalid_argument);
  EXPECT_THROW_NO_LOG(coordinateTransform.ENU2Geo(enu_pt_invalid),
                      std::invalid_argument);
  EXPECT_THROW_NO_LOG(coordinateTransform.Geo2ECEF(geo_pt_invalid),
                      std::invalid_argument);
  EXPECT_THROW_NO_LOG(coordinateTransform.ECEF2Geo(ecef_pt_invalid),
                      std::invalid_argument);
  EXPECT_THROW_NO_LOG(coordinateTransform.ENU2ECEF(enu_pt_invalid),
                      std::invalid_argument);
  EXPECT_THROW_NO_LOG(coordinateTransform.ECEF2ENU(ecef_pt_invalid),
                      std::invalid_argument);

  EXPECT_THROW_NO_LOG(coordinateTransform.setENUReferencePoint(geo_pt_invalid),
                      std::invalid_argument);
