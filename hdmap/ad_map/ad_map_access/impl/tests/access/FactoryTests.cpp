...
void FactoryTest::TearDown() { access::cleanup(); }
...
bool FactoryTest::Fill() {
  ...
  ok = ok &&
       pFactory->add(p, x11, lane::LaneType::NORMAL,
                     lane::LaneDirection::POSITIVE);
  ok = ok &&
       pFactory->add(p, x12, lane::LaneType::SHOULDER,
                     lane::LaneDirection::NEGATIVE);
  ok = ok &&
       pFactory->add(p, x13, lane::LaneType::MULTI,
                     lane::LaneDirection::BIDIRECTIONAL);
  ...
  lane::ContactTypeList curb_up(
      {lane::ContactType::FREE, lane::ContactType::CURB_UP});
  lane::ContactTypeList curb_down(
      {lane::ContactType::FREE, lane::ContactType::CURB_DOWN});
  ...
  predestrian_and_bikes.roadUserTypes = {restriction::RoadUserType::PEDESTRIAN,
                                         restriction::RoadUserType::BICYCLE};
  ...
  point::Geometry bounding_box =
      point::createGeometry({orientation, position}, true);
  ok = ok &&
       pFactory->addLandmark(p, landmarkId, landmark::LandmarkType::UNKNOWN,
                             position, orientation, bounding_box);
...
void FactoryTest::Check(const lane::LaneId &id, lane::LaneType type,
...
void FactoryTest::Check(const lane::LaneId &id, point::Geometry const &edgeLeft,
                        point::Geometry const &edgeRight) {
  auto lane = lane::getLanePtr(id);
  ASSERT_TRUE(static_cast<bool>(lane));
...
void FactoryTest::Check(const lane::LaneId &id_from, const lane::LaneId &id_to,
...
bool FactoryTest::IsAccessOk(const lane::LaneId &id,
...
TEST_F(FactoryTest, TestFactory) {
  ...
  ASSERT_FALSE(
      IsTransitionOk(x11, x12, succ, restriction::RoadUserType::PEDESTRIAN));
  ASSERT_FALSE(
      IsTransitionOk(x11, x12, succ, restriction::RoadUserType::BICYCLE));
  ...
  ASSERT_FALSE(
      IsTransitionOk(x12, x13, succ, restriction::RoadUserType::PEDESTRIAN));
  ASSERT_FALSE(
      IsTransitionOk(x12, x13, succ, restriction::RoadUserType::BICYCLE));
  ...
  ASSERT_TRUE(
      IsTransitionOk(x12, x13, left, restriction::RoadUserType::PEDESTRIAN));
  ASSERT_TRUE(
      IsTransitionOk(x12, x13, left, restriction::RoadUserType::BICYCLE));
  ...
  ASSERT_TRUE(
      IsTransitionOk(x13, x12, righ, restriction::RoadUserType::PEDESTRIAN));
  ASSERT_FALSE(
      IsTransitionOk(x13, x12, righ, restriction::RoadUserType::BICYCLE));
  ...
  leftBorderPoints.push_back(
      createGeoPoint(Longitude(8.3), Latitude(49.2), Altitude(115.)));
  leftBorderPoints.push_back(
      createGeoPoint(Longitude(8.5), Latitude(49.2), Altitude(115.)));
  rightBorderPoints.push_back(
      createGeoPoint(Longitude(8.3), Latitude(49.), Altitude(115.)));
  rightBorderPoints.push_back(
      createGeoPoint(Longitude(8.5), Latitude(49.), Altitude(115.)));
  ...
  x15Left.push_back(
      createGeoPoint(Longitude(8.5), Latitude(49.2), Altitude(115.)));
  x15Left.push_back(
      createGeoPoint(Longitude(8.7), Latitude(49.2), Altitude(115.)));
  x15Right.push_back(
      createGeoPoint(Longitude(8.5), Latitude(49.), Altitude(115.)));
  x15Right.push_back(
      createGeoPoint(Longitude(8.7), Latitude(49.), Altitude(115.)));
  x16Left.push_back(
      createGeoPoint(Longitude(8.7), Latitude(49.2), Altitude(115.)));
  x16Left.push_back(
      createGeoPoint(Longitude(8.9), Latitude(49.2), Altitude(115.)));
  x16Right.push_back(
      createGeoPoint(Longitude(8.7), Latitude(49.), Altitude(115.)));
  x16Right.push_back(
      createGeoPoint(Longitude(8.9), Latitude(49.), Altitude(115.)));
  ...
  EXPECT_THROW_NO_LOG(pFactory->add(p, left_ecef, right_ecef, x11, x12),
                      std::runtime_error);
...
TEST_F(FactoryTest, StoreLaneLength) {
  ...
  pFactory->add(access::PartitionId(0), lane::LaneId(11),
                lane::LaneType::NORMAL, lane::LaneDirection::POSITIVE);
  ...
  laneIdList = mStorePtr->getLanes(
      std::string("::ad::map::lane::LaneType::NORMAL"), false);
  ASSERT_EQ(laneIdList.size(), 1u);

  laneIdList =
      mStorePtr->getLanes(access::PartitionId(0), std::string("abc"), false);
  ASSERT_EQ(laneIdList.size(), 0u);
  laneIdList = mStorePtr->getLanes(
      access::PartitionId(0), std::string("::ad::map::lane::LaneType::NORMAL"),
      false);
  ASSERT_EQ(laneIdList.size(), 1u);
}
