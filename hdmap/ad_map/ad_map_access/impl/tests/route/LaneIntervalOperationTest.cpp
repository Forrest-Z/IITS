...
struct LaneIntervalOperationTest : ::testing::Test {
  ...
  void setupSingleLongLaneV1() {
    ...
    point::GeoPoint bottomRight = point::createGeoPoint(
        point::Longitude(8.04937788), point::Latitude(49.00736837),
        point::Altitude(0.));
    point::GeoPoint upRight = point::createGeoPoint(
        point::Longitude(8.05010064), point::Latitude(49.00884320),
        point::Altitude(0.));
    point::GeoPoint bottomLeft = point::createGeoPoint(
        point::Longitude(8.04933898), point::Latitude(49.00737633),
        point::Altitude(0.));
    point::GeoPoint upLeft = point::createGeoPoint(point::Longitude(8.05006151),
    ...
    pFactory->add(p, laneId, lane::LaneType::NORMAL,
  ...
  void setupSingleLongLaneV2() {
    ...
    x12 = lane::LaneId(i++);
    pFactory->add(p, x12, lane::LaneType::NORMAL,
                  lane::LaneDirection::POSITIVE);
    pFactory->set(access::TrafficType::RIGHT_HAND_TRAFFIC);
...
TEST_F(LaneIntervalOperationTest, LaneIntervalOperation) {
  ...
  EXPECT_THROW(getSignedDistance(laneInt1, first, second),
               std::invalid_argument);
  EXPECT_THROW(getUnsignedDistance(laneInt1, first, second),
               std::invalid_argument);
  ...
  ASSERT_EQ(getSignedDistance(laneInt1, first, second),
            ::ad::physics::ParametricValue(0.6));
  laneInt1.end = ::ad::physics::ParametricValue(0.1);
  ASSERT_EQ(getSignedDistance(laneInt1, first, second),
            ::ad::physics::ParametricValue(-0.6));
  ASSERT_EQ(getUnsignedDistance(laneInt1, first, second),
            ::ad::physics::ParametricValue(0.6));

  laneInt1.start = laneInt1.end = ::ad::physics::ParametricValue(0.5);
  ASSERT_EQ(extendIntervalFromStart(laneInt1, physics::Distance(0.5)),
            laneInt1);
  ...
  laneInt1.end = ::ad::physics::ParametricValue(0.8);
  resultInterval = laneInt1;
  ASSERT_EQ(cutIntervalAtStart(laneInt1, ::ad::physics::ParametricValue(0.1)),
            resultInterval);
  ASSERT_EQ(cutIntervalAtEnd(laneInt1, ::ad::physics::ParametricValue(0.1)),
            resultInterval);

  resultInterval.start = ::ad::physics::ParametricValue(0.6);
  ASSERT_EQ(cutIntervalAtStart(laneInt1, ::ad::physics::ParametricValue(0.6)),
            resultInterval);
  resultInterval = laneInt1;
  resultInterval.end = ::ad::physics::ParametricValue(0.6);
  ASSERT_EQ(cutIntervalAtEnd(laneInt1, ::ad::physics::ParametricValue(0.6)),
            resultInterval);

  laneInt1.start = ::ad::physics::ParametricValue(0.5);
  laneInt1.end = ::ad::physics::ParametricValue(0.3);
  resultInterval =
      shortenIntervalFromBegin(laneInt1, physics::Distance(17.2351));
  ...
  resultInterval =
      restrictIntervalFromBegin(laneInt1, physics::Distance(17.2351));
  ASSERT_EQ(resultInterval.laneId, laneInt1.laneId);
...
TEST_F(LaneIntervalOperationTest, GetEdgeAndBorder) {
  ...
  border_ecef_road =
      getECEFBorderOfRoadSegment(roadSeg, physics::ParametricValue(0.1));
  ...
  border_geo_road =
      getGeoBorderOfRoadSegment(roadSeg, physics::ParametricValue(0.1));
  ...
  edge_enu1.push_back(
      mCoordinateTransform.ECEF2ENU(basePoint + createECEFPoint(2., 0., 0.)));
  edge_enu1.push_back(
      mCoordinateTransform.ECEF2ENU(basePoint + createECEFPoint(8., 0., 0.)));
  edge_enu2.push_back(
      mCoordinateTransform.ECEF2ENU(basePoint + createECEFPoint(2., 10., 0.)));
  edge_enu2.push_back(
      mCoordinateTransform.ECEF2ENU(basePoint + createECEFPoint(8., 10., 0.)));
  ASSERT_EQ(border_enu1.left, edge_enu1);
  ASSERT_EQ(border_enu1.right, edge_enu2);

  lane::ENUBorder border_enu_road;
  border_enu_road =
      getENUBorderOfRoadSegment(roadSeg, physics::ParametricValue(0.1));
...
TEST_F(LaneIntervalOperationTest, GetProjectedENUEdgeOnTown01) {
  ...
  access::setENUReferencePoint(
      createGeoPoint(Longitude(8.0), Latitude(49.0), Altitude(0.)));
...
TEST_F(LaneIntervalOperationTest, GetProjectedOffsetOnNeighborLane) {
  ...
  EXPECT_THROW(getProjectedParametricOffsetOnNeighborLane(
                   laneInt1, laneInt2, ::ad::physics::ParametricValue(0.8)),
               std::invalid_argument);

  laneInt1.laneId = x12;
  laneInt2.laneId = x12;
  paraVal = getProjectedParametricOffsetOnNeighborLane(
      laneInt1, laneInt2, ::ad::physics::ParametricValue(0.8));
...
TEST_F(LaneIntervalOperationTest, RouteInterval) {
  ...
  para =
      getLaneParaPoint(physics::ParametricValue(0.5), laneSegment.laneInterval);
  ASSERT_EQ(para.laneId, x12);
  ...
  EXPECT_THROW(
      signedDistanceToLane(lane::LaneId(100), emptyRoute, centerMapMatched),
      std::runtime_error);
