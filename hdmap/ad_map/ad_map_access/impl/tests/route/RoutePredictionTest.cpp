...
struct RoutePredictionTest : ::testing::Test {
  ...
  virtual void SetUp() {
    ...
    auto mapMatchingResults = mapMatching.getMapMatchedPositions(
        predictionStartGeo, physics::Distance(1.), physics::Probability(0.8));

    ASSERT_GE(mapMatchingResults.size(), 1u);
    predictionStart.point = mapMatchingResults.front().lanePoint.paraPoint;
    if (lane::isLaneDirectionPositive(
            lane::getLane(predictionStart.point.laneId))) {
...
struct RoutePredictionTestTown01 : public RoutePredictionTest {
  std::string getTestMap() override { return "test_files/Town01.txt"; }
};

struct RoutePredictionTestTown03 : public RoutePredictionTest {
  std::string getTestMap() override { return "test_files/Town03.adm.txt"; }
};

TEST_F(RoutePredictionTestTown01, route_prediction_positive) {
  auto routePredictions = route::planning::predictRoutesOnDistance(
      predictionStart, physics::Distance(10.));
  EXPECT_EQ(routePredictions.size(), 1u);

  routePredictions = route::planning::predictRoutesOnDistance(
      predictionStart, physics::Distance(500.));
...
TEST_F(RoutePredictionTestTown03, route_prediction_constructor) {
  predictionStart.direction = route::planning::RoutingDirection::DONT_CARE;
  auto routePredictions = route::planning::predictRoutesOnDuration(
      predictionStart, physics::Duration(1.));
  ASSERT_EQ(routePredictions.size(), 1u);

  routePredictions = route::planning::predictRoutes(
      predictionStart, physics::Distance(500.), physics::Duration(500.));
  ASSERT_EQ(routePredictions.size(), 18u);
}

TEST_F(RoutePredictionTestTown03, route_getBasicRoutes) {
  predictionStart.direction = route::planning::RoutingDirection::DONT_CARE;
  map::route::planning::RoutePrediction routePrediction(predictionStart,
                                                        physics::Distance(10.));
...
struct RoutePredictionTestIntersection : public RoutePredictionTestTown01 {
  point::GeoPoint getPredictionStartGeo() override {
    return point::createGeoPoint(point::Longitude(8.0012071),
                                 point::Latitude(48.9971953),
                                 point::Altitude(0.));
  }
};

TEST_F(RoutePredictionTestIntersection,
       route_prediction_dont_stop_within_intersections) {
  auto routePredictions = route::planning::predictRoutesOnDistance(
      predictionStart, physics::Distance(7.));
  EXPECT_EQ(routePredictions.size(), 2u);

  for (auto &routePrediction : routePredictions) {
    auto intersections =
        intersection::Intersection::getIntersectionsForRoute(routePrediction);
    ASSERT_EQ(1u, intersections.size());
  }
}

TEST_F(RoutePredictionTestIntersection,
       route_extension_dont_stop_within_intersections) {
  auto routePredictions = route::planning::predictRoutesOnDistance(
      predictionStart, physics::Distance(1.));
  EXPECT_EQ(routePredictions.size(), 1u);

  route::FullRoute route = routePredictions.front();
  routePredictions.clear();

  EXPECT_TRUE(route::extendRouteToDistance(route, physics::Distance(7.),
                                           routePredictions));
...
struct RoutePredictionTestRoundabout : public RoutePredictionTestTown03 {
  point::GeoPoint getPredictionStartGeo() override {
    return point::createGeoPoint(point::Longitude(8.0004625),
                                 point::Latitude(49.0000714),
                                 point::Altitude(0.));
  }
};
...
TEST_F(RoutePredictionTestRoundabout, route_prediction) {
  auto routePredictions = route::planning::predictRoutesOnDistance(
      predictionStart, physics::Distance(50.));
