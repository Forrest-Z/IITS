...
struct RoutePlanningTest : ::testing::Test {
  ...
  virtual void SetUp() { access::cleanup(); }

  virtual void TearDown() { access::cleanup(); }
  ...
  route::FullRoute prepareTpkRoute() {
    ...
    /* 437 */ startLaneId = lane::uniqueLaneId(createGeoPoint(
        Longitude(8.4404755), Latitude(49.0195732), Altitude(0.)));  // id: 2682
    /* 314 */ firstLaneId = lane::uniqueLaneId(createGeoPoint(
        Longitude(8.4406803), Latitude(49.0197914),
        Altitude(
            0.)));  // lane inside intersection, todo more description, id: 2462
    /* 402 */ secondLaneId = lane::uniqueLaneId(createGeoPoint(
        Longitude(8.4407235), Latitude(49.0197907),
        Altitude(
            0.)));  // lane after intersection, todo more description, id: 3018
    /* 138 */ thirdLaneId = lane::uniqueLaneId(createGeoPoint(
        Longitude(8.4418464), Latitude(49.0193326),
        Altitude(
            0.)));  // lane inside intersection, todo more description, id: 2227
    /* 247 */ fourthLaneId = lane::uniqueLaneId(createGeoPoint(
        Longitude(8.4421272), Latitude(49.0192331),
        Altitude(
            0.)));  // lane inside intersection, todo more description, id: 1093
    /* 408 */ fifthLaneId = lane::uniqueLaneId(createGeoPoint(
        Longitude(8.4421012), Latitude(49.0192655),
        Altitude(
            0.)));  // lane inside intersection, todo more description, id: 1071
    /* 415 */ endLaneId = lane::uniqueLaneId(createGeoPoint(
        Longitude(8.4422283), Latitude(49.0192052), Altitude(0.)));

    return route::planning::planRoute(
        createParaPoint(startLaneId, physics::ParametricValue(0.2)),
  ...
  void validateRouteConnections(
      route::FullRoute const &route,
      RouteBehaviour const routeBehavior = RouteIsConstant);
  ...
  std::vector<std::pair<GeoPoint, size_t>> mTestPoints{
      std::make_pair(createGeoPoint(Longitude(8.4400665), Latitude(49.0192005),
                                    Altitude(0.)),
                     0u),
      std::make_pair(createGeoPoint(Longitude(8.4401882), Latitude(49.0191939),
                                    Altitude(0.)),
                     1u),
      std::make_pair(createGeoPoint(Longitude(8.4401510), Latitude(49.0191792),
                                    Altitude(0.)),
                     2u),
      std::make_pair(createGeoPoint(Longitude(8.4401742), Latitude(49.0192009),
                                    Altitude(0.)),
                     3u),
      // approaching middle of the intersection
      std::make_pair(createGeoPoint(Longitude(8.4401540), Latitude(49.0192082),
                                    Altitude(0.)),
                     4u)};
...
void RoutePlanningTest::compareRoutes(
    route::FullRoute const &left, route::FullRoute const &right,
  ...
  for (std::size_t i = 0u; i < left.roadSegments.size(); ++i) {
    EXPECT_EQ(left.roadSegments[i].boundingSphere,
              right.roadSegments[i].boundingSphere);
    EXPECT_EQ(left.roadSegments[i].segmentCountFromDestination,
              right.roadSegments[i].segmentCountFromDestination);
    EXPECT_EQ(left.roadSegments[i].drivableLaneSegments.size(),
              right.roadSegments[i].drivableLaneSegments.size());
    for (std::size_t j = 0u;
         j < left.roadSegments[i].drivableLaneSegments.size(); ++j) {
      EXPECT_EQ(
          left.roadSegments[i].drivableLaneSegments[j].laneInterval.laneId,
          right.roadSegments[i].drivableLaneSegments[j].laneInterval.laneId);
      EXPECT_EQ(
          left.roadSegments[i].drivableLaneSegments[j].laneInterval.wrongWay,
          right.roadSegments[i].drivableLaneSegments[j].laneInterval.wrongWay);
      EXPECT_EQ(
          left.roadSegments[i].drivableLaneSegments[j].laneInterval.start,
          right.roadSegments[i].drivableLaneSegments[j].laneInterval.start);

      if ((alignmentBehavior == EndSegmentDiffers) &&
          (i + 1 == left.roadSegments.size())) {
        EXPECT_NE(
            left.roadSegments[i].drivableLaneSegments[j].laneInterval.end,
...
void RoutePlanningTest::validateRouteParaPoints(route::FullRoute const &route) {
  for (std::size_t i = 0u; i < route.roadSegments.size(); ++i) {
    for (std::size_t j = 0u;
         j < route.roadSegments[i].drivableLaneSegments.size(); ++j) {
      ...
      if ((i != 0u) && (i != route.roadSegments.size() - 1u)) {
        EXPECT_TRUE(
            (physics::ParametricValue(0.) ==
             route.roadSegments[i]
                 .drivableLaneSegments[j]
                 .laneInterval.start) ||
            (physics::ParametricValue(1.) ==
             route.roadSegments[i].drivableLaneSegments[j].laneInterval.start));
        EXPECT_TRUE(
            (physics::ParametricValue(0.) ==
             route.roadSegments[i].drivableLaneSegments[j].laneInterval.end) ||
            (physics::ParametricValue(1.) ==
             route.roadSegments[i].drivableLaneSegments[j].laneInterval.end));
      }
      EXPECT_NE(
          isRouteDirectionAlignedWithDrivingDirection(
...
void RoutePlanningTest::validateRouteConnections(
    route::FullRoute const &route, RouteBehaviour const routeBehavior) {
  ...
  for (std::size_t i = 0u; i < route.roadSegments.size(); ++i) {
    ...
    } else if (route.roadSegments[i].drivableLaneSegments[0].routeLaneOffset >
               0) {
      EXPECT_EQ(RouteIsNarrowing, routeBehavior);
    }
    for (std::size_t j = 0u;
         j < route.roadSegments[i].drivableLaneSegments.size(); ++j) {
      EXPECT_LE(route.roadSegments[i].drivableLaneSegments[j].routeLaneOffset,
                route.maxLaneOffset)
          << "i:" << i << " j:" << j << " route:" << route;
      EXPECT_GE(route.roadSegments[i].drivableLaneSegments[j].routeLaneOffset,
                route.minLaneOffset)
          << "i:" << i << " j:" << j << " route:" << route;
      ...
      actualPresentLaneIds[i].insert(
          route.roadSegments[i].drivableLaneSegments[j].laneInterval.laneId);
      listedPredecessors[i].insert(
          std::begin(
              route.roadSegments[i].drivableLaneSegments[j].predecessors),
          std::end(route.roadSegments[i].drivableLaneSegments[j].predecessors));
      listedSuccessors[i].insert(
          std::begin(route.roadSegments[i].drivableLaneSegments[j].successors),
  ...
  for (std::size_t i = 0u; i < route.roadSegments.size(); ++i) {
    for (std::size_t j = 0u;
         j < route.roadSegments[i].drivableLaneSegments.size(); ++j) {
      for (auto predecessorId :
           route.roadSegments[i].drivableLaneSegments[j].predecessors) {
        // no predecessors in first road segment allowed
        ASSERT_NE(i, 0u);
        ASSERT_TRUE(actualPresentLaneIds[i - 1].find(predecessorId) !=
                    actualPresentLaneIds[i - 1].end());
      }
      for (auto successorId :
           route.roadSegments[i].drivableLaneSegments[j].successors) {
        // no successors in last road segment allowed
        ASSERT_NE(i, route.roadSegments.size() - 1u);
        ASSERT_TRUE(actualPresentLaneIds[i + 1].find(successorId) !=
                    actualPresentLaneIds[i + 1].end());
      }
      if (j == 0u) {
        ASSERT_EQ(route.roadSegments[i].drivableLaneSegments[j].rightNeighbor,
                  lane::LaneId());
      } else {
        ASSERT_TRUE(
            actualPresentLaneIds[i].find(
                route.roadSegments[i].drivableLaneSegments[j].rightNeighbor) !=
            actualPresentLaneIds[i].end());
      }
      if (j + 1 == route.roadSegments[i].drivableLaneSegments.size()) {
        ASSERT_EQ(route.roadSegments[i].drivableLaneSegments[j].leftNeighbor,
                  lane::LaneId());
      } else {
        ASSERT_TRUE(
            actualPresentLaneIds[i].find(
                route.roadSegments[i].drivableLaneSegments[j].leftNeighbor) !=
            actualPresentLaneIds[i].end());
      }
    }
    if (i > 0u) {
      ...
      if (actualPresentLaneIds[i - 1].size() > listedPredecessors[i].size()) {
        ...
        ASSERT_EQ(actualPresentLaneIds[i - 1].size(),
                  listedPredecessors[i].size());
      }
    }
...
TEST_F(RoutePlanningTest, route_planning) {
  ...
  startLaneId = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4404755), Latitude(49.0195732), Altitude(0.)));
  endLaneId = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4422283), Latitude(49.0192052), Altitude(0.)));
  auto startLaneDrivingDirectionNegative = lane::uniqueLaneId(createGeoPoint(
      Longitude(8.43988334), Latitude(49.02012825), Altitude(0.)));
  std::vector<
      std::pair<std::pair<RoutingParaPoint, RoutingParaPoint>, RouteValues>>
      routesToPlan{
          std::make_pair(
              std::make_pair(createRoutingPoint(startLaneId,
                                                physics::ParametricValue(0.2)),
                             createRoutingPoint(startLaneId,
          ...
          std::make_pair(
              std::make_pair(
                  ...
                  createRoutingPoint(startLaneId, physics::ParametricValue(0.7),
                                     RoutingDirection::POSITIVE)),
              RouteValues{1u, 35.2}),
          // ENUHeading of zero should be positive direction (lane is oriented
          // towards north/east
          std::make_pair(
              std::make_pair(createRoutingPoint(
                                 createParaPoint(startLaneId,
                                                 physics::ParametricValue(0.2)),
                                 ENUHeading(0.)),
                             createRoutingPoint(startLaneId,
                                                physics::ParametricValue(0.7))),
          ...
          std::make_pair(
              std::make_pair(
                  ...
                  createRoutingPoint(startLaneId,
                                     physics::ParametricValue(0.7))),
              RouteValues{0u, 0.}),
          // ENUHeading of 3.5 should be negative direction (lane is oriented
          // towards north/east
          std::make_pair(
              std::make_pair(createRoutingPoint(
                                 createParaPoint(startLaneId,
          ...
          std::make_pair(
              std::make_pair(
                  createRoutingPoint(startLaneDrivingDirectionNegative,
                                     physics::ParametricValue(0.7)),
                  createRoutingPoint(startLaneDrivingDirectionNegative,
                                     physics::ParametricValue(0.2))),
          ...
          std::make_pair(
              std::make_pair(
                  ...
                  createRoutingPoint(startLaneDrivingDirectionNegative,
                                     physics::ParametricValue(0.2))),
              RouteValues{1u, 95.75}),
          // ENUHeading of 0. should be positive lane direction and so negative
          // routing direction (lane is orientated towards
          // south/east)
          std::make_pair(
              std::make_pair(
                  createRoutingPoint(
                      createParaPoint(startLaneDrivingDirectionNegative,
                                      physics::ParametricValue(0.7)),
                      ENUHeading(0.)),
                  createRoutingPoint(startLaneDrivingDirectionNegative,
                                     physics::ParametricValue(0.2))),
  ...
  for (auto routeToPlan : routesToPlan) {
    auto routeResult = route::planning::planRoute(routeToPlan.first.first,
                                                  routeToPlan.first.second);

    auto routeLength = route::calcLength(routeResult);

    ASSERT_EQ(routeToPlan.second.routeSize, routeResult.roadSegments.size())
        << mTestFile;
    ASSERT_NEAR(double(routeToPlan.second.routeLength), double(routeLength), 1.)
        << mTestFile;
...
TEST_F(RoutePlanningTest, empty_route_following) {
  ...
  auto const paraPoint =
      createParaPoint(lane::LaneId(437), physics::ParametricValue(0.1));
...
TEST_F(RoutePlanningTest, empty_mapmatched_bounding_box) {
  ...
  auto findWaypointResult =
      route::objectOnRoute(emptyObjectBoundingBox, emptyRoute);
  ASSERT_FALSE(findWaypointResult.isValid());
}
...
TEST_F(RoutePlanningTest, route_following) {
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::SucceededBeforeRoute,
      route::shortenRoute(
          createParaPoint(startLaneId, physics::ParametricValue(0.1)), route));
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(startLaneId, physics::ParametricValue(0.4)), route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(0.4);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);

  // position exactly at the end of the segment: first segment will degenerate,
  // so drop it
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(startLaneId, physics::ParametricValue(1.)), route));
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(firstLaneId, physics::ParametricValue(.01)), route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.01);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(firstLaneId, physics::ParametricValue(.5)), route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.5);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);

  // store current state before entering the next segment (which has negative
  // driving direction!)
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(secondLaneId, physics::ParametricValue(1.)), route));
  eraseFirstSegment(compareRoute);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(secondLaneId, physics::ParametricValue(.5)), route));
  eraseFirstSegment(compareRoute);
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.5);
  compareRoutes(compareRoute, route);
  ...
  ASSERT_EQ(route::ShortenRouteResult::SucceededBeforeRoute,
            route::shortenRoute(
                createParaPoint(secondLaneId, physics::ParametricValue(0.51)),
                route));
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::SucceededBeforeRoute,
      route::shortenRoute(
          createParaPoint(secondLaneId, physics::ParametricValue(0.6)), route));
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(route::ShortenRouteResult::Succeeded,
            route::shortenRoute(
                createParaPoint(secondLaneId, physics::ParametricValue(0.49)),
                route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.49);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(secondLaneId, physics::ParametricValue(0.4)), route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.4);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(secondLaneId, physics::ParametricValue(0.)), route));
  eraseFirstSegment(compareRoute);
  eraseFirstSegment(compareRoute);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(thirdLaneId, physics::ParametricValue(0.)), route));
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(thirdLaneId, physics::ParametricValue(0.5)), route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.5);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(thirdLaneId, physics::ParametricValue(0.49)), route));
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::SucceededBeforeRoute,
      route::shortenRoute(
          createParaPoint(thirdLaneId, physics::ParametricValue(0.4)), route));
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);

  // jitter the same, but with PrependIfSucceededBeforeRoute shorten route mode,
  // should adapt the route
  // and expand to the front again
  ASSERT_EQ(route::ShortenRouteResult::Succeeded,
            route::shortenRoute(
                createParaPoint(thirdLaneId, physics::ParametricValue(0.4)),
                route, route::ShortenRouteMode::PrependIfSucceededBeforeRoute));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.4);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(thirdLaneId, physics::ParametricValue(0.51)), route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.51);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(thirdLaneId, physics::ParametricValue(0.6)), route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.6);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(fourthLaneId, physics::ParametricValue(0.5)), route));
  eraseFirstSegment(compareRoute);
  eraseFirstSegment(compareRoute);
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.5);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(endLaneId, physics::ParametricValue(0.2)), route));
  eraseFirstSegment(compareRoute);
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.2);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(endLaneId, physics::ParametricValue(0.69)), route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.69);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);

  // finishing exactly
  ASSERT_EQ(
      route::ShortenRouteResult::SucceededRouteEmpty,
      route::shortenRoute(
          createParaPoint(endLaneId, physics::ParametricValue(0.7)), route));
  eraseFirstSegment(compareRoute);
  compareRoutes(compareRoute, route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::SucceededRouteEmpty,
      route::shortenRoute(
          createParaPoint(endLaneId, physics::ParametricValue(0.71)), route));
  compareRoutes(compareRoute, route);
  ASSERT_TRUE(route.roadSegments.empty());

  // nothing left
  ASSERT_EQ(
      route::ShortenRouteResult::FailedRouteEmpty,
      route::shortenRoute(
          createParaPoint(endLaneId, physics::ParametricValue(0.7)), route));
  ...
  ASSERT_EQ(route::ShortenRouteResult::FailedRouteEmpty,
            route::shortenRoute(createParaPoint(lane::LaneId(123456),
                                                physics::ParametricValue(0.7)),
                                route));
  ASSERT_TRUE(route.roadSegments.empty());

  // DontCutIntersectionAndPrependIfSucceededBeforeRoute mode
  // jump at end of second lane: ensure correct handling on degenerated handling
  // at the beginning
  route = routeResult;
  compareRoute = routeResult;
  ASSERT_EQ(
      route::ShortenRouteResult::SucceededIntersectionNotCut,
      route::shortenRoute(
          createParaPoint(secondLaneId, physics::ParametricValue(0.)), route,
          ShortenRouteMode::
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::SucceededIntersectionNotCut,
      route::shortenRoute(
          createParaPoint(thirdLaneId, physics::ParametricValue(1.)), route,
          ShortenRouteMode::
              DontCutIntersectionAndPrependIfSucceededBeforeRoute));
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
  ...
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          createParaPoint(endLaneId, physics::ParametricValue(0.5)), route,
          ShortenRouteMode::
              DontCutIntersectionAndPrependIfSucceededBeforeRoute));
  eraseFirstSegment(compareRoute);
  eraseFirstSegment(compareRoute);
...
TEST_F(RoutePlanningTest, route_following_multiple_choices) {
  ...
  ASSERT_EQ(route::ShortenRouteResult::Succeeded,
            route::shortenRoute(
                std::vector<ParaPoint>{
                    createParaPoint(firstLaneId, physics::ParametricValue(.8)),
  ...
  // entering next segment with the second choice, first choice keeps segment
  // and is therefore nearer
  ASSERT_EQ(
      route::ShortenRouteResult::Succeeded,
      route::shortenRoute(
          std::vector<ParaPoint>{
              createParaPoint(firstLaneId, physics::ParametricValue(.98)),
              createParaPoint(secondLaneId, physics::ParametricValue(.01))},
          route));
  compareRoute.roadSegments[0].drivableLaneSegments[0].laneInterval.start =
      physics::ParametricValue(.98);
  compareRoutes(compareRoute, route);
  validateRouteConnections(route);
#if 0
...
TEST_F(RoutePlanningTest, find_waypoint_result) {
  ...
  auto findWaypointResult = route::findWaypoint(
      createParaPoint(firstLaneId, physics::ParametricValue(.5)), routeResult);

  uint64_t expectedSegmentCount = 6u;
  ASSERT_TRUE(findWaypointResult.isValid());

  lane::LaneId foundLaneId =
      findWaypointResult.laneSegmentIterator->laneInterval.laneId;
  route::SegmentCounter foundSegmentCounter =
      findWaypointResult.roadSegmentIterator->segmentCountFromDestination;
  ...
  expectedSegmentCount =
      findWaypointResult.roadSegmentIterator->segmentCountFromDestination - 1;
  for (auto successorLanes = findWaypointResult.getSuccessorLanes();
       !successorLanes.empty();
       successorLanes = successorLanes[0].getSuccessorLanes(),
            expectedSegmentCount--) {
    ASSERT_TRUE(successorLanes[0].isValid());
    foundSegmentCounter =
        successorLanes[0].roadSegmentIterator->segmentCountFromDestination;
    ASSERT_EQ(foundSegmentCounter, expectedSegmentCount);
  }
  ...
  expectedSegmentCount =
      findWaypointResult.roadSegmentIterator->segmentCountFromDestination + 1;
  for (auto predecessorLanes = findWaypointResult.getPredecessorLanes();
       !predecessorLanes.empty();
       predecessorLanes = predecessorLanes[0].getPredecessorLanes(),
            expectedSegmentCount++) {
    ASSERT_TRUE(predecessorLanes[0].isValid());
    foundSegmentCounter =
        predecessorLanes[0].roadSegmentIterator->segmentCountFromDestination;
    ASSERT_EQ(foundSegmentCounter, expectedSegmentCount);
  }
...
TEST_F(RoutePlanningTest, get_route_section) {
  ...
  route::FullRoute routeSection = route::getRouteSection(
      createParaPoint(firstLaneId, physics::ParametricValue(.5)),
      physics::Distance(1.), physics::Distance(1.), routeResult);

  validateRouteConnections(routeSection);
  ASSERT_NEAR(2., double(route::calcLength(routeSection)), 0.1);

  routeSection = route::getRouteSection(
      createParaPoint(firstLaneId, physics::ParametricValue(.5)),
      physics::Distance(10.), physics::Distance(10.), routeResult);
  ...
  routeSection = route::getRouteSection(
      createParaPoint(firstLaneId, physics::ParametricValue(.5)),
      physics::Distance(50.), physics::Distance(80.), routeResult);
  ...
  routeSection = route::getRouteSection(
      createParaPoint(firstLaneId, physics::ParametricValue(.5)),
      physics::Distance(1000.), physics::Distance(1000.), routeResult);
...
TEST_F(RoutePlanningTest, route_planning_multi_lane_changes_OstringKaeppelstr) {
  ...
  startLaneId = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4396445), Latitude(49.0186033), Altitude(0.)));
  auto rightLaneId = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4394605), Latitude(49.0183941), Altitude(0.)));
  auto leftLaneId = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.439489), Latitude(49.0183852), Altitude(0.)));

  route::FullRoute routeLeft = route::planning::planRoute(
      createParaPoint(startLaneId, physics::ParametricValue(0.2)),
      createParaPoint(leftLaneId, physics::ParametricValue(.5)));
  route::FullRoute routeRight = route::planning::planRoute(
      createParaPoint(startLaneId, physics::ParametricValue(0.2)),
  ...
  ASSERT_TRUE(extendRouteToDistance(
      routeLeft, routeLength + physics::Distance(30.), additionalRoutes));
...
TEST_F(RoutePlanningTest, routing_point) {
  ...
  occupiedRegion.longitudinalRange.minimum =
      ::ad::physics::ParametricValue(0.2);
  occupiedRegion.longitudinalRange.maximum =
      ::ad::physics::ParametricValue(0.8);
  ...
  routingPoint1 =
      createRoutingPoint(occupiedRegion, RoutingDirection::POSITIVE);
  ...
  routingPoint2 =
      createRoutingPoint(occupiedRegion, point::createENUHeading(M_PI_2));
...
TEST_F(RoutePlanningTest, route_para_point) {
  ...
  ASSERT_TRUE(
      getRouteParaPointFromParaPoint(paraPoint1, routeLeft, routeParaPoint1));
  ASSERT_TRUE(
      getRouteParaPointFromParaPoint(paraPoint2, routeLeft, routeParaPoint2));

  dis = ad::map::route::calcLength(routeParaPoint1, routeParaPoint2, routeLeft);
  ASSERT_NEAR((double)dis, 22.1776, 0.0001);

  FindWaypointResult findWaypointResult =
      route::findWaypoint(paraPoint2, routeLeft);
  ASSERT_TRUE(findWaypointResult.isValid());
  ...
  ASSERT_FALSE(calculateRouteParaPointAtDistance(
      routeEmpty, routeParaPoint1, physics::Distance(22.1776), resultingPoint));
  ASSERT_TRUE(calculateRouteParaPointAtDistance(
      routeLeft, routeParaPoint1, physics::Distance(22.1776), resultingPoint));
  ASSERT_EQ(routeParaPoint2.routePlanningCounter,
            resultingPoint.routePlanningCounter);
  ASSERT_EQ(routeParaPoint2.segmentCountFromDestination,
            resultingPoint.segmentCountFromDestination);
  ASSERT_NEAR((double)routeParaPoint2.parametricOffset,
              (double)resultingPoint.parametricOffset, 0.0001);
  ...
  dis = addOpposingLaneSegmentToRoadSegment(
      paraPoint1, physics::Distance(1.), routeLeft.roadSegments[0], routeLeft);
...
TEST_F(RoutePlanningTest, route_match_position) {
  ...
  point_geo1 = point::createGeoPoint(Longitude(8.4396445), Latitude(49.0186033),
                                     Altitude(0.));
  point_geo2 = point::createGeoPoint(Longitude(8.439489), Latitude(49.0183852),
                                     Altitude(0.));
  match::AdMapMatching mapMatching;
  match::MapMatchedPositionConfidenceList matchPosition1, matchPosition2;
  matchPosition1 = mapMatching.getMapMatchedPositions(
      point_geo1, physics::Distance(1), physics::Probability(0.5));
  matchPosition2 = mapMatching.getMapMatchedPositions(
      point_geo2, physics::Distance(1), physics::Probability(0.5));
  ...
  FindWaypointResult findWaypointResult =
      findNearestWaypoint(matchPosition1, routeLeft);
  ASSERT_TRUE(findWaypointResult.isValid());
  ...
  boundBox1 = mapMatching.getMapMatchedBoundingBox(mObjectPosition1,
                                                   physics::Distance(1));
  boundBox2 = mapMatching.getMapMatchedBoundingBox(mObjectPosition2,
                                                   physics::Distance(1));
  ...
  ASSERT_FALSE(addOpposingLaneToRoute(paraPoint1, physics::Distance(1.),
                                      routeResult, dis));
  ASSERT_FALSE(addOpposingLaneToRoute(paraPoint2, physics::Distance(10.),
                                      routeResult, dis));

  ParaPoint paraOppoPoint1;
  auto startLaneId1 = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4404330), Latitude(49.0195249), Altitude(0.)));
  auto rightLaneId1 = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4408261), Latitude(49.0200051), Altitude(0.)));
  auto oppositeLaneId = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4403594), Latitude(49.0194988), Altitude(0.)));
  ...
  paraOppoPoint1 =
      createParaPoint(oppositeLaneId, physics::ParametricValue(0.9));
  ASSERT_TRUE(addOpposingLaneToRoute(paraOppoPoint1, physics::Distance(100.),
                                     routeLeft, dis));
...
TEST_F(RoutePlanningTest, route_plan_given_geo) {
  ...
  startRouting = createRoutingPoint(startLaneId, physics::ParametricValue(0.1),
                                    RoutingDirection::DONT_CARE);
  ...
  physics::Distance dis;
  FindWaypointResult findWaypointResult1 =
      route::findWaypoint(paraPoint2, routeLeft);
  ...
  FindWaypointResult findWaypointResult2 =
      route::findWaypoint(paraPoint2, routeLeft);
  ASSERT_TRUE(findWaypointResult2.isValid());
  ...
  startRouting = createRoutingPoint(startLaneId1, physics::ParametricValue(0.1),
                                    RoutingDirection::DONT_CARE);
  point_geo2 = point::createGeoPoint(Longitude(8.440442), Latitude(49.0199380),
                                     Altitude(0.));
  dest.clear();
  dest.push_back(point_geo2);
  routeLeft = planRoute(startRouting, dest);
  ...
  point_geo2 = point::createGeoPoint(Longitude(8.4404311), Latitude(49.0199250),
                                     Altitude(0.));
  dest.push_back(point_geo2);
  routeLeft = planRoute(startRouting, dest);
  ASSERT_EQ(routeLeft, route::FullRoute());
  ...
  dest.clear();
  point_geo1 = point::createGeoPoint(Longitude(8.4404844), Latitude(49.0195607),
                                     Altitude(0.));
  point_geo2 = point::createGeoPoint(Longitude(8.4406460), Latitude(49.0197543),
                                     Altitude(0.));
  point_geo3 = point::createGeoPoint(Longitude(8.4407874), Latitude(49.0199274),
                                     Altitude(0.));
  dest.push_back(point_geo2);
  dest.push_back(point_geo3);
  auto startLaneId3 = lane::uniqueLaneId(point_geo1);
  auto leftLaneId3 = lane::uniqueLaneId(point_geo3);
  startRouting = createRoutingPoint(startLaneId3, physics::ParametricValue(0.1),
                                    RoutingDirection::DONT_CARE);
  routeLeft = planRoute(startRouting, dest);
  paraPoint2 = createParaPoint(leftLaneId3, physics::ParametricValue(.5));
  FindWaypointResult findWaypointResult3 =
      route::findWaypoint(leftLaneId3, routeLeft);
...
TEST_F(RoutePlanningTest, connecting_route) {
  ...
  auto const emmyToAda = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4381157), Latitude(49.0208767), Altitude(0.)));
  auto const opositeToEmmyToAda =
      getContactLanes(lane::getLane(emmyToAda), lane::ContactLocation::LEFT)
          .front()
          .toLane;

  // Hirtenweg to Haid-und-Neu Str.
  auto const hirtToHaid = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4401012), Latitude(49.0181061), Altitude(0.)));
  // Haid-und-Neu Str. crossing Hirtenweg to Haid-und-Neu Str.
  auto const haidXhirtXHaid = lane::uniqueLaneId(
      createGeoPoint(Longitude(8.4404876), Latitude(49.0179614), Altitude(0.)));
  ...
  std::vector<std::tuple<
      // ego positioning within the lane (paraPoint, lateralOffset, delta lane
      // heading)
      lane::LaneId, physics::ParametricValue, physics::ParametricValue, double,
      // object positioning within the lane (paraPoint, lateralOffset, delta
      // lane heading)
      lane::LaneId, physics::ParametricValue, physics::ParametricValue, double,
      // calculate connected route distances: too short (invalid result), normal
      physics::Distance, physics::Distance,
      // expected connecting route results: type, routeA length, routeB length
      route::ConnectingRouteType, physics::Distance, physics::Distance>>
      testEntries{
          // simple standard: directly in front same direction, same lane
          std::make_tuple(emmyToAda, physics::ParametricValue(0.5),
                          physics::ParametricValue(0.5), 0., emmyToAda,
                          physics::ParametricValue(0.8),
                          physics::ParametricValue(0.5), 0.,
                          physics::Distance(0.), physics::Distance(50.),
                          route::ConnectingRouteType::Following,
                          physics::Distance(4.33336), physics::Distance(0.)),
          ...
          std::make_tuple(emmyToAda, physics::ParametricValue(0.5),
                          ...
                          physics::Distance(0.), physics::Distance(50.),
                          route::ConnectingRouteType::Following,
                          physics::Distance(0.), physics::Distance(4.32631)),
          // simple standard: directly in front other direction, other lane
          std::make_tuple(
              emmyToAda, physics::ParametricValue(0.5),
              physics::ParametricValue(0.5), 0., opositeToEmmyToAda,
              physics::ParametricValue(0.8), physics::ParametricValue(0.5), 0.,
              physics::Distance(0.), physics::Distance(50.),
              route::ConnectingRouteType::Opposing, physics::Distance(4.34512),
          ...
          std::make_tuple(emmyToAda, physics::ParametricValue(0.5),
                          ...
                          physics::Distance(0.), physics::Distance(0.)),
          // simple standard: Y connected route
          std::make_tuple(
              hirtToHaid, physics::ParametricValue(0.2),
              physics::ParametricValue(0.5), 0., haidXhirtXHaid,
              physics::ParametricValue(0.2), physics::ParametricValue(0.5), 0.,
              physics::Distance(5.), physics::Distance(50.),
              route::ConnectingRouteType::Merging, physics::Distance(53.5544),
  ...
  for (auto const &testEntry : testEntries) {
    ...
    point::ParaPoint egoParaPoint =
        point::createParaPoint(std::get<EGO_LANEID>(testEntry),
                               std::get<EGO_LANE_OFFSET_LON>(testEntry));
    egoObject.enuPosition.centerPoint = lane::getENULanePoint(
        egoParaPoint, std::get<EGO_LANE_OFFSET_LAT>(testEntry));
    egoObject.enuPosition.heading = point::createENUHeading(
        static_cast<double>(lane::getLaneENUHeading(egoParaPoint)) +
    ...
    egoObject.mapMatchedBoundingBox = mapMatching.getMapMatchedBoundingBox(
        egoObject.enuPosition, physics::Distance(1.));

    match::Object otherObject;
    point::ParaPoint otherParaPoint =
        point::createParaPoint(std::get<OTHER_LANEID>(testEntry),
                               std::get<OTHER_LANE_OFFSET_LON>(testEntry));
    otherObject.enuPosition.centerPoint = lane::getENULanePoint(
        otherParaPoint, std::get<OTHER_LANE_OFFSET_LAT>(testEntry));
    otherObject.enuPosition.heading = point::createENUHeading(
        static_cast<double>(lane::getLaneENUHeading(otherParaPoint)) +
    ...
    otherObject.mapMatchedBoundingBox = mapMatching.getMapMatchedBoundingBox(
        otherObject.enuPosition, physics::Distance(2.));

    auto const tooShortDistance = std::get<MAX_DIST_TOO_SHORT>(testEntry);
    ConnectingRoute connRoute = route::planning::calculateConnectingRoute(
        egoObject, otherObject, tooShortDistance);
    ...
    connRoute = route::planning::calculateConnectingRoute(
        egoObject, otherObject, standardDistance);
    ...
    if (route::ConnectingRouteType::Merging == connRoute.type) {
      EXPECT_EQ(
          connRoute.routeA.roadSegments.back().drivableLaneSegments.size(),
          connRoute.routeB.roadSegments.back().drivableLaneSegments.size());
      for (auto i = 0u;
           i < connRoute.routeA.roadSegments.back().drivableLaneSegments.size();
           i++) {
        EXPECT_TRUE(route::isDegenerated(connRoute.routeA.roadSegments.back()
                                             .drivableLaneSegments[i]
                                             .laneInterval));
        EXPECT_EQ(connRoute.routeA.roadSegments.back()
                      .drivableLaneSegments[i]
                      .laneInterval,
                  connRoute.routeB.roadSegments.back()
                      .drivableLaneSegments[i]
                      .laneInterval);
      }
      routeBehavior = RouteBehaviour::RouteIsBroadening;
    }
    EXPECT_EQ(std::get<RESULT_ROUTE_EGO_LENGTH>(testEntry),
              calcLength(connRoute.routeA));
    EXPECT_EQ(std::get<RESULT_ROUTE_OTHER_LENGTH>(testEntry),
              calcLength(connRoute.routeB));
