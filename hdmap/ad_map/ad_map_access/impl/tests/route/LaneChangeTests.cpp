...
struct LaneChangeTest : ::testing::Test {
  ...
  virtual void SetUp() { loadTestFile(); }

  virtual void TearDown() { ad::map::access::cleanup(); }

  match::MapMatchedPosition getPOIMapMatchedPosition(
      std::string const poiName) const;
  ...
  lane::ContactLaneList getSuccessorsAndPredecessorsForLaneId(
      lane::LaneId const &laneId) const;

  void testsLaneChangeEnd(
      route::FindLaneChangeResult const &findLaneChangeResult,
      int expectedEndIndex) const;

  void testsLaneChangeStart(
      route::FindLaneChangeResult const &findLaneChangeResult,
...
match::MapMatchedPosition LaneChangeTest::getPOIMapMatchedPosition(
    std::string const poiName) const {
  ...
  auto foundPoi =
      std::find_if(std::begin(ad::map::access::getPointsOfInterest()),
                   ...
                   [&poiName](config::PointOfInterest const &poi) {
                     return poi.name.compare(poiName) == 0;
                   });
  ...
  match::MapMatchedPositionConfidenceList mapMatchingResult =
      mapMatching.getMapMatchedPositions(foundPoi->geoPoint,
                                         ::ad::physics::Distance(0.2),
                                         physics::Probability(.8));
...
void LaneChangeTest::testsLaneChangeEnd(
    route::FindLaneChangeResult const &findLaneChangeResult,
    int expectedEndIndex) const {
  ...
  // The end of the lane change cannot be the end of the route, otherwise we
  // would not have a "next" lane to change to
  ASSERT_NE(findLaneChangeResult.laneChangeEndRouteIterator,
            std::prev(findLaneChangeResult.queryRoute.roadSegments.end()));
}

void LaneChangeTest::testsLaneChangeStart(
    route::FindLaneChangeResult const &findLaneChangeResult,
    int expectedStartIndex) const {
  // The lane change start is never at the end
  ASSERT_NE(findLaneChangeResult.laneChangeStartRouteIterator,
            std::end(findLaneChangeResult.queryRoute.roadSegments));
...
TEST_F(LaneChangeTest, current_position_not_on_route) {
  ...
  auto route =
      route::planning::planRoute(mapMatchedPositionStart.lanePoint.paraPoint,
                                 mapMatchedPositionEnd.lanePoint.paraPoint);

  // This function needs to be tested:
  auto findLaneChangeResult =
      route::findFirstLaneChange(currentPosition, route);
  ...
  ASSERT_EQ(findLaneChangeResult.laneChangeStartRouteIterator,
            std::end(route.roadSegments));
  ASSERT_EQ(findLaneChangeResult.laneChangeEndRouteIterator,
            std::end(route.roadSegments));
  ASSERT_EQ(findLaneChangeResult.laneChangeDirection,
            route::LaneChangeDirection::Invalid);
}
...
TEST_F(LaneChangeTest, find_lane_change_A_to_H_start_at_A) {
  ...
  ASSERT_EQ(findLaneChangeResult.laneChangeDirection,
            route::LaneChangeDirection::LeftToRight);
...
TEST_F(LaneChangeTest, find_lane_change_BL_to_F_start_at_BR) {
  ...
  ASSERT_EQ(findLaneChangeResult.laneChangeDirection,
            route::LaneChangeDirection::RightToLeft);
