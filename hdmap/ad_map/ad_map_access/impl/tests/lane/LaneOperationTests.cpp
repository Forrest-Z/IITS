...
struct LaneOperationTest : ::testing::Test {
  ...
  void setupSingleLongLane() {
    ...
    point::GeoPoint bottomRight = point::createGeoPoint(
        point::Longitude(8.44937788), point::Latitude(49.00736837),
        point::Altitude(0.));
    point::GeoPoint upRight = point::createGeoPoint(
        point::Longitude(8.45010064), point::Latitude(49.00884320),
        point::Altitude(0.));
    point::GeoPoint bottomLeft = point::createGeoPoint(
        point::Longitude(8.44933898), point::Latitude(49.00737633),
        point::Altitude(0.));
    point::GeoPoint upLeft = point::createGeoPoint(point::Longitude(8.45006151),
                                                   point::Latitude(49.00885163),
                                                   point::Altitude(0.));
    ...
    pFactory->add(access::PartitionId(0), laneId, lane::LaneType::NORMAL,
...
TEST_F(LaneOperationTest, getLane) {
  ...
  auto pos =
      point::createGeoPoint(point::Longitude(8.4398011),
                            point::Latitude(49.0188015), point::Altitude(0.));
  auto bad = point::createGeoPoint(point::Longitude(9.), point::Latitude(9.),
                                   point::Altitude(0.));
...
TEST_F(LaneOperationTest, getPointsOfInterest) {
  ...
  auto pos =
      point::createGeoPoint(point::Longitude(8.0026912),
                            point::Latitude(48.9970472), point::Altitude(3.1));
...
TEST_F(LaneOperationTest, getLaneHeading) {
  ...
  position.matchedPoint = point::toECEF(
      point::createGeoPoint(point::Longitude(8.4401510),
                            point::Latitude(49.0191792), point::Altitude(0.)));
  mapMatchedPositions.push_back(position);
  position.lanePoint.paraPoint.laneId = lane::uniqueLaneId(
      point::createGeoPoint(point::Longitude(8.4398226),
                            point::Latitude(49.0187687), point::Altitude(0.)));
  mapMatchedPositions.push_back(position);
  ...
  // since the two lanes are in opposite direction, the heading should differ by
  // PI
  ASSERT_NEAR(static_cast<double>(headings[0]) + M_PI,
              static_cast<double>(headings[1]), 0.1);
  headings.clear();
}
...
TEST_F(LaneOperationTest, findNearestPointOnEdge) {
  ...
  point::GeoPoint requestPoint =
      point::createGeoPoint(point::Longitude(8.4493674),
                            point::Latitude(49.0073880), point::Altitude(0.));

  match::MapMatchedPosition mmpos;
  EXPECT_TRUE(
      lane::findNearestPointOnLane(lane, point::toECEF(requestPoint), mmpos));
  EXPECT_EQ(mmpos.type, match::MapMatchedPositionType::LANE_IN);
  point::GeoPoint requestPointVeryClose =
      point::createGeoPoint(point::Longitude(8.4493662),
                            point::Latitude(49.0073866), point::Altitude(0.));
  EXPECT_TRUE(lane::findNearestPointOnLane(
      lane, point::toECEF(requestPointVeryClose), mmpos));
  EXPECT_EQ(mmpos.type, match::MapMatchedPositionType::LANE_IN);
  point::GeoPoint requestPointVeryFar =
      point::createGeoPoint(point::Longitude(9.4493662),
                            point::Latitude(49.0073866), point::Altitude(0.));
  EXPECT_TRUE(lane::findNearestPointOnLane(
      lane, point::toECEF(requestPointVeryFar), mmpos));
  ...
  serializer.open("test_files/test_straight.adm", versionMajorWrite,
                  versionMinorWrite);
...
TEST_F(LaneOperationTest, LaneRelation) {
  ...
  ASSERT_TRUE(isPhysicalPredecessor(*(lane::getLanePtr(x11)),
                                    *(lane::getLanePtr(x12))));
  ASSERT_TRUE(
      isPyhsicalSuccessor(*(lane::getLanePtr(x12)), *(lane::getLanePtr(x11))));
  ...
  ASSERT_EQ(calcWidth(x12, physics::ParametricValue(0.5)),
            physics::Distance(2.));
  ...
  ECEFPoint ecef_pt1;
  ecef_pt1 = getProjectedParametricPoint(*(lane::getLanePtr(x12)),
  ...
  pFactory->set(x12, geo1, geo2);
  ASSERT_TRUE(
      isPyhsicalSuccessor(*(lane::getLanePtr(x11)), *(lane::getLanePtr(x12))));
  ...
  pFactory->add(p, x12, lane::LaneType::NORMAL, lane::LaneDirection::POSITIVE);
  pFactory->set(x12, geo1, geo2);
  ASSERT_FALSE(isPhysicalPredecessor(*(lane::getLanePtr(x12)),
                                     *(lane::getLanePtr(x11))));
}
...
TEST_F(LaneOperationTest, LanePoint) {
  ...
  pFactory->add(p, x11, lane::LaneType::NORMAL, lane::LaneDirection::POSITIVE);
  pFactory->set(x11, geo1, geo2);
  ecef_pt1 = getParametricPoint(*(lane::getLanePtr(x11)),
                                physics::ParametricValue(0.5),
                                physics::ParametricValue(0.5));
  ...
  all_vehicles.roadUserTypes = {restriction::RoadUserType::CAR,
  ...
  ASSERT_TRUE(projectPositionToLaneInHeadingDirection(para1, inverseLaneHeading,
                                                      para2));

  ASSERT_FALSE(
      satisfiesFilter(*(lane::getLanePtr(x11)), std::string("abc"), false));
...
TEST_F(LaneOperationTest, BoarderOperation) {
  ...
  border_enu1.right = edge_enu2;
  edge_enu3 =
      getLateralAlignmentEdge(border_enu1, physics::ParametricValue(0.5));
  edge_enu4.clear();
  ...
  physics::Distance dis = getDistanceEnuPointToLateralAlignmentEdge(
      createENUPoint(101, 202, 1), edge_enu3);
