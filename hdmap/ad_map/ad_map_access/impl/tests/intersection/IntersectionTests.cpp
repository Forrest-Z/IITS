...
struct IntersectionTest : ::testing::Test {
  ...
  void compareParaLists(point::ParaPointList resultParaPoints,
                        point::ParaPointList expectedParaPoints) {
    std::vector<point::ParaPoint> idVector(resultParaPoints.begin(),
                                           resultParaPoints.end());
    std::vector<point::ParaPoint> otherIdVector(expectedParaPoints.begin(),
                                                expectedParaPoints.end());

    auto compareParaWithLaneId = [](point::ParaPoint &left,
                                    point::ParaPoint &right) {
      return left.laneId < right.laneId;
    };
    std::sort(idVector.begin(), idVector.end(), compareParaWithLaneId);
    std::sort(otherIdVector.begin(), otherIdVector.end(),
              compareParaWithLaneId);
    ASSERT_EQ(idVector.size(), otherIdVector.size());
...
TEST_F(IntersectionTest, create_intersections_all_way_stop) {
  ...
  auto fromWest =
      createGeoPoint(Longitude(8.437270), Latitude(49.018665), Altitude(0.));
  lane::LaneId fromWestId = lane::uniqueLaneId(fromWest);
  auto toEast =
      createGeoPoint(Longitude(8.437818), Latitude(49.018672), Altitude(0.));
  lane::LaneId toEastId = lane::uniqueLaneId(toEast);
  ASSERT_NE(fromWestId, toEastId);
  point::ParaPoint routeStart(
      point::createParaPoint(fromWestId, physics::ParametricValue(0.8)));
  point::ParaPoint routeEnd(
      point::createParaPoint(toEastId, physics::ParametricValue(0.5)));
  auto fullRoute = route::planning::planRoute(routeStart, routeEnd);

  auto intersections =
      intersection::Intersection::getIntersectionsForRoute(fullRoute);
  ...
  ::ad::physics::ParametricValue parametricOffset =
      intersection->outgoingParaPointsOnRoute()[0].parametricOffset;
  ASSERT_EQ(physics::ParametricValue(0.), parametricOffset);
  auto incoming = intersection->incomingLanes();
  ASSERT_EQ(3u, incoming.size());
  lane::LaneIdSet expectedIncoming{lane::LaneId{248}, lane::LaneId{252},
                                   lane::LaneId{264}};
  compareLists(incoming, expectedIncoming);
  auto internal = intersection->internalLanes();
  ASSERT_EQ(12u, internal.size());
  lane::LaneIdSet expectedInternal{
      lane::LaneId{63},  lane::LaneId{76},  lane::LaneId{98},
      lane::LaneId{110}, lane::LaneId{132}, lane::LaneId{144},
      lane::LaneId{165}, lane::LaneId{176}, lane::LaneId{197},
      lane::LaneId{208}, lane::LaneId{229}, lane::LaneId{240}};
  ...
  expectedLaneIdSets =
      lane::LaneIdSet({lane::LaneId{63}, lane::LaneId{98}, lane::LaneId{165},
                       lane::LaneId{176}, lane::LaneId{229}});
  compareLists(resultLaneIdSets, expectedLaneIdSets);

  resultLaneIdSets = intersection->outgoingLanes();
  expectedLaneIdSets = lane::LaneIdSet({lane::LaneId{244}, lane::LaneId{256},
                                        lane::LaneId{260}, lane::LaneId{272}});
  compareLists(resultLaneIdSets, expectedLaneIdSets);

  resultLaneIdSets = intersection->entryLanes();
  expectedLaneIdSets = lane::LaneIdSet({lane::LaneId{248}, lane::LaneId{252},
                                        lane::LaneId{264}, lane::LaneId{268}});
  compareLists(resultLaneIdSets, expectedLaneIdSets);
...
TEST_F(IntersectionTest,
       create_intersection_from_route_ending_within_intersection) {
  ASSERT_NO_THROW(::map_setup::prepareMapAllWayStop());
  point::ParaPoint incomingLane(
      point::createParaPoint(lane::LaneId{268}, physics::ParametricValue(0.8)));
  point::ParaPoint laneWithinIntersection(
      point::createParaPoint(lane::LaneId{110}, physics::ParametricValue(0.5)));
  auto fullRoute =
      route::planning::planRoute(incomingLane, laneWithinIntersection);
...
TEST_F(IntersectionTest, traffic_lights_pfz_elf_to_rusch) {
  ...
  // inner lane (1990)
  auto fromSouth =
      createGeoPoint(Longitude(8.457617), Latitude(49.020345), Altitude(0.));
  lane::LaneId fromSouthId = lane::uniqueLaneId(fromSouth);
  // 1964
  auto toEast =
      createGeoPoint(Longitude(8.457384), Latitude(49.020785), Altitude(0.));
  lane::LaneId toEastId = lane::uniqueLaneId(toEast);
  ASSERT_NE(fromSouthId, toEastId);
  point::ParaPoint routeStart(
      point::createParaPoint(fromSouthId, physics::ParametricValue(0.5)));
  point::ParaPoint routeEnd(
      point::createParaPoint(toEastId, physics::ParametricValue(0.5)));
  auto fullRoute = route::planning::planRoute(routeStart, routeEnd);
  ...
  auto const &priorityLanes =
      intersection->incomingParaPointsWithHigherPriority();
  ...
  geoPoint =
      point::createGeoPoint(point::Longitude(8.457552564),
                            point::Latitude(49.02053703), point::Altitude(3.));
  id1 = landmark::uniqueLandmarkId(geoPoint);
  geoPoint =
      point::createGeoPoint(point::Longitude(8.457664809),
                            point::Latitude(49.02051672), point::Altitude(5.));
  ...
  lane::LaneIdSet resultLaneIdSets, expectedLaneIdSets;
  resultLaneIdSets = intersection->lanesOnRoute();
  expectedLaneIdSets = lane::LaneIdSet(
      {lane::LaneId{360}, lane::LaneId{621}, lane::LaneId{624}});
  compareLists(resultLaneIdSets, expectedLaneIdSets);
  ...
  resultLaneIdSets = intersection->internalLanesWithLowerPriority();
  expectedLaneIdSets =
      lane::LaneIdSet({lane::LaneId{187}, lane::LaneId{199}, lane::LaneId{220},
                       lane::LaneId{231}, lane::LaneId{253}, lane::LaneId{275},
                       lane::LaneId{297}, lane::LaneId{318}, lane::LaneId{339},
                       lane::LaneId{382}, lane::LaneId{404}, lane::LaneId{425},
                       lane::LaneId{446}, lane::LaneId{467}, lane::LaneId{475},
                       lane::LaneId{479}, lane::LaneId{511}, lane::LaneId{514},
                       lane::LaneId{552}});
  compareLists(resultLaneIdSets, expectedLaneIdSets);

  resultLaneIdSets = intersection->outgoingLanes();
  expectedLaneIdSets =
      lane::LaneIdSet({lane::LaneId{471}, lane::LaneId{508}, lane::LaneId{548},
                       lane::LaneId{627}, lane::LaneId{630}});
  compareLists(resultLaneIdSets, expectedLaneIdSets);

  resultLaneIdSets = intersection->entryLanes();
  expectedLaneIdSets =
      lane::LaneIdSet({lane::LaneId{487}, lane::LaneId{491}, lane::LaneId{522},
                       lane::LaneId{526}, lane::LaneId{578}, lane::LaneId{588},
                       lane::LaneId{598}});
  compareLists(resultLaneIdSets, expectedLaneIdSets);
...
TEST_F(IntersectionTest, traffic_lights_pfz_rusch_to_elf) {
  ...
  // 1912
  auto toWest =
      createGeoPoint(Longitude(8.458091), Latitude(49.020521), Altitude(0.));
  lane::LaneId toWestId = lane::uniqueLaneId(toWest);
  point::ParaPoint routeStart(
      point::createParaPoint(fromSouthId, physics::ParametricValue(0.5)));
  point::ParaPoint routeEnd(
      point::createParaPoint(toWestId, physics::ParametricValue(0.5)));
  auto fullRoute = route::planning::planRoute(routeStart, routeEnd);
  ...
  landmark::LandmarkId id1, id2;
  geoPoint =
      point::createGeoPoint(point::Longitude(8.457704425),
                            point::Latitude(49.02050956), point::Altitude(5.));
  id1 = landmark::uniqueLandmarkId(geoPoint);
  geoPoint =
      point::createGeoPoint(point::Longitude(8.457737438),
                            point::Latitude(49.02050359), point::Altitude(3.));
  id2 = landmark::uniqueLandmarkId(geoPoint);
  ASSERT_EQ(1u, lights.count(id1));
  ASSERT_EQ(1u, lights.count(id2));
  ...
  resultLaneIdSets = intersection->lanesOnRoute();
  expectedLaneIdSets = lane::LaneIdSet(
      {lane::LaneId{382}, lane::LaneId{621}, lane::LaneId{624}});
  compareLists(resultLaneIdSets, expectedLaneIdSets);
  ...
  expectedLaneIdSets =
      lane::LaneIdSet({lane::LaneId{187}, lane::LaneId{199}, lane::LaneId{220},
                       lane::LaneId{231}, lane::LaneId{253}, lane::LaneId{275},
                       lane::LaneId{297}, lane::LaneId{318}, lane::LaneId{339},
                       lane::LaneId{360}, lane::LaneId{404}, lane::LaneId{425},
                       lane::LaneId{446}, lane::LaneId{467}, lane::LaneId{475},
                       lane::LaneId{479}, lane::LaneId{511}, lane::LaneId{514},
                       lane::LaneId{552}});
  compareLists(resultLaneIdSets, expectedLaneIdSets);
...
TEST_F(IntersectionTest, extract_turn_direction) {
  ...
  // straight
  {
    // 1944
    auto toNorth =
        createGeoPoint(Longitude(8.457968), Latitude(49.020877), Altitude(0.));
    lane::LaneId toNorthId = lane::uniqueLaneId(toNorth);
    point::ParaPoint routeEnd(
        point::createParaPoint(toNorthId, physics::ParametricValue(0.5)));
    auto fullRoute = route::planning::planRoute(routeStart, routeEnd);
    auto intersections =
        intersection::Intersection::getIntersectionsForRoute(fullRoute);
    ASSERT_EQ(1u, intersections.size());
    auto const &intersection = intersections[0];
    ASSERT_EQ(intersection::TurnDirection::Straight,
  ...
  // right turn
  {
    ...
    ASSERT_EQ(1u, intersections.size());
    auto const &intersection = intersections[0];
    ASSERT_EQ(intersection::TurnDirection::Right,
              intersection->turnDirection());
  }
}
...
TEST_F(IntersectionTest, validate_tpk_to_pfz_map) {
  ...
  auto start =
      point::createGeoPoint(point::Longitude(8.4571128),
                            point::Latitude(49.0169280), point::Altitude(0));
  auto end =
      point::createGeoPoint(point::Longitude(8.457508),
                            point::Latitude(49.019861), point::Altitude(0));
  match::AdMapMatching mapMatching;
  auto from = mapMatching.getMapMatchedPositions(start, physics::Distance(1),
                                                 physics::Probability(0.5));
  auto to = mapMatching.getMapMatchedPositions(end, physics::Distance(1),
                                               physics::Probability(0.5));
  ASSERT_EQ(from.size(), 1u);
  ASSERT_EQ(to.size(), 1u);
  auto fullRoute = route::planning::planRoute(from[0].lanePoint.paraPoint,
                                              to[0].lanePoint.paraPoint);
  auto intersections =
      intersection::Intersection::getIntersectionsForRoute(fullRoute);
  ASSERT_EQ(1u, intersections.size());
  auto const &intersection = intersections[0];
  ...
  ASSERT_EQ(intersection::IntersectionType::TrafficLight,
            ptrInter->intersectionType());
  ...
  route::FindWaypointResult findWaypoint =
      route::intersectionOnRoute(*intersection, fullRoute);
  ...
  route::FindWaypointResult findWaypoint1 =
      route::intersectionOnRoute(*intersection, routeConstruct);
  ASSERT_TRUE(findWaypoint1.isValid());

  ASSERT_FALSE(intersection->objectRouteCrossesIntersectionRoute(fullRoute));
  ASSERT_TRUE(
      intersection->objectRouteFromSameArmAsIntersectionRoute(fullRoute));
  ASSERT_FALSE(intersection->objectRouteOppositeToIntersectionRoute(fullRoute));
  ASSERT_FALSE(
      intersection->objectRouteCrossesLanesWithHigherPriority(fullRoute));
}
