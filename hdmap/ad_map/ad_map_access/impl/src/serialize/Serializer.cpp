...
Serializer::Serializer(bool store, bool calc_checksum) : ISerializer(store) {
  open_ = false;
...
bool Serializer::open(std::string const &config, size_t &version_major,
...
bool Serializer::close() {
  if (open_) {
    ...
    access::getLogger()->warn(
        "Serializer: Attempt to close already closed stream.");
    return false;
  }
}
...
bool Serializer::openForRead(std::string const &config, size_t &version_major,
                             size_t &version_minor) {
  if (doOpenForRead(config)) {
    ...
    size_t magic = 0;
    initChecksum();
    if (ISerializer::serialize(magic) &&
        ISerializer::serialize(version_major) &&
        ISerializer::serialize(version_minor)) {
      if (magic == MAGIC) {
        if (version_major == VERSION_MAJOR && version_minor == VERSION_MINOR) {
          ...
          access::getLogger()->error(
              "Serializer: Version mismatch: Expected {}, Found {}",
              VERSION_MAJOR, version_major);
      ...
      access::getLogger()->error("Serializer: Invalid stream (too short) {}",
                                 config);
    }
    doCloseForRead();
  } else {
    access::getLogger()->warn("Serializer: Cannot open stream for reading {}",
                              config);
  }
  return false;
}
...
bool Serializer::openForWrite(std::string const &config) {
  if (doOpenForWrite(config)) {
    ...
    if (ISerializer::serialize(MAGIC) &&
      ...
      access::getLogger()->error("Serializer: Cannot write header to steam {}",
                                 config);
    }
    doCloseForWrite();
  } else {
    access::getLogger()->error("Serializer: Cannot open stream for writing {}",
                               config);
  }
  return false;
}
