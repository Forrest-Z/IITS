...
inline ostream &operator<<(
    ostream &out,
    ::ad::map::route::planning::RouteAstar::RoutingPoint const &value) {
...
RouteAstar::RouteAstar(const RoutingParaPoint &start,
    ...
    : RouteExpander(start, dest, maxDistance, physics::Duration::getMax(),
                    typ) {
  initLanePointer();
}
...
RouteAstar::RouteAstar(const RoutingParaPoint &start,
                       const RoutingParaPoint &dest,
                       physics::Duration const &maxDuration, Type typ)
    : RouteExpander(start, dest, physics::Distance::getMax(), maxDuration,
                    typ) {
  initLanePointer();
}

RouteAstar::RouteAstar(const RoutingParaPoint &start,
                       const RoutingParaPoint &dest, Type typ)
    : RouteExpander(start, dest, physics::Distance::getMax(),
                    physics::Duration::getMax(), typ) {
  initLanePointer();
}
...
bool RouteAstar::calculate() {
  ...
  // Initial values.
  RoutingCost cost;
  cost.costData.estimatedDistanceToTarget =
      costEstimate(mStartLane, mStart.point);
  mProcessingMap.insert({mStart, cost});
  // Run
  bool pathFound = false;
#if DEBUG_OUTPUT
  std::cout << "######### (typ:" << (int)mType << " #########"
            << " start: " << mStart.point << " dest: " << mDest.point
            << std::endl;
#endif
  while (!mProcessingMap.empty()) {
    ...
    struct FScoreCompare {
      bool operator()(RoutingPoint const &left, RoutingPoint const &right) {
        return left.second.costData.estimatedDistanceToTarget <
               right.second.costData.estimatedDistanceToTarget;
      }
    };
    auto minimumCostIterator = std::min_element(
        mProcessingMap.begin(), mProcessingMap.end(), FScoreCompare());
    if (((mDest.direction == RoutingDirection::DONT_CARE) ||
         (mDest.direction == minimumCostIterator->first.direction)) &&
...
physics::Distance RouteAstar::costEstimate(lane::Lane::ConstPtr neighborLane,
                                           point::ParaPoint const &neighbor) {
  point::ECEFPoint pt_a = getParametricPoint(
      *neighborLane, neighbor.parametricOffset, physics::ParametricValue(0.5));
  point::ECEFPoint pt_b = getParametricPoint(
      *mDestLane, getDest().parametricOffset, physics::ParametricValue(0.5));
...
void RouteAstar::addNeighbor(lane::Lane::ConstPtr originLane,
  ...
  if (mProcessedPoints.find(neighbor.first) == mProcessedPoints.end()) {
    ...
    if (  // insertion succeeded
        ...
        (neighbor.second.routeDistance <
         insertResult.first->second.routeDistance)) {
      auto const estimatedDestCost =
          costEstimate(neighborLane, neighbor.first.point);
      insertResult.first->second.routeDistance = neighbor.second.routeDistance;
      insertResult.first->second.costData.estimatedDistanceToTarget =
          neighbor.second.routeDistance + estimatedDestCost;
...
void RouteAstar::reconstructPath(RoutingPoint const &dest) {
  ...
  for (auto current = dest.first;;) {
    rawRoute.paraPointList.insert(rawRoute.paraPointList.begin(),
                                  current.point);
