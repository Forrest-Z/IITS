...
Route::Route(const RoutingParaPoint &start, const RoutingParaPoint &dest,
...
Route::BasicRoute Route::getBasicRoute(size_t const routeIndex) const {
  ...
  for (size_t i = 0; i < rawRoute.paraPointList.size(); i++) {
    ...
    for (auto contactLocation :
      ...
      for (lane::Lane::ConstPtr lane = lane::getLanePtr(paraPoint.laneId);
           lane;) {
        lane::LaneDirection const direction = lane->direction;
        lane::ContactLaneList const cl =
            getContactLanes(*lane, contactLocation);
        lane = nullptr;
        for (auto contact : cl) {
          ...
          if (!isNext && !isPrev) {
            ...
            if ((direction == other_lane->direction) ||
