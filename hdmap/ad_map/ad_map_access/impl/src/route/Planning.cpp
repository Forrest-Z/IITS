...
void addParaPointToRouteDestList(
  ...
  // paraPoint
  // in this case, we need to ensure, that the order of the points is aligned to
  // the
  // driving direction. If this is the case, we *add* the new paraPoint,
  // otherwise, we
...
ConnectingRoute calculateConnectingRoute(const match::Object &objectA,
  ...
  if (resultRawRoute.paraPointList.size() > 0u) {
    ...
    // of B
    auto objectAPredictions =
        predictRoutes(objectA.mapMatchedBoundingBox, maxDistance, maxDuration);
    auto objectBPredictions =
        predictRoutes(objectB.mapMatchedBoundingBox, maxDistance, maxDuration);
...
ConnectingRoute calculateConnectingRoute(const match::Object &startObject,
  ...
  return calculateConnectingRoute(
      startObject, destObject, maxDistance,
...
ConnectingRoute calculateConnectingRoute(const match::Object &startObject,
                                         const match::Object &destObject,
                                         physics::Duration const &maxDuration) {
  return calculateConnectingRoute(startObject, destObject,
                                  std::numeric_limits<physics::Distance>::max(),
                                  maxDuration);
}
