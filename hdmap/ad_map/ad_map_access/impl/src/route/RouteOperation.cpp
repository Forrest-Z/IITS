...
RouteIterator getRouteIterator(route::RouteParaPoint const &routePosition,
  ...
  if (route.roadSegments.empty() ||
      (route.roadSegments.front().segmentCountFromDestination <
       routePosition.segmentCountFromDestination) ||
      (route.roadSegments.back().segmentCountFromDestination >
       routePosition.segmentCountFromDestination)) {
    // route position not found within route
    return result;
  }

  for (result.roadSegmentIterator = route.roadSegments.begin();
       result.roadSegmentIterator != route.roadSegments.end();
       result.roadSegmentIterator++) {
    if (result.roadSegmentIterator->segmentCountFromDestination ==
        routePosition.segmentCountFromDestination) {
      // found the segment
      break;
    }
...
point::ParaPoint getLaneParaPoint(
  ...
  if (isDegenerated(laneInterval)) {
    ...
    if (isRouteDirectionPositive(laneInterval)) {
      lanePoint.parametricOffset =
          laneInterval.start + parametricLengthOffsetFromRouteStart;
    } else {
      lanePoint.parametricOffset =
          laneInterval.start - parametricLengthOffsetFromRouteStart;
    }
...
point::ParaPointList getLaneParaPoints(
    route::RouteParaPoint const &routePosition, route::FullRoute const &route) {
  point::ParaPointList resultPoints;
  auto routeIter = getRouteIterator(routePosition, route);
  if (routeIter.isValid()) {
    for (auto &laneSegment :
         routeIter.roadSegmentIterator->drivableLaneSegments) {
      resultPoints.push_back(getLaneParaPoint(routePosition.parametricOffset,
                                              laneSegment.laneInterval));
    }
  }
  return resultPoints;
}
...
physics::Distance signedDistanceToLane(
    lane::LaneId const &checkLaneId, FullRoute const &route,
    match::MapMatchedPositionConfidenceList const &mapMatchedPositions) {
  physics::Distance distance =
      match::signedDistanceToLane(checkLaneId, mapMatchedPositions);

  auto laneInRoute = findWaypoint(checkLaneId, route);

  if (!laneInRoute.isValid()) {
    throw std::runtime_error(
        "::ad::map::route::signedDistanceToLane() laneId not found in route");
  }
...
physics::Distance calcLength(ConnectingRoute const &connectingRoute) {
  ...
  if (connectingRoute.type == ConnectingRouteType::Invalid) {
    ...
    result = std::max(calcLength(connectingRoute.routeA),
                      calcLength(connectingRoute.routeB));
  }
  return result;
}

physics::Distance calcLength(RouteIterator const &startIterator,
                             RouteIterator const &endIterator) {
  physics::Distance distance(0.);
  if (startIterator.isValid() && endIterator.isValid() &&
      (std::distance(startIterator.roadSegmentIterator,
                     endIterator.roadSegmentIterator) >= 0u)) {
    for (auto roadSegmentIter = startIterator.roadSegmentIterator;
         roadSegmentIter < endIterator.roadSegmentIterator; roadSegmentIter++) {
...
physics::Distance calcLength(RouteParaPoint const &startRouteParaPoint,
                             RouteParaPoint const &endRouteParaPoint,
                             FullRoute const &route) {
  physics::Distance distance = std::numeric_limits<physics::Distance>::max();

  const auto laneParaPointsStart =
      getLaneParaPoints(startRouteParaPoint, route);
...
physics::Duration calcDuration(ConnectingRoute const &connectingRoute) {
  physics::Duration result = std::max(calcDuration(connectingRoute.routeA),
                                      calcDuration(connectingRoute.routeB));
  return result;
}
...
restriction::SpeedLimitList getSpeedLimits(RoadSegment const &roadSegment) {
  ...
  for (auto const &laneSegment : roadSegment.drivableLaneSegments) {
    ...
    resultLimits.insert(resultLimits.end(), segmentSpeedLimits.begin(),
                        segmentSpeedLimits.end());
  }
  return resultLimits;
}
...
restriction::SpeedLimitList getSpeedLimits(RouteIterator const &startIterator,
                                           RouteIterator const &endIterator) {
  restriction::SpeedLimitList resultLimits;
  if (startIterator.isValid() && endIterator.isValid() &&
      (std::distance(startIterator.roadSegmentIterator,
...
restriction::SpeedLimitList getSpeedLimits(
    ConnectingRoute const &connectingRoute) {
  restriction::SpeedLimitList resultLimits =
      getSpeedLimits(connectingRoute.routeA);
  auto const speedLimitsB = getSpeedLimits(connectingRoute.routeA);
  resultLimits.insert(resultLimits.end(), speedLimitsB.begin(),
                      speedLimitsB.end());
  return resultLimits;
}

bool isWithinInterval(RoadSegment const &roadSegment,
...
void updateLaneSegmentNeighbors(RoadSegment &roadSegment) {
  ...
  while (rightLaneSegmentIter != std::end(roadSegment.drivableLaneSegments)) {
    leftLaneSegmentIter->rightNeighbor =
        rightLaneSegmentIter->laneInterval.laneId;
    rightLaneSegmentIter->leftNeighbor =
        leftLaneSegmentIter->laneInterval.laneId;
...
void updateLaneSegmentSuccessors(RoadSegment &roadSegment,
                                 RoadSegment const &successorSegment) {
  for (auto &laneSegment : roadSegment.drivableLaneSegments) {
...
void updateLaneSegmentPredecessors(RoadSegment &roadSegment,
                                   RoadSegment const &predecessorSegment) {
  for (auto &laneSegment : roadSegment.drivableLaneSegments) {
    laneSegment.predecessors.erase(
        std::remove_if(
            std::begin(laneSegment.predecessors),
            std::end(laneSegment.predecessors),
            [&predecessorSegment](lane::LaneId const &predecessorId) {
              auto findResult = std::find_if(
                  std::begin(predecessorSegment.drivableLaneSegments),
...
physics::Distance calcLength(FindWaypointResult const &findWaypointResult) {
  ...
  if (findWaypointResult.isValid()) {
    ...
    // all segments before
    for (;
         roadSegmentIter != findWaypointResult.queryRoute.roadSegments.end() &&
    ...
    if (roadSegmentIter == findWaypointResult.roadSegmentIterator) {
      ...
      if (laneSegmentIter == findWaypointResult.laneSegmentIterator) {
        ...
        throw(std::runtime_error(
            "::ad::map::route::calcLength(FindWaypointResult) intervalIter of "
            "the result is not valid"));
      }
    } else {
      throw(
          std::runtime_error("::ad::map::route::calcLength(FindWaypointResult) "
                             "roadSegmentIterator of the result is not valid"));
    }
  }
  return result;
}
...
FindWaypointResult::FindWaypointResult(FullRoute const &route)
    : queryRoute(route), roadSegmentIterator(queryRoute.roadSegments.end()) {}

FindWaypointResult &FindWaypointResult::operator=(
    FindWaypointResult const &other) {
  if ((&this->queryRoute) != (&other.queryRoute)) {
    throw std::invalid_argument(
        "FindWaypointResult::operator= incompatible input parameter");
  }
  if (this != &other) {
...
FindWaypointResult FindWaypointResult::getLeftLane() const {
  ...
  if (!isValid() ||
      (!lane::isValid(laneSegmentIterator->leftNeighbor, false))) {
    return result;
  }

  result = *this;
  result.laneSegmentIterator++;
  if (!result.isValid() || (result.laneSegmentIterator->laneInterval.laneId !=
                            laneSegmentIterator->leftNeighbor)) {
    throw std::runtime_error(
        "ad::map::route::FindWaypointResult::getLeftLane()>> Route "
        "inconsistent: "
        "left lane not found");
  }
...
FindWaypointResult FindWaypointResult::getRightLane() const {
  FindWaypointResult result(queryRoute);
  if (!isValid() ||
      (!lane::isValid(laneSegmentIterator->rightNeighbor, false))) {
    return result;
  }
  ...
  result.laneSegmentIterator--;
  if (!result.isValid() || (result.laneSegmentIterator->laneInterval.laneId !=
                            laneSegmentIterator->rightNeighbor)) {
    throw std::runtime_error(
        "ad::map::route::FindWaypointResult::getRightLane()>> Route "
        "inconsistent: "
...
// supporting function used commonly by getSuccessorLanes() and
// getPredecessorLanes()
std::vector<FindWaypointResult> getLanesOfCurrentRoadSegment(
    FindWaypointResult &result, lane::LaneIdList const &expectedLanesVector) {
  ...
  for (result.laneSegmentIterator =
           result.roadSegmentIterator->drivableLaneSegments.begin();
       result.laneSegmentIterator !=
       result.roadSegmentIterator->drivableLaneSegments.end();
       result.laneSegmentIterator++) {
    auto const findResult =
        expectedLanes.find(result.laneSegmentIterator->laneInterval.laneId);
    if (findResult != expectedLanes.end()) {
      // found a predecessor
      if (!result.isValid()) {
        throw std::runtime_error(
            "ad::map::route::FindWaypointResult::getLanesOfCurrentRoadSegment()"
            ">> unexpected error");
      }
      result.queryPosition.laneId =
          result.laneSegmentIterator->laneInterval.laneId;
...
std::vector<FindWaypointResult> FindWaypointResult::getSuccessorLanes() const {
  ...
  if (result.roadSegmentIterator == result.queryRoute.roadSegments.end()) {
    throw std::runtime_error(
        "ad::map::route::FindWaypointResult::getSuccessorLanes()>> Route "
        "inconsistent: "
...
std::vector<FindWaypointResult> FindWaypointResult::getPredecessorLanes()
    const {
  ...
  if (result.roadSegmentIterator == result.queryRoute.roadSegments.begin()) {
    throw std::runtime_error(
        "ad::map::route::FindWaypointResult::getPredecessorLanes()>> Route "
        "inconsistent: "
        "previous road segment not found");
  }
  result.roadSegmentIterator--;
  return getLanesOfCurrentRoadSegment(result,
                                      laneSegmentIterator->predecessors);
}

FindWaypointResult findWaypointImpl(point::ParaPoint const &position,
                                    route::FullRoute const &route,
                                    bool considerRouteStart) {
  for (route::RoadSegmentList::const_iterator roadSegmentIter =
           route.roadSegments.begin();
       roadSegmentIter != route.roadSegments.end(); ++roadSegmentIter) {
    for (route::LaneSegmentList::const_iterator laneSegmentIter =
             roadSegmentIter->drivableLaneSegments.begin();
      ...
      if (position.laneId == laneSegmentIter->laneInterval.laneId) {
        if (!considerRouteStart) {
          ...
          return FindWaypointResult(route, startPoint, roadSegmentIter,
                                    laneSegmentIter);
        } else {
          // due to numeric inaccuracies we need to check not just the internal
          // itself
          // but also some surroundings
          // @todo: Ideally this has to be covered by the laneInterval
          // operations
          // For now, just extend the Interval by 0.5 meter on each end
          LaneInterval laneInterval = laneSegmentIter->laneInterval;
          laneInterval =
              extendIntervalFromStart(laneInterval, physics::Distance(0.5));
          laneInterval =
              extendIntervalFromEnd(laneInterval, physics::Distance(0.5));

          if (isWithinInterval(laneInterval, position)) {
            return FindWaypointResult(route, position, roadSegmentIter,
                                      laneSegmentIter);
          }
        }
      }
...
FindWaypointResult findWaypoint(point::ParaPoint const &position,
                                route::FullRoute const &route) {
  return findWaypointImpl(position, route, true);
}

FindWaypointResult findWaypoint(lane::LaneId const &laneId,
                                route::FullRoute const &route) {
...
FindWaypointResult findNearestWaypoint(point::ParaPointList const &positions,
                                       route::FullRoute const &route) {
  ...
  for (auto const &position : positions) {
    ...
    if (findResult.isValid()) {
      if (  // no other result yet
          ...
          || (findResult.roadSegmentIterator <
              resultWaypoint.roadSegmentIterator)) {
        resultWaypoint = findResult;
      } else if (findResult.roadSegmentIterator ==
                 resultWaypoint.roadSegmentIterator) {
        // new result is on the same segment, so let the parametricOffset value
        // of the query decide
        if (isRouteDirectionPositive(
                resultWaypoint.laneSegmentIterator->laneInterval)) {
          if (findResult.queryPosition.parametricOffset <
              resultWaypoint.queryPosition.parametricOffset) {
            resultWaypoint = findResult;
          }
        } else {
          if (findResult.queryPosition.parametricOffset >
...
FindWaypointResult findNearestWaypoint(
    match::MapMatchedPositionConfidenceList const &mapMatchedPositions,
    route::FullRoute const &route) {
  return findNearestWaypoint(match::getParaPoints(mapMatchedPositions), route);
}

FindWaypointResult findCenterWaypoint(match::Object const &object,
                                      route::FullRoute const &route) {
  FindWaypointResult resultWaypoint(route);
  ...
  if (object.mapMatchedBoundingBox.referencePointPositions.size() >
    ...
    resultWaypoint = findNearestWaypoint(
        object.mapMatchedBoundingBox
            .referencePointPositions[static_cast<std::size_t>(
                match::ObjectReferencePoints::Center)],
        route);
  }
  if (!resultWaypoint.isValid()) {
    // sort the occupied region center points by the length of the region
    // to start the search with the largest longitudinal region (which is most
    // likely the main lane)
    std::multimap<physics::Distance, point::ParaPoint,
                  std::greater<physics::Distance>>
        occupiedRegionCenters;
    for (auto const &occupiedRegion :
         object.mapMatchedBoundingBox.laneOccupiedRegions) {
      auto const occupiedRegionLength = lane::calcLength(occupiedRegion);
      auto const occupiedRegionCenter =
          match::getCenterParaPoint(occupiedRegion);
      occupiedRegionCenters.insert(
          {occupiedRegionLength, occupiedRegionCenter});
    }
  ...
  if (!resultWaypoint.isValid()) {
    // still nothing found, so we take the nearest of the rest of the reference
    // point positions
    point::ParaPointList mapMatchedParaPoints;
    for (auto i = 0u;
         i < object.mapMatchedBoundingBox.referencePointPositions.size(); ++i) {
      if (i != static_cast<std::size_t>(match::ObjectReferencePoints::Center)) {
        auto paraPoints = match::getParaPoints(
            object.mapMatchedBoundingBox.referencePointPositions[i]);
        mapMatchedParaPoints.insert(mapMatchedParaPoints.end(),
                                    paraPoints.begin(), paraPoints.end());
      }
    }
    resultWaypoint = findNearestWaypoint(mapMatchedParaPoints, route);
  }
  if (!resultWaypoint.isValid()) {
    // still nothing found, so search if any point of the route is within our
    // occupied regions
    for (auto const &occupiedRegion :
         object.mapMatchedBoundingBox.laneOccupiedRegions) {
      auto laneWaypoint = findWaypoint(occupiedRegion.laneId, route);
      if (laneWaypoint.isValid()) {
        auto const routeParmetricLaneRange = route::toParametricRange(
            laneWaypoint.laneSegmentIterator->laneInterval);
        auto const intersectingRange = physics::getIntersectionRange(
            routeParmetricLaneRange, occupiedRegion.longitudinalRange);
        if (physics::isRangeValid(intersectingRange)) {
          laneWaypoint.queryPosition.parametricOffset =
              intersectingRange.minimum;
...
FindWaypointResult objectOnRoute(
    match::MapMatchedObjectBoundingBox const &object,
    route::FullRoute const &route) {
...
FindWaypointResult intersectionOnRoute(
    intersection::Intersection const &intersection,
    route::FullRoute const &route) {
   ...
   */
  auto findResult =
      std::find_if(std::begin(route.roadSegments.front().drivableLaneSegments),
                   std::end(route.roadSegments.front().drivableLaneSegments),
                   [&intersection](LaneSegment const &laneSegment) {
                     return intersection.internalLanes().find(
                                laneSegment.laneInterval.laneId) !=
  ...
  if (findResult != std::end(route.roadSegments.front().drivableLaneSegments)) {
    ...
    result.queryPosition.laneId =
        result.laneSegmentIterator->laneInterval.laneId;
    result.queryPosition.parametricOffset =
        result.laneSegmentIterator->laneInterval.start;
  } else {
    result =
        findNearestWaypoint(intersection.incomingParaPointsOnRoute(), route);
    if (!result.isValid()) {
...
bool isConnectedRoutePartOfAnIntersection(
    route::ConnectingRoute const &connectingRoute) {
  return intersection::Intersection::isRoutePartOfAnIntersection(
             connectingRoute.routeA) ||
         intersection::Intersection::isRoutePartOfAnIntersection(
             connectingRoute.routeB);
}

void alignRouteStartingPoints(point::ParaPoint const &startAlignmentParaPoint,
  ...
  // i.e. the starting points are located on an imaginary line on the curve
  // radius
  ...
  auto const startAlignmentPoint = lane::getProjectedParametricPoint(
      startAlignmentLane, startAlignmentParaPoint.parametricOffset,
  ...
  for (route::LaneSegmentList::iterator laneSegmentIter =
       ...
       ++laneSegmentIter) {
    if (laneSegmentIter->laneInterval.laneId !=
        startAlignmentParaPoint.laneId) {
      auto lane = lane::getLane(laneSegmentIter->laneInterval.laneId);
      auto rightT =
          point::findNearestPointOnEdge(lane.edgeRight, startAlignmentPoint);
      auto leftT =
          point::findNearestPointOnEdge(lane.edgeLeft, startAlignmentPoint);
      laneSegmentIter->laneInterval.start = 0.5 * (rightT + leftT);
    }
  }
}

void alignRouteEndingPoints(point::ParaPoint const &alignmentParaPoint,
                            route::FullRoute &route) {
  if (route.roadSegments.empty()) {
    return;
  }
  ...
  auto const alignmentPoint = lane::getProjectedParametricPoint(
      alignmentLane, alignmentParaPoint.parametricOffset,
      physics::ParametricValue(0.5));

  for (route::LaneSegmentList::iterator laneSegmentIter =
           route.roadSegments.back().drivableLaneSegments.begin();
    ...
    if (laneSegmentIter->laneInterval.laneId != alignmentParaPoint.laneId) {
      auto lane = lane::getLane(laneSegmentIter->laneInterval.laneId);
      auto rightT =
          point::findNearestPointOnEdge(lane.edgeRight, alignmentPoint);
...
ShortenRouteResult shortenRoute(point::ParaPointList const &currentPositions,
  ...
  if (findWaypointResult.isValid()) {
    ...
    if ((shortenRouteMode ==
      ...
      for (auto roadSegmentIter = route.roadSegments.begin();
           roadSegmentIter != route.roadSegments.end(); roadSegmentIter++) {
        ...
        auto intersectionStarts =
            intersection::Intersection::isRoadSegmentEnteringIntersection(
                routeIterator, routePreviousSegmentIter);
        if (intersectionStarts) {
          ...
          for (route::LaneSegmentList::iterator laneSegmentIter =
                   route.roadSegments.front().drivableLaneSegments.begin();
               laneSegmentIter !=
               route.roadSegments.front().drivableLaneSegments.end();
               ++laneSegmentIter) {
            laneSegmentIter->laneInterval.start =
                laneSegmentIter->laneInterval.end;
          }
    ...
    if (result != ShortenRouteResult::SucceededIntersectionNotCut) {
      ...
      for (route::LaneSegmentList::iterator laneSegmentIter =
           ...
           ++laneSegmentIter) {
        if (isWithinInterval(
                laneSegmentIter->laneInterval,
                findWaypointResult.queryPosition.parametricOffset)) {
          laneSegmentIter->laneInterval.start =
              findWaypointResult.queryPosition.parametricOffset;
      ...
      if (route::isDegenerated(route.roadSegments.front()
                                   .drivableLaneSegments.front()
                                   .laneInterval)) {
  ...
  // check if we are right after the route ---> clear in this case, but still
  // return true
  for (auto const &laneSegment :
       route.roadSegments.back().drivableLaneSegments) {
    for (auto const &currentPosition : currentPositions) {
      if (!isDegenerated(laneSegment.laneInterval) &&
          isAfterInterval(laneSegment.laneInterval, currentPosition)) {
...
ShortenRouteResult shortenRoute(
    match::MapMatchedPositionConfidenceList const &mapMatchedPositions,
    route::FullRoute &route, ShortenRouteMode const shortenRouteMode) {
  return shortenRoute(match::getParaPoints(mapMatchedPositions), route,
                      shortenRouteMode);
}
...
void shortenRouteToDistance(route::FullRoute &route,
  ...
  while ((roadSegmentIterator != route.roadSegments.end()) &&
    ...
    if (segmentLength <= remainingLength) {
      ...
      if (intersection::Intersection::isLanePartOfAnIntersection(
              roadSegmentIterator->drivableLaneSegments.front()
                  .laneInterval.laneId)) {
        // don't cut in between of intersections, rather keep the whole
        // intersection within the route
        for (roadSegmentIterator++;
             (roadSegmentIterator != route.roadSegments.end());
             roadSegmentIterator++) {
          if (!intersection::Intersection::isLanePartOfAnIntersection(
                  roadSegmentIterator->drivableLaneSegments.front()
                      .laneInterval.laneId)) {
...
void removeLastRoadSegmentIfDegenerated(route::FullRoute &route) {
  // first drop degenerated road segments at the end of route to ensure we get
  // proper direction
  if (!route.roadSegments.empty() &&
      (route.roadSegments.back().drivableLaneSegments.empty() ||
       isDegenerated(route.roadSegments.back()
                         .drivableLaneSegments.front()
                         .laneInterval))) {
...
bool extendRouteToDistance(route::FullRoute &route,
  ...
  if (!additionalRoutes.empty()) {
    access::getLogger()->error(
        "ad::map::route::extendRouteToDistance: additional Routes parameter is "
        "not empty, "
  ...
  auto const lastRightLaneSegment =
      route.roadSegments.back().drivableLaneSegments.front();
  auto routingStartPoint = route::planning::createRoutingPoint(
      ...
      (isRouteDirectionPositive(lastRightLaneSegment.laneInterval)
  ...
  auto routeExtensions = route::planning::predictRoutesOnDistance(
      routingStartPoint, distance, route.routeCreationMode);
  ...
  for (; it != routeExtensions.end(); ++it) {
    ...
    for (auto const &roadSegment : routeExtension.roadSegments) {
      route::appendRoadSegmentToRoute(
          roadSegment.drivableLaneSegments.front().laneInterval,
...
void shortenSegmentFromBegin(RoadSegment &roadSegment,
                             physics::Distance const &distance) {
  if (roadSegment.drivableLaneSegments.empty()) {
    throw std::runtime_error(
        "ad::map::route::shortenSegmentFromBegin>> Route inconsistent: "
        "route contains no drivableLaneSegments");
  }

  auto shortenedInterval = shortenIntervalFromBegin(
      roadSegment.drivableLaneSegments.front().laneInterval, distance);
  for (auto &laneSegment : roadSegment.drivableLaneSegments) {
    laneSegment.laneInterval.start = shortenedInterval.start;
  }
}

void shortenSegmentFromEnd(RoadSegment &roadSegment,
                           physics::Distance const &distance) {
  if (roadSegment.drivableLaneSegments.empty()) {
    throw std::runtime_error(
        "ad::map::route::shortenSegmentFromBegin>> Route inconsistent: "
  ...
  auto shortenedInterval = shortenIntervalFromEnd(
      roadSegment.drivableLaneSegments.front().laneInterval, distance);
  for (auto &laneSegment : roadSegment.drivableLaneSegments) {
...
bool calculateRouteParaPointAtDistance(route::FullRoute const &route,
  ...
  while (!routeParaPointFound && routeIterator.isValid()) {
    ...
    const physics::Distance segmentLength =
        calcLength((*routeIterator.roadSegmentIterator));

    physics::Distance distanceToSegmentBorder = segmentLength;
    if (routeIterator.roadSegmentIterator->segmentCountFromDestination ==
        origin.segmentCountFromDestination) {
      originOffset = origin.parametricOffset;
      if (distance < physics::Distance(0)) {
        distanceToSegmentBorder = segmentLength * (origin.parametricOffset);
      } else {
        distanceToSegmentBorder =
            segmentLength *
            (physics::ParametricValue(1.0) - origin.parametricOffset);
      }
    } else {
      if (distance < physics::Distance(0)) {
    ...
    if ((accumulatedDistance + distanceToSegmentBorder) >=
        std::fabs(distance)) {
      physics::Distance const remainingLength =
          std::fabs(distance) - accumulatedDistance;

      resultingPoint.routePlanningCounter = route.routePlanningCounter;
      resultingPoint.segmentCountFromDestination =
          routeIterator.roadSegmentIterator->segmentCountFromDestination;
    ...
    if (distance < physics::Distance(0)) {
      if (routeIterator.roadSegmentIterator !=
          std::begin(routeIterator.route.roadSegments)) {
        routeIterator.roadSegmentIterator--;
      } else {
        routeIterator.roadSegmentIterator =
            std::end(routeIterator.route.roadSegments);
      }
    } else {
      routeIterator.roadSegmentIterator++;
    }
...
bool getRouteParaPointFromParaPoint(point::ParaPoint const &paraPoint,
  ...
  routeParaPoint.segmentCountFromDestination =
      waypointResult.roadSegmentIterator->segmentCountFromDestination;
  routeParaPoint.parametricOffset =
      std::fabs(paraPoint.parametricOffset -
                waypointResult.laneSegmentIterator->laneInterval.start);

  return true;
}

route::FullRoute getRouteExpandedTo(route::FullRoute const &route,
  ...
  if (!route.roadSegments.empty()) {
    if (!route.roadSegments.front().drivableLaneSegments.empty()) {
      point::ParaPoint const alignmentParaPoint = getIntervalStart(
          route.roadSegments.front().drivableLaneSegments.front().laneInterval);
      alignRouteStartingPoints(alignmentParaPoint, expandedRoute);
    }
    if (!route.roadSegments.back().drivableLaneSegments.empty()) {
      point::ParaPoint const alignmentParaPoint = getIntervalEnd(
          route.roadSegments.back().drivableLaneSegments.front().laneInterval);
...
FullRoute getRouteSection(
    FindWaypointResult const &currentLane,
    physics::Distance const &distanceFront,
    physics::Distance const &distanceEnd, FullRoute const &route,
  ...
  currentStartInterval.laneId =
      currentLane.laneSegmentIterator->laneInterval.laneId;
  currentStartInterval.start =
      currentLane.laneSegmentIterator->laneInterval.start;
  ...
  if (accumulatedDistanceFront >= distanceFront) {
    currentLaneSegment.laneInterval =
        shortenIntervalFromBegin(currentLaneSegment.laneInterval,
    ...
    while (  // required distance not yet reached
      ...
      newRoadSegment.boundingSphere =
          currentPredecessors[0].roadSegmentIterator->boundingSphere;
      newRoadSegment.segmentCountFromDestination =
          currentPredecessors[0]
              .roadSegmentIterator->segmentCountFromDestination;

      std::vector<FindWaypointResult> nextPredecessors;
      for (auto const &predecessor : currentPredecessors) {
        newRoadSegment.drivableLaneSegments.push_back(
            *predecessor.laneSegmentIterator);
        auto const furtherPredecessors = predecessor.getPredecessorLanes();
        nextPredecessors.insert(std::end(nextPredecessors),
                                std::begin(furtherPredecessors),
                                std::end(furtherPredecessors));
      }

      auto currentSegmentLength = calcLength(newRoadSegment);
      if (accumulatedDistanceFront + currentSegmentLength > distanceFront) {
        route::shortenSegmentFromBegin(
            newRoadSegment,
            accumulatedDistanceFront + currentSegmentLength - distanceFront);
        accumulatedDistanceFront = distanceFront;
      } else {
      ...
      access::getLogger()->trace(
          "ad::map::route::getRouteSection: prepending road segment {}: {} "
          "({})",
          newRoadSegment, accumulatedDistanceFront, distanceFront);
      resultRoute.roadSegments.insert(std::begin(resultRoute.roadSegments),
                                      newRoadSegment);
  ...
  if (accumulatedDistanceEnd >= distanceEnd) {
    currentLaneSegment.laneInterval = shortenIntervalFromEnd(
        currentLaneSegment.laneInterval, accumulatedDistanceEnd - distanceEnd);
  ...
  currentRoadSegment.boundingSphere =
      currentLane.roadSegmentIterator->boundingSphere;
  currentRoadSegment.segmentCountFromDestination =
      currentLane.roadSegmentIterator->segmentCountFromDestination;
  currentRoadSegment.drivableLaneSegments.push_back(currentLaneSegment);
  access::getLogger()->trace(
      "ad::map::route::getRouteSection: appending current road segment {}: "
      "{}({}) -> {}({})",
      currentRoadSegment, accumulatedDistanceFront, distanceFront,
      accumulatedDistanceEnd, distanceEnd);
  resultRoute.roadSegments.insert(std::end(resultRoute.roadSegments),
                                  currentRoadSegment);
  ...
  while (  // required distance not yet reached
    ...
    route::RoadSegment newRoadSegment;
    newRoadSegment.boundingSphere =
        currentSuccessors[0].roadSegmentIterator->boundingSphere;
    newRoadSegment.segmentCountFromDestination =
        currentSuccessors[0].roadSegmentIterator->segmentCountFromDestination;

    std::vector<FindWaypointResult> nextSuccessors;
    for (auto const &successor : currentSuccessors) {
      newRoadSegment.drivableLaneSegments.push_back(
          *successor.laneSegmentIterator);
      auto const furtherSuccessors = successor.getSuccessorLanes();
      nextSuccessors.insert(std::end(nextSuccessors),
                            std::begin(furtherSuccessors),
                            std::end(furtherSuccessors));
    }

    auto currentSegmentLength = calcLength(newRoadSegment);
    if (accumulatedDistanceEnd + currentSegmentLength > distanceEnd) {
      route::shortenSegmentFromEnd(
          newRoadSegment,
          accumulatedDistanceEnd + currentSegmentLength - distanceEnd);
      accumulatedDistanceEnd = distanceEnd;
    } else {
    ...
    access::getLogger()->trace(
        "ad::map::route::getRouteSection: appending road segment {}: {} ({})",
        newRoadSegment, accumulatedDistanceEnd, distanceEnd);
    resultRoute.roadSegments.insert(std::end(resultRoute.roadSegments),
                                    newRoadSegment);
  ...
  access::getLogger()->trace(
      "ad::map::route::getRouteSection: result before update lane connections "
      "{}",
      resultRoute);
  ...
  access::getLogger()->debug(
      "ad::map::route::getRouteSection({} < {}:{} > {} ) {}", distanceFront,
      currentLane.laneSegmentIterator->laneInterval.laneId,
      currentLane.queryPosition.parametricOffset, distanceEnd, resultRoute);

  return resultRoute;
}

FullRoute getRouteSection(
    point::ParaPoint const &centerPoint, physics::Distance const &distanceFront,
    physics::Distance const &distanceEnd, FullRoute const &route,
    RouteSectionCreationMode const routeSectionCreationMode) {
  return getRouteSection(findWaypoint(centerPoint, route), distanceFront,
                         distanceEnd, route, routeSectionCreationMode);
}

FullRoute getRouteSection(
    match::Object const &object, FullRoute const &route,
    RouteSectionCreationMode const routeSectionCreationMode) {
  auto const objectCenterPointOnRoute = findCenterWaypoint(object, route);
  auto const routeAroundVehicle = getRouteSection(
      objectCenterPointOnRoute, object.enuPosition.dimension.length,
      object.enuPosition.dimension.length, route, routeSectionCreationMode);
...
physics::Distance FindLaneChangeResult::calcZoneLength() const {
  physics::Distance distance(0.);
  if (isValid()) {
    for (auto roadSegmentIter = laneChangeStartRouteIterator;
         roadSegmentIter < laneChangeEndRouteIterator; roadSegmentIter++) {
      distance += calcLength(*roadSegmentIter);
    }
...
FindLaneChangeResult findFirstLaneChange(
    match::MapMatchedPosition const &currentPosition,
    route::FullRoute const &route) {
  // this is what we want to return. Two indices defining the start and the end
  // of the lane change
  FindLaneChangeResult result(route);

  // first, find the iterator where the first lane change needs to be finished
  // at the latest. If the target lane is
  // always blocked the user of this function may want to set a stop line at the
  // routeIteratorLaneChangeEnd. To do
  // this,
  // - we travel along the successors starting from the current position as long
  // as we can
  // - if the next route section is not reachable by successors and is not the
  // end of the route, we found the lane
  //   change end and traverse along the route in the other direction to find
  //   the lane change start
  // - if we can travel along the whole route without changing lanes, we return
  // an invalid result (since there is no
  //   lane change on this route segment)
  FindWaypointResult const findWaypointResult =
      findWaypoint(currentPosition.lanePoint.paraPoint, route);

  if (!findWaypointResult.isValid()) {
    if (!route.roadSegments.empty()) {
      access::getLogger()->error(
          "ad::map::route::findFirstLaneChange: Current position is not part "
          "of the route {} {}",
  ...
  for (auto successorLanes = findWaypointResult.getSuccessorLanes();
       successorLanes.size() == 1u;
       successorLanes = successorLanes[0].getSuccessorLanes()) {
    laneChangeEndResult = successorLanes[0];
  }

  // Find the transition lane, to which we need to change to be able to get to
  ...
  for (auto neighbor = laneChangeEndResult.getRightLane(); neighbor.isValid();
       neighbor = neighbor.getRightLane()) {
  ...
  for (auto neighbor = laneChangeEndResult.getLeftLane(); neighbor.isValid();
       neighbor = neighbor.getLeftLane()) {
  ...
  if (leftNeighbor.isValid() && rightNeighbor.isValid()) {
    ...
    access::getLogger()->trace("ad::map::route::no lane change required {} {}",
                               currentPosition, route);
    return result;
  }
  ...
  result.laneChangeEndLaneSegmentIterator =
      transitionEndLane.laneSegmentIterator;

  // now, traverse back to the beginning (currentPosiion) from the transition
  // lane
  // if there is no predecessor, the lane change has to begin latest there
  // and if there is more than one predecessor, the lane change has to begin
  // there, too
  bool currentPositionReached = false;

  while (!currentPositionReached) {
    auto laneChangeBeginResult = transitionEndLane;
    for (auto predecessorLanes = transitionEndLane.getPredecessorLanes();
         predecessorLanes.size() == 1u;
    ...
    result.laneChangeStartRouteIterator =
        transitionStartLane.roadSegmentIterator;
    result.laneChangeStartLaneSegmentIterator =
        transitionStartLane.laneSegmentIterator;

    access::getLogger()->debug(
        "ad::map::route::findFirstLaneChange: found valid lane change {} "
        "starting at {}  "
        "laneId[] {} ending at {}  laneId[] {} input position {} and route {}",
        result.laneChangeDirection, *result.laneChangeStartRouteIterator,
    ...
    if (result.laneChangeStartLaneSegmentIterator->laneInterval.laneId ==
      ...
      transitionEndLane.laneSegmentIterator =
          result.laneChangeEndLaneSegmentIterator;
      transitionEndLane.roadSegmentIterator =
          result.laneChangeStartRouteIterator;
    }
...
void addRoutePredecessorRelationToNewSegment(lane::Lane const &lane,
  ...
  for (auto predecessorLaneContact : lane::getContactLanes(
           lane, routeDirectionPositive ? lane::ContactLocation::PREDECESSOR
                                        : lane::ContactLocation::SUCCESSOR)) {
    auto predecessorLaneId = predecessorLaneContact.toLane;
    auto predecessorLaneSegmentIter = std::find_if(
        std::begin(route.roadSegments.back().drivableLaneSegments),
    ...
    if (predecessorLaneSegmentIter !=
        std::end(route.roadSegments.back().drivableLaneSegments)) {
      addLaneIdUnique(predecessorLaneSegmentIter->successors,
                      laneSegment.laneInterval.laneId);
...
bool isRightNeighbor(lane::ContactLocation const contactLocation,
                     bool const isRouteDirectionPositive) {
  bool rightNeighbor;
...
void updateRouteLaneOffsetRange(RouteLaneOffset const &routeLaneOffset,
                                FullRoute &route) {
...
void updateRouteLaneOffset(bool const rightNeighbor,
                           RouteLaneOffset &routeLaneOffset, FullRoute &route) {
  if (rightNeighbor) {
    routeLaneOffset--;
  } else {
    routeLaneOffset++;
...
void appendLaneSegmentToRoute(
    route::LaneInterval const &laneInterval, route::FullRoute &route,
  ...
  addRoutePredecessorRelationToNewSegment(lane, laneSegment, route,
                                          routeDirectionPositive);
...
void appendRoadSegmentToRoute(route::LaneInterval const &laneInterval,
  ...
  for (auto contactLocation :
       {lane::ContactLocation::LEFT, lane::ContactLocation::RIGHT}) {
    route::RouteLaneOffset currentRouteLaneOffset = routeLaneOffset;
    bool const rightNeighbor =
        isRightNeighbor(contactLocation, routeDirectionPositive);
    lane::ContactLaneList contactLanes =
        lane::getContactLanes(lane, contactLocation);

    lane::LaneIdSet visitedLaneIds;
    visitedLaneIds.insert(laneInterval.laneId);

    // we expect that per map model only one contact lane is possible in one
    // direction
    while (contactLanes.size() == 1u) {
      // in some broken map cases we could end in an infinite loop here, due to
      // broken neighborhood relations
      // to avoid this issue, add an early return upon detecting the start lane
      // again
      ...
      if (!laneNotVisited.second) {
        access::getLogger()->warn(
            "ad::map::route::appendRoadSegmentToRoute: Broken neighborhood "
            "relations detected "
      ...
      if (((route.routeCreationMode ==
            RouteCreationMode::SameDrivingDirection) &&
           (direction == otherLane.direction)) ||
          ((route.routeCreationMode == RouteCreationMode::AllRoutableLanes) &&
           (lane::isRouteable(otherLane))) ||
        ...
        roadSegment.boundingSphere =
            roadSegment.boundingSphere + otherLane.boundingSphere;

        addRoutePredecessorRelationToNewSegment(otherLane, newLaneSegment,
                                                route, routeDirectionPositive);

        // sorting: right lanes are added at front, left lanes at back
        if (rightNeighbor) {
          roadSegment.drivableLaneSegments.front().rightNeighbor =
              newLaneSegment.laneInterval.laneId;
          newLaneSegment.leftNeighbor =
              roadSegment.drivableLaneSegments.front().laneInterval.laneId;
          roadSegment.drivableLaneSegments.insert(
              roadSegment.drivableLaneSegments.begin(), newLaneSegment);
        } else {
          roadSegment.drivableLaneSegments.back().leftNeighbor =
              newLaneSegment.laneInterval.laneId;
          newLaneSegment.rightNeighbor =
              roadSegment.drivableLaneSegments.back().laneInterval.laneId;
          roadSegment.drivableLaneSegments.insert(
              roadSegment.drivableLaneSegments.end(), newLaneSegment);
        }
    ...
    if (!contactLanes.empty()) {
      throw std::runtime_error(
          "AdRoadNetworkAdm::fillRoadSegment algorithm is not able to handle "
          "multiple left/right contact lanes");
    }
  }
  route.roadSegments.push_back(roadSegment);
}

physics::Distance addOpposingLaneSegmentToRoadSegment(
    point::ParaPoint const &startpoint, physics::Distance const &distance,
    route::RoadSegment &roadSegment, route::FullRoute &route) {
  if (roadSegment.drivableLaneSegments.empty()) {
  ...
  // for right-hand traffic this means, the lane segment is left of the
  // left-most lane segment
  // for left-hand traffic this is the segment right of the right-most lane
  // segment
  ...
  auto const neighborhood = getDirectNeighborhoodRelation(
      laneInterval.laneId, neighborIterator->laneInterval.laneId);
  ...
  if (!access::isLeftHandedTraffic()) {
    ...
    roadSegment.drivableLaneSegments.insert(
        roadSegment.drivableLaneSegments.begin(), laneSegment);
  }
...
bool addOpposingLaneToRoute(point::ParaPoint const &pointOnOppositeLane,
  ...
  if (startWayPoint.isValid()) {
    if (std::find_if(route.roadSegments[0].drivableLaneSegments.begin(),
                     ...
                     [&startPoint](const route::LaneSegment &l) {
                       return l.laneInterval.laneId == startPoint.laneId;
                     }) != route.roadSegments[0].drivableLaneSegments.end()) {
  ...
  while (coveredDistance < distanceOnWrongLane) {
    auto segmentDistance = route::addOpposingLaneSegmentToRoadSegment(
        startPoint, distanceOnWrongLane, route.roadSegments[segmentCounter],
        route);
    ...
    if (coveredDistance < distanceOnWrongLane) {
      ...
      if (route::isRouteDirectionNegative(route.roadSegments[segmentCounter]
                                              .drivableLaneSegments[0]
                                              .laneInterval)) {
        // get predecessor, if there are multiple its an intersection so stop
        successors = lane::getContactLanes(lane::getLane(startPoint.laneId),
      ...
      if ((successors.size() == 0) || (successors.size() > 1) ||
          (segmentCounter == route.roadSegments.size())) {
        break;
      }

      startPoint.laneId = successors[0].toLane;
      startPoint.parametricOffset = route.roadSegments[segmentCounter]
                                        .drivableLaneSegments[0]
                                        .laneInterval.start;
...
route::FullRoute getRouteExpandedToOppositeLanes(
    route::FullRoute const &route) {
  return getRouteExpandedTo(route, RouteCreationMode::AllRoutableLanes);
}

route::FullRoute getRouteExpandedToAllNeighborLanes(
    route::FullRoute const &route) {
  return getRouteExpandedTo(route, RouteCreationMode::AllNeighborLanes);
}

bool calculateBypassingRoute(route::FullRoute const &route,
  ...
  for (const auto &segment : route.roadSegments) {
    ...
    // depending on the lane orientation and the driving direction (left-hand
    // vs. right-hand)
  ...
  for (size_t i = 0; i < bypassingRoute.roadSegments.size(); ++i) {
    bypassingRoute.roadSegments[i].segmentCountFromDestination =
        bypassingRoute.roadSegments.size() - i;
  }
...
inline LaneInterval cutLaneIntervalAtEndByRoadSegmentParametricOffset(
    LaneInterval const &interval,
  ...
  if (isRouteDirectionNegative(interval)) {
    laneParametricOffset =
        physics::ParametricValue(1.) - segmentParametricOffset;
  }
  return cutIntervalAtEnd(interval, laneParametricOffset);
}

template <typename BorderType>
void getBorderOfRoadSegment(RoadSegment const &roadSegment, BorderType &border,
                            physics::ParametricValue const parametricOffset) {
  if (!roadSegment.drivableLaneSegments.empty()) {
    getRightProjectedEdge(
        cutLaneIntervalAtEndByRoadSegmentParametricOffset(
            roadSegment.drivableLaneSegments.front().laneInterval,
            parametricOffset),
        border.right);
    getLeftProjectedEdge(
        cutLaneIntervalAtEndByRoadSegmentParametricOffset(
            roadSegment.drivableLaneSegments.back().laneInterval,
            parametricOffset),
        border.left);
  }
}

lane::ECEFBorder getECEFBorderOfRoadSegment(
...
lane::ENUBorder getENUBorderOfRoadSegment(
    RoadSegment const &roadSegment,
    physics::ParametricValue const parametricOffset) {
...
lane::GeoBorder getGeoBorderOfRoadSegment(
    RoadSegment const &roadSegment,
    physics::ParametricValue const parametricOffset) {
...
point::ENUHeading getENUHeadingOfRoute(match::Object const &object,
                                       FullRoute const &route) {
  FindWaypointResult objectCenterWaypoint = findCenterWaypoint(object, route);
  if (!objectCenterWaypoint.isValid()) {
    access::getLogger()->error(
        "ad::map::route::getENUHeadingOfRoute: object not found in route {} {}",
        object, route);
    throw std::runtime_error(
        "route::getENUHeadingOfRoute>> object not found in route");
  }
  // ensure the object center point is in
  auto const routeSectionAroundObject =
      getRouteSection(objectCenterWaypoint, object.enuPosition.dimension.length,
                      object.enuPosition.dimension.length, route,
...
bool isObjectHeadingInRouteDirection(match::Object const &object,
                                     FullRoute const &route) {
  auto const enuHeadingOfRoute = getENUHeadingOfRoute(object, route);
  // enforce normalization of angle difference
  const auto headingDifference = point::createENUHeading(static_cast<double>(
      std::fabs(enuHeadingOfRoute - object.enuPosition.heading)));
