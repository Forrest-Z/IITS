...
point::ParaPoint getIntervalStart(FullRoute const &route,
  ...
  for (auto const &roadSegment : route.roadSegments) {
    auto it = find_if(roadSegment.drivableLaneSegments.begin(),
                      ...
                      [&laneId](const LaneSegment &l) {
                        return l.laneInterval.laneId == laneId;
                      });
  ...
  throw std::invalid_argument(
      "ad::map::route::getIntervalStart: laneId not found in route");
}
...
physics::ParametricValue getSignedDistance(LaneInterval const &laneInterval,
  ...
  if ((first.laneId != second.laneId) ||
      (first.laneId != laneInterval.laneId)) {
    throw std::invalid_argument(
        "ad::map::route::getSignedDistance: lane id's not matching");
  }
...
physics::ParametricValue getProjectedParametricOffsetOnNeighborLane(
  ...
  if (!lane::isSameOrDirectNeighbor(currentInterval.laneId,
                                    neighborInterval.laneId)) {
    throw std::invalid_argument(
        "ad::map::route::getProjectedParametricOffsetOnNeighborLane: lanes are "
        "not neighbors");
  }
  ...
  auto leftNeighbors =
      lane::getContactLanes(currentLane, lane::ContactLocation::LEFT);
  auto rightNeighbors =
      lane::getContactLanes(currentLane, lane::ContactLocation::RIGHT);

  point::ECEFPoint leftECEFPoint;
  point::ECEFPoint rightECEFPoint;
  physics::ParametricValue offset;
  lane::projectParametricPointToEdges(currentLane, parametricOffset,
                                      leftECEFPoint, rightECEFPoint);

  if ((leftNeighbors.size() > 0) &&
      (leftNeighbors[0].toLane == neighborInterval.laneId)) {
    offset =
        ((point::findNearestPointOnEdge(neighborLane.edgeRight, leftECEFPoint) +
          point::findNearestPointOnEdge(neighborLane.edgeLeft, leftECEFPoint)) /
         2.);
  } else if ((rightNeighbors.size() > 0) &&
             (rightNeighbors[0].toLane == neighborInterval.laneId)) {
    offset = ((point::findNearestPointOnEdge(neighborLane.edgeRight,
                                             rightECEFPoint) +
               point::findNearestPointOnEdge(neighborLane.edgeLeft,
                                             rightECEFPoint)) /
              2.);
  } else {
    throw std::invalid_argument(
...
physics::Distance calcLength(LaneInterval const &laneInterval) {
  ...
  auto const resultDistance =
      currentLane.length * calcParametricLength(laneInterval);
...
enum class EdgeType { LEFT, RIGHT, LEFT_PROJECTED, RIGHT_PROJECTED };

template <typename LaneEdge>
void getEdge(LaneInterval const &laneInterval, EdgeType edgeType,
             LaneEdge &outputEdge) {
  auto currentLane = lane::getLane(laneInterval.laneId);

  if (isRouteDirectionPositive(laneInterval)) {
    if (edgeType == EdgeType::LEFT) {
      point::getParametricRange(currentLane.edgeLeft,
                                toParametricRange(laneInterval), outputEdge,
                                false);
      ...
      projectedInterval.start = point::findNearestPointOnEdge(
          ...
          lane::getProjectedParametricPoint(currentLane, laneInterval.start,
                                            physics::ParametricValue(0.)));
      projectedInterval.end = point::findNearestPointOnEdge(
          currentLane.edgeLeft,
          lane::getProjectedParametricPoint(currentLane, laneInterval.end,
                                            physics::ParametricValue(0.)));
      point::getParametricRange(currentLane.edgeLeft,
                                toParametricRange(projectedInterval),
                                outputEdge, false);
    } else if (edgeType == EdgeType::RIGHT) {
      point::getParametricRange(currentLane.edgeRight,
                                toParametricRange(laneInterval), outputEdge,
                                false);
...
LaneInterval shortenIntervalFromBegin(LaneInterval const &laneInterval,
  ...
  physics::ParametricValue delta(distance /
                                 lane::calcLength(laneInterval.laneId));
  if (isRouteDirectionPositive(laneInterval)) {
...
LaneInterval restrictIntervalFromBegin(LaneInterval const &laneInterval,
  ...
  if (isRouteDirectionNegative(laneInterval)) {
    result.end =
        std::max(physics::ParametricValue(0.), laneInterval.start - delta);
  } else {
    result.end =
        std::min(physics::ParametricValue(1.), laneInterval.start + delta);
  }
...
LaneInterval extendIntervalFromStart(LaneInterval const &laneInterval,
  ...
  LaneInterval resultInterval = laneInterval;
  physics::ParametricValue offset(distance /
                                  lane::calcLength(laneInterval.laneId));

  if (isRouteDirectionPositive(resultInterval)) {
    resultInterval.start =
        std::max(physics::ParametricValue(0.0), laneInterval.start - offset);
  } else {
    resultInterval.start =
        std::min(physics::ParametricValue(1.0), laneInterval.start + offset);
  }
  return resultInterval;
}
...
LaneInterval extendIntervalFromEnd(LaneInterval const &laneInterval,
  ...
  if (isRouteDirectionPositive(resultInterval)) {
    resultInterval.end =
        std::min(physics::ParametricValue(1.0), laneInterval.end + offset);
  } else {
    resultInterval.end =
        std::max(physics::ParametricValue(0.0), laneInterval.end - offset);
  }
  return resultInterval;
}
...
void getMetricRanges(LaneInterval const &laneInterval,
  ...
  auto lanePtr = lane::getLanePtr(laneInterval.laneId);
  if (std::fabs(laneInterval.end - laneInterval.start) ==
      physics::ParametricValue(1.0)) {
    ...
    if ((lanePtr->widthRange.maximum - lanePtr->widthRange.minimum) <=
        physics::Distance(.1)) {
      widthRange = lanePtr->widthRange;
    } else {
      // take the effort on with range calculation only if there is significant
      // difference within the lane
      auto const widthRangeResult = calculateWidthRange(
          enuBorders.left, leftLength, enuBorders.right, rightLength);
