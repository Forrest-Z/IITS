...
inline ostream &operator<<(
  ...
  out << value.routingPoint.first.point << "|"
      << static_cast<int16_t>(value.routingPoint.first.direction) << " ("
      << static_cast<double>(value.routingPoint.second.routeDistance)
      << "meters ,"
      << static_cast<double>(value.routingPoint.second.routeDuration)
      << " seconds) [";
...
RoutePrediction::RoutePrediction(const RoutingParaPoint &start,
    ...
    : RouteExpander(start, start, predictionDistance, predictionDuration,
                    Route::Type::SHORTEST) {}

RoutePrediction::RoutePrediction(const RoutingParaPoint &start,
                                 physics::Distance const &predictionDistance)
    : RouteExpander(start, start, predictionDistance,
                    physics::Duration::getMax(), Route::Type::SHORTEST) {}

RoutePrediction::RoutePrediction(const RoutingParaPoint &start,
                                 physics::Duration const &predictionDuration)
    : RouteExpander(start, start, physics::Distance::getMax(),
                    predictionDuration, Route::Type::SHORTEST) {}
...
void RoutePrediction::addNeighbor(lane::Lane::ConstPtr originLane,
  ...
  auto newChildElement =
      std::make_shared<RouteTreeElement>(currentTreeElement.get(), neighbor);
  auto insertProcessedResult =
      mProcessedTransitions[origin.first].insert(newChildElement);
  if (insertProcessedResult.second) {
    auto insertToProcessResult =
        currentTreeElement->children.insert(newChildElement);
    if (!insertToProcessResult.second) {
      // this child is already in the current element, nothing to be done
      return;
    }
  } else {
    // a similar child (a twin) was already already expanded, take that one as
    // our child
        ...
        if (element->children.empty()) {
          ...
          while (parent != nullptr) {
            rawRoute.paraPointList.insert(rawRoute.paraPointList.begin(),
                                          parent->routingPoint.first.point);
