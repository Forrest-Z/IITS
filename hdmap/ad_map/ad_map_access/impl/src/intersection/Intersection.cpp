 ...
 *********************************************************************/
bool Intersection::isLanePartOfIntersection(
...
Intersection::getDirectSuccessorsInLaneDirection(
    lane::LaneId const laneId, SuccessorMode const successorMode) const {
...
lane::LaneIdSet
Intersection::getDirectSuccessorsInLaneDirectionWithinIntersection(
    lane::LaneId const laneId, SuccessorMode const successorMode) const {
  return getDirectSuccessorsInLaneDirection(laneId, successorMode).first;
}

lane::LaneIdSet Intersection::getAllSuccessorsInLaneDirectionWithinIntersection(
    lane::LaneId const laneId, SuccessorMode const successorMode) const {
  lane::LaneIdSet result;
  auto directSuccessors = getDirectSuccessorsInLaneDirectionWithinIntersection(
      laneId, successorMode);
  for (auto directSuccessorId : directSuccessors) {
    auto directSuccessorRecursiveResult =
        getAllSuccessorsInLaneDirectionWithinIntersection(directSuccessorId,
                                                          successorMode);
    result.insert(directSuccessorRecursiveResult.begin(),
                  directSuccessorRecursiveResult.end());
  }
...
lane::LaneIdSet
Intersection::getLaneAndAllSuccessorsInLaneDirectionWithinIntersection(
    lane::LaneId const laneId, SuccessorMode const successorMode) const {
  auto result =
      getAllSuccessorsInLaneDirectionWithinIntersection(laneId, successorMode);
...
lane::LaneIdSet Intersection::getAllReachableOutgoingLanes(
    lane::LaneId const laneId, SuccessorMode const successorMode) const {
  lane::LaneIdSet result;
  auto directSuccessors =
      getDirectSuccessorsInLaneDirection(laneId, successorMode);
  // recursive until end of intersection
  for (auto directSuccessorId : directSuccessors.first) {
    auto directSuccessorRecursiveResult =
        getAllReachableOutgoingLanes(directSuccessorId, successorMode);
    result.insert(directSuccessorRecursiveResult.begin(),
                  directSuccessorRecursiveResult.end());
  }
...
std::pair<lane::LaneIdSet, lane::LaneIdSet>
Intersection::getAllReachableInternalAndOutgoingLanes(
    lane::LaneId const laneId, SuccessorMode const successorMode) const {
  std::pair<lane::LaneIdSet, lane::LaneIdSet> result;
  auto directSuccessors =
      getDirectSuccessorsInLaneDirection(laneId, successorMode);
  ...
  for (auto directSuccessorId : directSuccessors.first) {
    auto directSuccessorRecursiveResult =
        getAllReachableInternalAndOutgoingLanes(directSuccessorId,
                                                successorMode);
    result.first.insert(directSuccessorRecursiveResult.first.begin(),
                        directSuccessorRecursiveResult.first.end());
    result.second.insert(directSuccessorRecursiveResult.second.begin(),
                         directSuccessorRecursiveResult.second.end());
  }
  result.first.insert(directSuccessors.first.begin(),
                      directSuccessors.first.end());
  result.second.insert(directSuccessors.second.begin(),
                       directSuccessors.second.end());
  return result;
}
    ...
    switch (contactType) {
      ...
      default:
        throw std::invalid_argument(
            "Intersection fromContactTypes: Invalid contact type passed");
    }
  }
  return IntersectionType::Unknown;
}

IntersectionType getRightOfWayForTransition(lane::LaneId fromLaneId,
...
point::ParaPoint getIncomingParaPoint(lane::LaneId const laneId) {
  ...
  p.laneId = laneId;
  p.parametricOffset =
      (lane::isLaneDirectionNegative(laneId) ? physics::ParametricValue(0.)
...
point::ParaPoint getOutgoingParaPoint(lane::LaneId const laneId) {
  ...
  p.laneId = laneId;
  p.parametricOffset =
      (lane::isLaneDirectionNegative(laneId) ? physics::ParametricValue(1.)
                                             : physics::ParametricValue(0.));
  return p;
}

bool Intersection::isRoadSegmentEnteringIntersection(
  ...
  if ((routeIterator.roadSegmentIterator !=
       routeIterator.route.roadSegments.end()) &&
      (routeIterator.roadSegmentIterator !=
       routeIterator.route.roadSegments.begin())) {
    auto previousSegmentIter = routeIterator.roadSegmentIterator;
    previousSegmentIter--;
    // only look at the first parapoint per index
    auto const fromLane =
        previousSegmentIter->drivableLaneSegments.front().laneInterval.laneId;
    auto const toLane =
        routeIterator.roadSegmentIterator->drivableLaneSegments.front()
            .laneInterval.laneId;
...
IntersectionPtr Intersection::getIntersectionForRoadSegment(
    route::RouteIterator const &routeIterator) {
  IntersectionPtr result;
  route::RoadSegmentList::const_iterator routePreviousSegmentIter;
  bool const intersectionStart = isRoadSegmentEnteringIntersection(
      routeIterator, routePreviousSegmentIter);
  if (intersectionStart) {
    try {
      result = IntersectionPtr(
          new Intersection(routeIterator.route, routePreviousSegmentIter,
                           routeIterator.roadSegmentIterator));
    } catch (...) {
      access::getLogger()->warn(
          "Failed to create intersection at route iterator {} for route: {}",
          *routeIterator.roadSegmentIterator, routeIterator.route);
    }
  }
  return result;
}

std::vector<IntersectionPtr> Intersection::getIntersectionsForRoute(
  ...
  for (auto roadSegmentIter = route.roadSegments.begin();
    ...
    // @todo: also consider segments leaving intersection if first segment is
    // already within intersection
    auto roadSegmentResult = getIntersectionForRoadSegment(
        route::RouteIterator(route, roadSegmentIter));
...
IntersectionPtr Intersection::getNextIntersectionOnRoute(
    route::FullRoute const &route) {
  IntersectionPtr result;
  for (auto roadSegmentIter = route.roadSegments.begin();
       roadSegmentIter != route.roadSegments.end(); roadSegmentIter++) {
    // @todo: also consider segments leaving intersection if first segment is
    // already within intersection
    result = getIntersectionForRoadSegment(
        route::RouteIterator(route, roadSegmentIter));
    if (bool(result)) {
      return result;
    }
...
bool Intersection::isIntersectionOnRoute(route::FullRoute const &route) {
  for (auto roadSegmentIter = route.roadSegments.begin();
    ...
    // @todo: also consider segments leaving intersection if first segment is
    // already within intersection
    if (isRoadSegmentEnteringIntersection(
            route::RouteIterator(route, roadSegmentIter),
...
bool Intersection::segmentLeavesIntersectionOnRoute(
    route::FullRoute const &route,
  ...
  if (isLanePartOfAnIntersection(laneId)) {
    ...
    for (auto const &segment : roadSegmentIt->drivableLaneSegments) {
      if (segment.laneInterval.laneId == laneId) {
        for (auto successorId : segment.successors) {
          if (segmentLeavesIntersectionOnRoute(route, roadSegmentIt + 1,
                                               successorId)) {
            return true;
          }
        }
    ...
    return false;  // we did not find a (successor of a) a successor that leaves
                   // the intersection
  } else {
    // laneId is outside of the intersection, thus the route left the
    // intersection
...
lane::LaneIdSet Intersection::successorsOnRouteLeavingIntersection(
    route::FullRoute const &route,
    route::RoadSegmentList::const_iterator const &roadSegmentIt,
...
lane::LaneIdSet Intersection::successorsOnRouteLeavingIntersection(
  ...
  for (auto successorId : laneSegment.successors) {
    auto allReachableInternalAndOutgoingLanes =
        getAllReachableInternalAndOutgoingLanes(successorId,
                                                SuccessorMode::AnyIntersection);
    mLanesOnRoute.insert(allReachableInternalAndOutgoingLanes.first.begin(),
                         allReachableInternalAndOutgoingLanes.first.end());
    mOutgoingLanesOnRoute.insert(
        allReachableInternalAndOutgoingLanes.second.begin(),
...
void Intersection::extractRightOfWayAndCollectTrafficLights(
    route::LaneInterval const &laneInterval, lane::LaneIdSet const &successors,
    lane::LaneId &toLaneId) {
  ...
  for (auto succId : successors) {
    auto intersectionType =
        getRightOfWayForTransition(beforeLaneId, succId, useSuccessor);
    if ((mIntersectionType != IntersectionType::Unknown) &&
        (mIntersectionType != intersectionType)) {
      access::getLogger()->warn(
          "Different types of intersection detected! From {} To {}",
          beforeLaneId, succId);
    }
    toLaneId = succId;
    mIntersectionType = intersectionType;
...
Intersection::Intersection(
    ...
    : mRoutePlanningCounter(route.routePlanningCounter),
      mSegmentCountFromDestination(
          firstSegmentWithinIntersection->segmentCountFromDestination),
      mSpeedLimit(std::numeric_limits<physics::Speed>::max()) {
  // ensure all intersection arms are initialized, to be safe when later
  // accessing with std::map::at()
  ...
  for (auto const &segment :
       lastSegmentBeforeIntersection->drivableLaneSegments) {
    mIncomingLanesOnRoute.insert(segment.laneInterval.laneId);
    auto successors = successorsOnRouteLeavingIntersection(
        route, lastSegmentBeforeIntersection, segment);
    extractRightOfWayAndCollectTrafficLights(segment.laneInterval, successors,
                                             toLaneId);
  }
...
TurnDirection Intersection::turnDirection() const { return mTurnDirection; }
...
lane::LaneIdSet const &Intersection::entryLanes() const { return mEntryLanes; }
...
point::ParaPointList const &Intersection::incomingParaPointsWithHigherPriority()
    const {
  return mIncomingParaPointsWithHigherPriority;
}

point::ParaPointList const &Intersection::incomingParaPointsWithLowerPriority()
    const {
...
route::SegmentCounter Intersection::getRouteSegmentCountFromDestination()
    const {
  return mSegmentCountFromDestination;
}

void Intersection::updateRouteCounters(
    route::RoutePlanningCounter newRoutePlanningCounter,
...
void Intersection::processContactsForLane(lane::Lane const &lane,
...
void Intersection::extractCrossingLanes() {
  ...
  for (auto insideId : mLanesOnRoute) {
    mCrossingLanes.insert(mOverlapping[insideId].begin(),
                          mOverlapping[insideId].end());
  }
  // remove all lanes that are a successor of one of the incoming lanes on the
  // route
  // otherwise vehicles driving in the same or parallel direction would also be
  // considered...
  lane::LaneIdSet internalSuccessorsOfRouteEntry;
  for (auto laneId : mIncomingLanesOnRoute) {
    auto succ = getAllSuccessorsInLaneDirectionWithinIntersection(
        laneId, SuccessorMode::OwnIntersection);
...
void Intersection::insertOutgoingLane(lane::LaneId const laneId) {
  if (!isLanePartOfAnIntersection(laneId) &&
      (mIncomingLanes.count(laneId) == 0)) {
    if (mOutgoingLanes.insert(laneId).second) {
      point::ParaPoint p;
      p.laneId = laneId;
      p.parametricOffset =
          (isLaneDirectionNegative(laneId) ? physics::ParametricValue(0.)
                                           : physics::ParametricValue(1.));
...
point::ENUHeading getLaneDirectionalAngle(lane::LaneId laneId) {
  ...
  physics::ParametricValue laneParapoint =
      (isLaneDirectionNegative(lane) ? physics::ParametricValue(0.)
                                     : physics::ParametricValue(1.));

  // get the starting point and end point of the lane
  point::ECEFPoint laneStartPoint =
      getParametricPoint(lane, laneParapoint, physics::ParametricValue(0.5));
  point::ECEFPoint laneEndPoint =
      getParametricPoint(lane, physics::ParametricValue(1.) - laneParapoint,
                         physics::ParametricValue(0.5));

  // get the directional vector
  point::ECEFHeading directionalVector =
      point::createECEFHeading(laneStartPoint, laneEndPoint);
  point::ENUHeading headingENU =
      point::createENUHeading(directionalVector, laneStartPoint);

  return headingENU;
}

static void directionOfLanes(
    point::ParaPointList const &lanes, point::ENUHeading referenceAngle,
    std::vector<std::pair<point::ENUHeading, point::ParaPoint>>
        &intersectionLanesDirection)

{
  for (auto const &lane : lanes) {
    auto angle = getLaneDirectionalAngle(lane.laneId);

    // all angles should be relative to the angle of the arm where the route
...
TurnDirection turnDirectionForAngle(point::ENUHeading const angle) {
  // we assume that the angle is in range [0 2 PI] where 0 is the direction of
  // the entering lane
...
void Intersection::orderIntersectionArmsAndExtractTurnDirection() {
  ...
  auto const referenceAngle =
      getLaneDirectionalAngle(egoIntersectionEntryLaneId);

  // calculate the angle for all incoming lanes of the intersection
  directionOfLanes(mEntryParaPoints, referenceAngle,
                   intersectionLanesDirection);

  // calculate the angle for all outgoing lanes of the intersection
  directionOfLanes(mOutgoingParaPoints,
                   (referenceAngle + point::ENUHeading(M_PI)),
                   intersectionLanesDirection);
  ...
  for (auto laneDirection : intersectionLanesDirection) {
    ...
    if ((mTurnDirection == TurnDirection::Unknown) &&
        (mOutgoingLanesOnRoute.count(laneDirection.second.laneId) > 0)) {
...
void Intersection::extractLanesWithHigherPriority() {
  switch (mIntersectionType) {
    ...
    case IntersectionType::Stop:
      adjustLanesForYield();
      break;
    case IntersectionType::Unknown:  // fallthrough intended here, unknown means
                                     // be careful with crossing lanes
    ...
    case IntersectionType::HasWay:
      ...
      break;
    case IntersectionType::Crosswalk:  // no need for adaptation, there should
                                       // not be any crossing lanes
      break;
...
void Intersection::extractLanesWithLowerPriority() {
  for (auto laneId : mInternalLanes) {
    if ((mLanesOnRoute.find(laneId) == mLanesOnRoute.end()) &&
        (mInternalLanesWithHigherPriority.find(laneId) ==
         mInternalLanesWithHigherPriority.end())) {
...
void Intersection::extractLanesFromSameIntersectionArm() {
  for (auto laneId : mIncomingLanesOnRoute) {
    auto relevantLanes = getAllSuccessorsInLaneDirectionWithinIntersection(
        laneId, SuccessorMode::OwnIntersection);
    mInternalLanesFromSameIntersectionArm.insert(relevantLanes.begin(),
                                                 relevantLanes.end());
  }
}
...
void Intersection::calculateEnteringProrityParaPoints() {
  for (auto laneId : mIncomingLanes) {
    auto successorLanes = getAllSuccessorsInLaneDirectionWithinIntersection(
        laneId, SuccessorMode::OwnIntersection);
    ...
    for (auto successorLaneIter = successorLanes.begin();
         ...
         successorLaneIter++) {
      if (mInternalLanesWithHigherPriority.find(*successorLaneIter) !=
          mInternalLanesWithHigherPriority.end()) {
...
bool Intersection::turnDirectionCrossesStraightTraffic() const {
  ...
  if (access::isRightHandedTraffic()) {
    return (mTurnDirection == TurnDirection::Left) ||
           (mTurnDirection == TurnDirection::UTurn);
  }
  // left handed traffic
  else {
    return (mTurnDirection == TurnDirection::Right) ||
           (mTurnDirection == TurnDirection::UTurn);
  }
}

bool Intersection::outgoingIntersectionArmCanBeReached(
    lane::LaneId const laneId, TurnDirection const outgoingIntersectionArm) {
  for (auto reachableOutgoingLaneId :
       getAllReachableOutgoingLanes(laneId, SuccessorMode::OwnIntersection)) {
    if (mIntersectionArms[outgoingIntersectionArm].count(
            reachableOutgoingLaneId) > 0) {
      return true;
    }
  }
  return false;
...
void Intersection::
    addLaneAndSuccessorsToInternalLanesWithHigherPriorityIfCrossing(
        lane::LaneId const &lane,
        TurnDirection const restrictToOutgoingIntersectionArm) {
  auto lanesToCheck = getLaneAndAllSuccessorsInLaneDirectionWithinIntersection(
      lane, SuccessorMode::OwnIntersection);
  for (auto laneToCheckId : lanesToCheck) {
    if (mCrossingLanes.count(laneToCheckId) > 0) {
      if ((restrictToOutgoingIntersectionArm == TurnDirection::Unknown) ||
          outgoingIntersectionArmCanBeReached(
              laneToCheckId, restrictToOutgoingIntersectionArm)) {
...
void Intersection::
  ...
  for (auto laneId : lanes) {
    addLaneAndSuccessorsToInternalLanesWithHigherPriorityIfCrossing(
        laneId, restrictToOutgoingIntersectionArm);
  }
}
...
void Intersection::addLaneAndSuccessorsToInternalLanesWithHigherPriority(
    ...
    TurnDirection const restrictToOutgoingIntersectionArm) {
  for (auto laneId : lanes) {
    addLaneAndSuccessorsToInternalLanesWithHigherPriority(
        laneId, restrictToOutgoingIntersectionArm);
  }
}
...
void Intersection::adjustLanesForHasWay() {
  for (auto const &intersectionArm : mIntersectionArms) {
    for (auto laneId : intersectionArm.second) {
      auto sucessorLanes = getDirectSuccessorsInLaneDirectionWithinIntersection(
          laneId, SuccessorMode::OwnIntersection);
      for (auto sucessorLaneId : sucessorLanes) {
        auto trafficRuleAtOtherLane = getRightOfWayForTransition(
            laneId, sucessorLaneId, !isLaneDirectionNegative(laneId));

        // has way lanes have to be respected if we turn
        if ((trafficRuleAtOtherLane == IntersectionType::HasWay) &&
            turnDirectionCrossesStraightTraffic()) {
          addLaneAndSuccessorsToInternalLanesWithHigherPriorityIfCrossing(
              sucessorLaneId);
        }
      }
  ...
  for (auto const &intersectionArm : mIntersectionArms) {
    for (auto laneId : intersectionArm.second) {
      ...
      for (auto sucessorLaneId : sucessorLanes) {
        ...
        else if ((trafficRuleAtOtherLane == IntersectionType::Yield) &&
                 turnDirectionCrossesStraightTraffic()) {
          addLaneAndSuccessorsToInternalLanesWithHigherPriorityIfCrossing(
              sucessorLaneId);
        }
...
void Intersection::adjustPriorityToRight() {
  if ((IntersectionType::PriorityToRightAndStraight == mIntersectionType) &&
    ...
    addLaneAndSuccessorsToInternalLanesWithHigherPriority(
        mIntersectionArms[TurnDirection::Right], TurnDirection::Left);
    addLaneAndSuccessorsToInternalLanesWithHigherPriority(
        mIntersectionArms[TurnDirection::Right], TurnDirection::UTurn);
  } else {
    // all traffic from right
    addLaneAndSuccessorsToInternalLanesWithHigherPriority(
        mIntersectionArms[TurnDirection::Right]);
  }

  // plus take care on the straight traffic when turning
  if (turnDirectionCrossesStraightTraffic()) {
    addLaneAndSuccessorsToInternalLanesWithHigherPriorityIfCrossing(
        mIntersectionArms[TurnDirection::Straight]);
    if (IntersectionType::PriorityToRightAndStraight == mIntersectionType) {
      // the other straight driving lanes have priority, too
      addLaneAndSuccessorsToInternalLanesWithHigherPriorityIfCrossing(
          mIntersectionArms[TurnDirection::Right], TurnDirection::Left);
      addLaneAndSuccessorsToInternalLanesWithHigherPriorityIfCrossing(
          mIntersectionArms[TurnDirection::Left], TurnDirection::Right);
    }
  }
}
...
bool Intersection::objectWithinIntersection(
...
bool Intersection::objectOnIncomingLane(
    match::MapMatchedObjectBoundingBox const &object) const {
  for (auto const &occupiedLane : object.laneOccupiedRegions) {
...
bool Intersection::objectOnInternalLaneWithLowerPriority(
    match::MapMatchedObjectBoundingBox const &object) const {
  for (auto const &occupiedLane : object.laneOccupiedRegions) {
...
bool Intersection::objectOnIncomingLaneWithLowerPriority(
    match::MapMatchedObjectBoundingBox const &object) const {
  for (auto const &occupiedLane : object.laneOccupiedRegions) {
...
bool Intersection::objectOnLaneWithLowerPriority(
    match::MapMatchedObjectBoundingBox const &object) const {
  return objectOnInternalLaneWithLowerPriority(object) ||
         objectOnIncomingLaneWithLowerPriority(object);
}

bool Intersection::objectOnInternalLaneWithHigherPriority(
    match::MapMatchedObjectBoundingBox const &object) const {
  for (auto const &occupiedLane : object.laneOccupiedRegions) {
...
bool Intersection::objectOnIncomingLaneWithHigherPriority(
    match::MapMatchedObjectBoundingBox const &object) const {
  for (auto const &occupiedLane : object.laneOccupiedRegions) {
...
bool Intersection::objectOnLaneWithHigherPriority(
    match::MapMatchedObjectBoundingBox const &object) const {
  return objectOnInternalLaneWithHigherPriority(object) ||
         objectOnIncomingLaneWithHigherPriority(object);
}

bool Intersection::objectOnCrossingLane(
    match::MapMatchedObjectBoundingBox const &object) const {
  for (auto const &occupiedLane : object.laneOccupiedRegions) {
...
bool Intersection::objectOnIntersectionRoute(
    match::MapMatchedObjectBoundingBox const &object) const {
  for (auto const &occupiedLane : object.laneOccupiedRegions) {
...
bool Intersection::objectRouteCrossesIntersectionRoute(
...
bool Intersection::objectRouteCrossesIntersection(
    route::FullRoute const &objectRoute) const {
...
bool Intersection::objectRouteFromSameArmAsIntersectionRoute(
    route::FullRoute const &objectRoute) const {
  bool routeLaneFound = false;
  for (auto const &roadSegment : objectRoute.roadSegments) {
    for (auto const &laneSegment : roadSegment.drivableLaneSegments) {
      if ((mInternalLanesFromSameIntersectionArm.count(
               laneSegment.laneInterval.laneId) > 0) ||
        ...
        // if we previously have found a route lane and are now again outside
        // the intersection: route is identical
        return routeLaneFound;
      } else {
        // nothing decided yet since neither a lane on intersection route found
        // nor any other internal lane touched
      }
    }
  }
  return routeLaneFound;
...
bool Intersection::objectRouteOppositeToIntersectionRoute(
    route::FullRoute const &objectRoute) const {
  route::FindWaypointResult findResult(objectRoute);
  for (auto const &laneOnOutgoingIntersectionArm :
       mIntersectionArms.at(mTurnDirection)) {
    findResult =
        route::findWaypoint(laneOnOutgoingIntersectionArm, objectRoute);
    if (findResult.isValid()) {
      break;
    }
  }
  if (!findResult.isValid()) {
    ...
    if (!objectRoute.roadSegments.empty() &&
        !objectRoute.roadSegments.begin()->drivableLaneSegments.empty() &&
        mInternalLanes.count(objectRoute.roadSegments.begin()
                                 ->drivableLaneSegments.begin()
                                 ->laneInterval.laneId) > 0) {
      ...
      for (auto const &laneOnOutgoingIntersectionArm :
        ...
        if (internalLanesFromOutgoingIntersectionArm.count(
                objectRoute.roadSegments.begin()
                    ->drivableLaneSegments.begin()
                    ->laneInterval.laneId) > 0) {
          // and object came from outgoing intersection arm
          findResult = route::findWaypoint(objectRoute.roadSegments.begin()
                                               ->drivableLaneSegments.begin()
                                               ->laneInterval.laneId,
                                           objectRoute);
        }
  ...
  if (findResult.isValid()) {
    ...
    for (auto const &laneOnIncomingIntersectionArm :
         mIntersectionArms.at(TurnDirection::UTurn)) {
      auto result = route::findWaypoint(laneOnIncomingIntersectionArm,
                                        remainingObjectRoute);
      if (result.isValid()) {
        return true;
      }
    }
...
physics::Distance Intersection::objectDistanceToIntersection(
  ...
  if (objectWithinIntersection(object.mapMatchedBoundingBox)) {
    ...
    for (auto internalLaneId : mInternalLanes) {
      minDistance = std::min(minDistance,
                             lane::getDistanceToLane(internalLaneId, object));
    }
  }
  return minDistance;
}

physics::Distance Intersection::objectInterpenetrationDistanceWithIntersection(
    match::Object const &object) const {
  ...
  for (auto const &occupiedLane :
    ...
    if (mInternalLanes.count(occupiedLane.laneId) > 0u) {
      // internal lane
      // if this lane is at the intersection border and the object is still
      // touching that border
      // then the range covered within this lane has to be considered for the
      // inter penetration distances
      bool isRelevantBorderLane = false;
      if ((occupiedLane.longitudinalRange.maximum ==
           physics::ParametricValue(1.)) ||
          (occupiedLane.longitudinalRange.minimum ==
           physics::ParametricValue(0.))) {
        // check predecessors
        ...
        if (findPredecessors != mPredecessor.end()) {
          for (auto predecessorId : findPredecessors->second) {
            if (!isLanePartOfAnIntersection(predecessorId)) {
              ...
              // (outgoingParaPonts logic)
              auto const entryBorderParaPoint =
                  getOutgoingParaPoint(occupiedLane.laneId);
              if ((entryBorderParaPoint.parametricOffset ==
                   occupiedLane.longitudinalRange.maximum) ||
                  (entryBorderParaPoint.parametricOffset ==
                   occupiedLane.longitudinalRange.minimum)) {
          ...
          if (findSuccessors != mSuccessor.end()) {
            for (auto successorId : findSuccessors->second) {
              if (!isLanePartOfAnIntersection(successorId)) {
                ...
                // (incomingParaPonts logic)
                auto const entryBorderParaPoint =
                    getIncomingParaPoint(occupiedLane.laneId);
                if ((entryBorderParaPoint.parametricOffset ==
                     occupiedLane.longitudinalRange.maximum) ||
                    (entryBorderParaPoint.parametricOffset ==
                     occupiedLane.longitudinalRange.minimum)) {
      ...
      if (isRelevantBorderLane) {
        ...
        // otherwise this lane is touched as part of an actual internal
      ...
      // not an internal lane, check if different intersection arms covered by
      // the vehicle
      for (auto const &intersectionArm : mIntersectionArms) {
        if (intersectionArm.second.count(occupiedLane.laneId) > 0u) {
          if (coveredIntersectionArms == TurnDirection::Unknown) {
            ...
            // covering two different intersection arms, actually crossing the
            // intersection
  ...
  if ((maxDistance > physics::Distance(0.)) &&
      (coveredIntersectionArms == TurnDirection::Unknown)) {
...
bool Intersection::objectRouteCrossesLanesWithHigherPriority(
    route::FullRoute const &objectRoute) const {
...
void Intersection::collectTrafficLights(lane::LaneId fromLaneId,
                                        lane::LaneId toLaneId,
                                        bool useSuccessor) {
...
physics::Speed Intersection::getSpeedLimit() const { return mSpeedLimit; }
...
landmark::TrafficLightType Intersection::extractTrafficLightType(
...
bool Intersection::isSolidTrafficLight(landmark::LandmarkId trafficLightId) {
  switch (extractTrafficLightType(trafficLightId)) {
    ...
    default:
      throw std::invalid_argument(
          "Intersection isSolidTrafficLight: Invalid traffic light type given");
  }
}
