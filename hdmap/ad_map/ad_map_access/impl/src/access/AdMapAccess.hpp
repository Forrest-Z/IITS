 ...
 *
 * The type of map store depends on the implementation that is linked to the
 * application. There exists two variants of AdMapAcces, one for AD Map Store
 * and one for Lanelet.
 *
 * The variants (adg::ad::map::Store, Lanelet) themselves are a singleton and
 * therefore any consumer/user of the Store can use it directly.
 */
class AdMapAccess {
 public:
  /**
   ...
   *
   * Before accessing the instance you have to call AdMapAccess::initialize() to
   * initialize the static instance. If the instance is not initialized when
   * this function is called, an exception is thrown.
   *
   ...
   * @return true if initialization was successful (i.e. config file exists or
   * singleton was already initialized)
   * @return false if OpenDRIVE data wasn't valid or the singleton was already
   * initialized differently
   */
  bool initializeFromOpenDriveContent(
   ...
   *
   * The ENURefPoint of this is set at the beginning of getMapMatchedPositions()
   * call.
   */
