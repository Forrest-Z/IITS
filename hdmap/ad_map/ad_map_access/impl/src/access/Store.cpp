...
bool Store::empty() const { return lane_map_.empty() && landmark_map_.empty(); }

////////////////////
// Access Operations
MapMetaData const &Store::getMetaData() const { return meta_data_; }
...
PartitionIdList Store::getPartitions() const {
  ...
  for (auto partition_id_and_ids : part_landmark_map_) {
    if (std::find(part_ids.begin(), part_ids.end(),
                  partition_id_and_ids.first) == part_ids.end()) {
      part_ids.push_back(partition_id_and_ids.first);
    }
  }
  return part_ids;
...
lane::LaneIdList Store::getLanes(std::string const &type_filter,
                                 bool is_hov) const {
  lane::LaneIdList ids;
...
lane::LaneIdList Store::getLanes(PartitionId partition_id,
...
landmark::Landmark::ConstPtr Store::getLandmarkPtr(
    landmark::LandmarkId id) const {
