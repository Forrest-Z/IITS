...
bool Store::save(serialize::ISerializer &serializer, bool use_magic,
                 bool use_embedded_geometry, bool use_geometry_store) {
...
bool Store::isValid() const {
  ...
  for (auto id_and_lane : lane_map_) {
    ...
    if (!lane::isValid(lane_id, false) || !lane ||
        !lane::isValid(*lane, false)) {
      return false;
    }
  }
...
bool Store::serialize(serialize::ISerializer &serializer) {
  ...
  bool ok = serializer.serialize(serialize::SerializeableMagic::Store) &&
            serializer.serialize(use_magic_) &&
            serializer.serialize(use_embedded_geometry_) &&
            serializer.serialize(use_geometry_store_);
  ...
  {
    if (serializer.isStoring()) {
      ok = serializer.serializeEmptyObjectVec() &&
           serializer.serializeEmptyObjectVec();
    } else {
      ok = serializer.serializeEmptyObjectVec() &&
           serializer.serializeEmptyObjectVec();
    }
...
bool Store::storeGeometry(GeometryStore &gs) {
  for (auto lane : lane_map_) {
    if (!gs.store(lane.second)) {
      access::getLogger()->error("Store geometry failed for lane {}",
                                 lane.first);
      return false;
    }
  }
...
bool Store::restoreGeometry(const GeometryStore &gs) {
  for (auto lane : lane_map_) {
    if (!gs.restore(lane.second)) {
      access::getLogger()->error("Restore geometry failed for lane {}",
                                 lane.first);
      return false;
    }
  }
