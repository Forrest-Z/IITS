...
GeometryStore::~GeometryStore() { destroy(); }
  ...
  if (lane) {
    ...
    if (it == lane_items_.end()) {
      ...
      if (store(lane, lane::ContactLocation::LEFT, offset_left, size_left)) {
        ...
        if (store(lane, lane::ContactLocation::RIGHT, offset_right,
                  size_right)) {
      ...
      access::getLogger()->error("GeometryStore: Lane already in Store?! {}",
                                 id);
      ...
      if (restore(left, item.leftEdgeOffset, item.leftEdgePoints)) {
        ...
        if (restore(right, item.rightEdgeOffset, item.rightEdgePoints)) {
          ...
          access::getLogger()->error(
              "GeometryStore: Lane right edge not in Store?! {}", id);
        }
      } else {
        access::getLogger()->error(
            "GeometryStore: Lane left edge not in Store?! {}", id);
      }
    } else {
  ...
  if (lane) {
    ...
    if (it != lane_items_.end()) {
      ...
      if (restore(left, item.leftEdgeOffset, item.leftEdgePoints)) {
        point::ECEFEdge right;
        if (restore(right, item.rightEdgeOffset, item.rightEdgePoints)) {
          if (lane->edgeLeft.ecefEdge == left &&
              lane->edgeRight.ecefEdge == right) {
            return true;
          } else {
            access::getLogger()->error(
                "GeometryStore: Lane geometry mismatch?! {}", id);
          }
        } else {
          access::getLogger()->error(
              "GeometryStore: Lane right edge not in Store?! {}", id);
...
bool GeometryStore::store(lane::Lane::ConstPtr lane,
                          lane::ContactLocation location, uint32_t &offs3d,
                          uint32_t &size) {
  if ((location != lane::ContactLocation::LEFT) &&
      (location != lane::ContactLocation::RIGHT)) {
    throw std::runtime_error("Location must be LEFT or RIGHT");
  }
  const point::ECEFEdge &ecef = (location == lane::ContactLocation::LEFT)
                                    ? lane->edgeLeft.ecefEdge
                                    : lane->edgeRight.ecefEdge;
  ...
  for (auto contact_lane : contact_lanes) {
    ...
    if (it != lane_items_.end()) {
      ...
      if (clane) {
        const point::ECEFEdge &ecef1 = (location == lane::ContactLocation::LEFT)
                                           ? clane->edgeRight.ecefEdge
                                           : clane->edgeLeft.ecefEdge;
        if (ecef == ecef1) {
          offs3d = (location == lane::ContactLocation::LEFT)
                       ? it->second.rightEdgeOffset
                       : it->second.leftEdgeOffset;
          return true;
        }
      }
    }
...
bool GeometryStore::restore(point::ECEFEdge &ecef, uint32_t offset3d,
                            uint32_t points3d) const {
  if (!ecef.empty()) {
...
bool GeometryStore::expand() {
  if (store_ == nullptr) {
    ...
    if (store != nullptr) {
      ...
      access::getLogger()->error("GeometryStore: Cannot expand to {} bytes.",
                                 bytes);
      return false;
    }
  }
...
bool GeometryStore::create(uint32_t capacity3d) {
  ...
  if (store_ != nullptr) {
    ...
    access::getLogger()->error("GeometryStore: Cannot allocate {} bytes.",
                               bytes);
    return false;
  }
}
...
bool GeometryStore::serialize(serialize::ISerializer &serializer) {
  bool ok =
      serializer.serialize(serialize::SerializeableMagic::GeometryStore) &&
      serializer.serializeObjectMap(lane_items_) &&
      serializer.serialize(points3d_);

  /*Todo the two lines below will be deleted after the map are generated again
   * without use_zfp_*/
  bool use_zfp_ = false;
