...
point::ENUHeading getObjectENUHeading(
  ...
  if (rearLeftAvailable) {
    rearLeftPosition = mapMatchedBoundingBox
                           .referencePointPositions[int32_t(
                               match::ObjectReferencePoints::RearLeft)]
                           .front()
                           .queryPoint;
  ...
  if (rearRightAvailable) {
    rearRightPosition = mapMatchedBoundingBox
                            .referencePointPositions[int32_t(
                                match::ObjectReferencePoints::RearRight)]
                            .front()
                            .queryPoint;
  ...
  bool const frontLeftAvailable =
      mapMatchedBoundingBox
          .referencePointPositions[int32_t(
              match::ObjectReferencePoints::FrontLeft)]
          .size() > 0u;
  if (frontLeftAvailable) {
    frontLeftPosition = mapMatchedBoundingBox
                            .referencePointPositions[int32_t(
                                match::ObjectReferencePoints::FrontLeft)]
                            .front()
                            .queryPoint;
  ...
  if (frontRightAvailable) {
    frontRightPosition = mapMatchedBoundingBox
                             .referencePointPositions[int32_t(
                                 match::ObjectReferencePoints::FrontRight)]
                             .front()
                             .queryPoint;
  ...
  if (rearLeftAvailable && rearRightAvailable && frontLeftAvailable &&
      frontRightAvailable) {
    auto const rearCenterPosition =
        0.5 * (rearLeftPosition + rearRightPosition);
    auto const frontCenterPosition =
        0.5 * (frontLeftPosition + frontRightPosition);
    ecefHeading =
        point::createECEFHeading(rearCenterPosition, frontCenterPosition);
    ...
    ecefHeading =
        point::createECEFHeading(rearRightPosition, frontRightPosition);
  } else if (frontLeftAvailable && frontRightAvailable) {
    ecefHeading =
        point::createECEFHeading(frontLeftPosition, frontRightPosition);
    ...
    throw std::runtime_error(
        "point::createENUHeading no two corner points available. Heading "
        "estimate is impossible");
  }
  auto enuHeading = point::createENUHeading(ecefHeading);
  if (rotateENUHeading) {
    enuHeading =
        point::createENUHeading(static_cast<double>(enuHeading) + M_PI_2);
  }
  return enuHeading;
}
...
physics::Distance signedDistanceToLane(
  ...
  auto findCheckPosition = std::find_if(
      std::begin(mapMatchedPositions), std::end(mapMatchedPositions),
  ...
  if (findCheckPosition != std::end(mapMatchedPositions)) {
    ...
    if (findCheckPosition->type == match::MapMatchedPositionType::LANE_IN) {
    ...
    } else if (findCheckPosition->type ==
               match::MapMatchedPositionType::LANE_LEFT) {
      if (findCheckPosition->lanePoint.lateralT > physics::RatioValue(0.)) {
        throw std::runtime_error(
            "ad::map::match::signedDistanceToLane: inconsistent map matched "
            "positions."
            " Expected lateralT to be < 0. for LANE_LEFT");
      }
      distance = findCheckPosition->lanePoint.lateralT *
                 findCheckPosition->lanePoint.laneWidth;
    } else if (findCheckPosition->type ==
               match::MapMatchedPositionType::LANE_RIGHT) {
      ...
      distance =
          (findCheckPosition->lanePoint.lateralT - physics::RatioValue(1.)) *
          findCheckPosition->lanePoint.laneWidth;
    } else {
      throw std::runtime_error(
          "ad::map::match::signedDistanceToLane: inconsistent map matched "
