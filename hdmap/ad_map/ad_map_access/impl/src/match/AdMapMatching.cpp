...
match::MapMatchedPositionConfidenceList AdMapMatching::findLanesInputChecked(
  ...
  std::sort(
      ...
      [](MapMatchedPosition const &left, MapMatchedPosition const &right) {
        ...
        return left.matchedPointDistance > right.matchedPointDistance;
      });
...
point::ENUHeading AdMapMatching::getLaneENUHeading(
    MapMatchedPosition const &mapMatchedPosition) {
...
double AdMapMatching::getHeadingFactor(
    MapMatchedPosition const &matchedPosition) {
...
MapMatchedObjectBoundingBox AdMapMatching::getMapMatchedBoundingBox(
  ...
  // in
  // between the the sampling points, we have to ensure: point matching distance
  // >= sampling distance
  ...
  // Otherwise the fill-up to inner borders of the region is not working
  // properly
...
void AdMapMatching::addLaneRegions(
  ...
  // longitudinal)
  // Second, ensure all inner-regions are expanded to the borders (lateral AND
  // longitudinal)
  ...
  // span up to the borders
  // since in the above loop only collects actual matches within the lane
  // Since the likelihood for a match exactly at the border is very low, we have
  // to enlarge
  // the inner regions to cover also the space in between the map matching
  // points
  ...
  for (auto const &currentPosition : mapMatchedPositions) {
    if (!isActualWithinLaneMatch(currentPosition)) {
      ...
      if (search != laneOccupiedRegions.end()) {
        ...
        // present in each case, or out-of-lane and
