...
point::ENUHeading getENUHeading(Landmark const &landmark) {
  return point::createENUHeading(
      point::createECEFHeading(landmark.position, landmark.orientation));
}
...
Landmark::ConstPtr getLandmarkPtr(LandmarkId const &id) {
  ...
  if (!bool(landmarkPtr)) {
    throw std::invalid_argument(
        "ad::map::landmark::getLandmarkPtr: LandmarkId not found in store");
  }
  return landmarkPtr;
}
...
LandmarkIdList getVisibleLandmarks(lane::LaneId const &laneId) {
  ...
  if (!bool(lanePtr)) {
    throw std::invalid_argument(
        "ad::map::landmark::getVisibleLandmarks: laneId not found in store");
  }
...
LandmarkId uniqueLandmarkId(point::GeoPoint const &geoPoint) {
  ...
  for (auto const landmarkId : landmarksIds) {
    ...
    if (landmarkPtr->boundingBox.isValid) {
      if (distance <= landmarkPtr->boundingBox.length &&
