...
size_t CoordinateTransform::getENURef() const { return enu_ref_; }
...
ENUPoint CoordinateTransform::Geo2ENU(const GeoPoint &pt) const {
  if (isENUValid()) {
    if (isValid(pt)) {
      ...
      double de =
          (A / enu_tmp1_ + enu_h_) * enu_cp_ * dlam -
          (A * (1 - E2) / enu_tmp1_3_ + enu_h_) * enu_sp_ * dphi * dlam +
          enu_cp_ * dlam * dh;
      double dn = (A * (1 - E2) / enu_tmp1_3_ + enu_h_) * dphi +
                  1.5 * enu_cp_ * enu_sp_ * A * E2 * dphi_2 +
                  enu_sp_2_ * dh * dphi +
                  0.5 * enu_sp_ * enu_cp_ * (A / enu_tmp1_ + enu_h_) * dlam_2;
      double du = dh -
                  0.5 * (A - 1.5 * A * E2 * enu_cp_2_ + 0.5 * A * E2 + enu_h_) *
                      dphi_2 -
                  0.5 * enu_cp_2_ * (A / enu_tmp1_ - enu_h_) * dlam_2;
      return createENUPoint(de, dn, du);
    } else {
      access::getLogger()->error(
          "Cannot convert from Geo to ENU: Input Point invalid.");
      throw std::invalid_argument(
          "Cannot convert from Geo to ENU: Input Point invalid.");
    }
  } else {
    access::getLogger()->error(
        "Cannot convert from Geo to ENU: Reference Point not defined.");
    throw std::invalid_argument(
        "Cannot convert from Geo to ENU: Reference Point not defined.");
  }
}
...
GeoPoint CoordinateTransform::ENU2Geo(const ENUPoint &pt) const {
  if (isENUValid()) {
    if (isValid(pt)) {
      ...
      access::getLogger()->error(
          "Cannot convert from ENU to Geo: Input Point invalid.");
      throw std::invalid_argument(
          "Cannot convert from ENU to Geo: Input Point invalid.");
    }
  } else {
    access::getLogger()->error(
        "Cannot convert from ENU to Geo: Reference Point not defined.");
    throw std::invalid_argument(
        "Cannot convert from ENU to Geo: Reference Point not defined.");
  }
}
...
ECEFPoint CoordinateTransform::Geo2ECEF(const GeoPoint &pt) const {
  if (isValid(pt)) {
    ...
    access::getLogger()->error(
        "Cannot convert from Geo to ECEF: Input point invalid.");
    throw std::invalid_argument(
        "Cannot convert from Geo to ECEF: Input point invalid.");
  }
}
...
GeoPoint CoordinateTransform::ECEF2Geo(const ECEFPoint &pt) const {
  if (isValid(pt)) {
    ...
    return createGeoPoint(Longitude(radians2degree(lon)),
                          Latitude(radians2degree(lat)), Altitude(alt));
  } else {
    access::getLogger()->error(
        "Cannot convert from ECEF to Geo: Input point invalid.");
    throw std::invalid_argument(
        "Cannot convert from ECEF to Geo: Input point invalid.");
  }
}
...
ECEFPoint CoordinateTransform::ENU2ECEF(const ENUPoint &pt) const {
  if (isENUValid()) {
    if (isValid(pt)) {
      ...
      access::getLogger()->error(
          "Cannot convert from ENU to ECEF: ENU Reference Point invalid.");
      throw std::invalid_argument(
          "Cannot convert from ENU to ECEF: ENU Reference Point invalid.");
    }
  } else {
    access::getLogger()->error(
        "Cannot convert from ENU to ECEF: Input Point invalid.");
    throw std::invalid_argument(
        "Cannot convert from ENU to ECEF: Input Point invalid.");
  }
}
...
ENUPoint CoordinateTransform::ECEF2ENU(const ECEFPoint &pt) const {
  if (isENUValid()) {
    if (isValid(pt)) {
      ...
      access::getLogger()->error(
          "Cannot convert from ECEF to ENU: ENU Reference Point invalid.");
      throw std::invalid_argument(
          "Cannot convert from ECEF to ENU: ENU Reference Point invalid.");
    }
  } else {
    access::getLogger()->error(
        "Cannot convert from ECEF to ENU: Input Point invalid.");
    throw std::invalid_argument(
        "Cannot convert from ECEF to ENU: Input Point invalid.");
  }
}
