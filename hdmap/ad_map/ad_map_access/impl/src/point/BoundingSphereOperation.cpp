...
void expandBounds(point::ECEFPoint &upperBound, point::ECEFPoint &lowerBound,
                  point::ECEFPoint const &point) {
...
point::BoundingSphere calcBoundingSphere(Geometry const &geometryLeft,
                                         Geometry const &geometryRight) {
...
::ad::map::point::BoundingSphere operator+(
  ...
  auto const sphereCenterDistance =
      ::ad::map::point::vectorLength(fromLargeToSmallCenter);
  ...
  auto const displacement =
      0.5 * (sphereCenterDistance - large->radius + small->radius);
  if (displacement <= ::ad::physics::Distance(0.)) {
    ...
    result.center =
        large->center +
        ::ad::physics::Distance(displacement / sphereCenterDistance) *
            (fromLargeToSmallCenter);
