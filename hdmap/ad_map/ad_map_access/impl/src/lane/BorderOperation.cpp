...
void removeDegeneratedEdgePoints(point::ENUEdge &edge,
                                 point::ENUEdge const *previousEdge = nullptr) {
  if (edge.size() < 2u) {
    return;
  }
  ...
  if (previousEdge != nullptr) {
    ...
    previousEdgeDir = previousEdge->at(previousEdge->size() - 1u) -
                      previousEdge->at(previousEdge->size() - 2u);
    auto nextEdgeDir = edge[0] - previousEdge->at(previousEdge->size() - 1u);

    // if the two edges are near to each other or the direction is dramatically
    // different,
...
std::vector<std::size_t> calcSmallerEdgeIndexPairs(
    std::vector<physics::ParametricValue> const &biggerEdgeParametricPoints,
  ...
  for (std::size_t biggerIndex = 1u;
       biggerIndex < biggerEdgeParametricPoints.size(); biggerIndex++) {
    physics::ParametricValue minParametricDistance(1.);
    bool matchingIndexFound = false;
    // since both lists are sorted in ascending, it's ok to start search at last
    // found index
    for (std::size_t smallerIndex = matchingSmallerIndex;
         (!matchingIndexFound) &&
         (smallerIndex < smallerEdgeParametricPoints.size());
         smallerIndex++) {
      auto const parametricDistance =
          std::fabs(smallerEdgeParametricPoints[smallerIndex] -
                    biggerEdgeParametricPoints[biggerIndex]);
...
point::ENUPoint interpolatePoint(
    point::ENUPoint const &pointBefore, point::ENUPoint const &pointAfter,
  ...
  physics::ParametricValue const parametricOffset(
      (pointInterpolatedParametricEdgeOffset -
       pointBeforeParametricEdgeOffset) /
...
void fillSmallerEdge(point::ENUEdge const &biggerEdge,
                     point::ENUEdge &smallerEdge) {
  ...
  auto const smallerEdgeIndices = calcSmallerEdgeIndexPairs(
      biggerEdgeParametricPoints, smallerEdgeParametricPoints);
  ...
  for (std::size_t i = 1u; i < smallerEdgeIndices.size() - 1u; ++i) {
    // i-1, i, i+1 indices are valid for smallerEdgeIndices and
    // biggerEdgeParametricPoints

    if (  // smaller point already processed, insert an interpolated
        (smallerEdgeIndices[i] == lastWrittenSmallerEdgeIndex)
        // the point before the last point of the smaller edge is already
        // written, so we have to interpolate till the end
        || (lastWrittenSmallerEdgeIndex == (smallerEdge.size() - 2u))) {
      auto interpolatedPoint = interpolatePoint(
          smallerEdge[lastWrittenSmallerEdgeIndex],
          smallerEdge[lastWrittenSmallerEdgeIndex + 1u],
          biggerEdgeParametricPoints[i - 1], biggerEdgeParametricPoints[i],
...
void normalizeBorder(ENUBorder &border, ENUBorder const *previousBorder) {
  ...
  if ((previousBorder != nullptr) && (previousBorder->left.size() >= 2u) &&
      (previousBorder->right.size() >= 2u)) {
...
IndexPairs getIndexPairs(point::ENUEdge const &leftEdge,
                         point::ENUEdge const &rightEdge) {
  ...
  if (leftEdge.size() == rightEdge.size()) {
    ...
    indexPairs.rightEdgeIndices = calcSmallerEdgeIndexPairs(
        getParametricEdgePoints(leftEdge), getParametricEdgePoints(rightEdge));
  } else {
    indexPairs.leftEdgeIndices = calcSmallerEdgeIndexPairs(
        getParametricEdgePoints(rightEdge), getParametricEdgePoints(leftEdge));
    indexPairs.rightEdgeIndices.swap(equalIndices);
  }
  return indexPairs;
}
...
point::ENUEdge getLateralAlignmentEdge(
  ...
  return point::getLateralAlignmentEdge(border.left, border.right,
                                        lateralAlignment);
}
...
inline point::ENUHeading createHeadingFromBorderPoints(
    point::ENUPoint const &ptLeft, point::ENUPoint const &ptRight) {
  auto const headingLeftToRight = point::createENUHeading(ptLeft, ptRight);
  auto const resultHeading =
      point::createENUHeading(static_cast<double>(headingLeftToRight) + M_PI_2);
  return resultHeading;
}

point::ENUHeading getENUHeading(std::vector<ENUBorder> const &borderList,
                                point::ENUPoint const &enuPoint) {
  ...
  for (auto borderIter = borderList.begin(); borderIter != borderList.end();
       borderIter++) {
    ...
    if ((edgeLeftLength == physics::Distance(0.)) ||
        (edgeRightLength == physics::Distance(0.))) {
      if ((borderIter->left.size() == 0u) || (borderIter->right.size() == 0u)) {
        access::getLogger()->error(
            "ad::map::lane::getENUHeading: invalid enu border of size "
            "border-left:{} border-rigth:{} query-point:{}!",
            borderIter->left, borderIter->right, enuPoint);
        return resultHeading;
      }

      // the question here is: are we actually before or after the border
      auto const headingLeftToRight =
          point::createENUHeading(borderIter->left[0], borderIter->right[0]);
      auto const headingLeftToPoint =
          point::createENUHeading(borderIter->left[0], enuPoint);
      auto const headingDiff = point::createENUHeading(
          static_cast<double>(headingLeftToPoint - headingLeftToRight));
      ...
      auto parametricOffsetLeft = point::findNearestPointOnEdge(
          borderIter->left, edgeLeftLength, enuPoint);
      auto parametricOffsetRight = point::findNearestPointOnEdge(
          borderIter->right, edgeRightLength, enuPoint);

      // catch errors
      if ((!parametricOffsetLeft.isValid()) ||
          (!parametricOffsetRight.isValid())) {
        access::getLogger()->error(
            "ad::map::lane::getENUHeading: invalid input data border-left:{} "
            "border-rigth:{} query-point:{}",
            borderIter->left, borderIter->right, enuPoint);
        return resultHeading;
      }
      ...
      if ((parametricOffsetLeft == physics::ParametricValue(0.)) ||
        ...
        auto const ptLeft =
            point::getParametricPoint(borderIter->left, parametricOffsetLeft);
        auto const ptRight =
            point::getParametricPoint(borderIter->right, parametricOffsetRight);
        bool const withinBorder =
            isPointWithinBorderPoints(ptLeft, ptRight, enuPoint);
  ...
  // if we didn't find any actual match in between so far, use either the first
  // before or the last after to create the
  // heading
  if (firstBeforeBorderDetected != borderList.end()) {
...
physics::Distance getDistanceEnuPointToLateralAlignmentEdge(
  ...
  auto const parametricOffset =
      point::findNearestPointOnEdge(lateralAlignmentEdge, edgeLength, enuPoint);
  auto const projectedPoint = point::getParametricPoint(
      lateralAlignmentEdge, edgeLength, parametricOffset);
  auto const distance = point::distance(projectedPoint, enuPoint);
  return distance;
}

bool areEdgesContinuous(point::ENUEdge const &first,
                        point::ENUEdge const &second) {
  if ((first.size() < 2u) || (second.size() < 2u)) {
    return true;
  }

  auto const firstToSecondDistance =
      point::distance(second.front(), first.back());
...
void performMakeTransitionToSecondEdgeContinuous(
    point::ENUEdge const &first, point::ENUEdge &second,
  ...
  double const sinAngle = static_cast<double>(point::vectorLength(
      point::vectorCrossProduct(averageEdgeDir, firstToSecondDir)));
  // the distance of our interpolation changes depend on the offset of the two
  // edges ends and the lateral offset
  // (given by the sine of the angle between the general edge direction and the
  // offset direction)
  ...
  physics::ParametricValue const parametricInterpolatingDistance(
      std::min(.95, interpolatingDistance / secondLength));
  ...
  if ((secondOtherLane != nullptr) &&
      (secondOtherLane->size() == second.size())) {
  ...
  for (std::size_t i = 1u; i < secondParametricEdgePoints.size(); ++i) {
    if (secondParametricEdgePoints[i] >= parametricInterpolatingDistance) {
      if (newSecond.empty()) {
        ...
        point::ENUPoint interpolatedPoint = interpolatePoint(
            second[i - 1], second[i], secondParametricEdgePoints[i - 1],
            parametricInterpolatingDistance, secondParametricEdgePoints[i]);

        //@todo: one potential issue is not considered in here yet:
        // if second curvature is extreme, the newly created linear edge segment
        // by the above two points could
        // potentially intersect with the old second border and by that
        // enlarging a driving area beyond the
        // allowed borders. To prevent from this one would have to implement
        // 'point::vectorIntersects':
        //
        ...
        //     {
        //        i = j;
        //        std::size_t const newSecondSize =
        //        secondParametricEdgePoints.size()-i + 1u;
        //        newSecond.reserve(newSecondSize);
        ...
        // before adding the interpolated point we have to add also some
        // interpolated points on the vector
        // newSecond[0] -> interpolatedPoint if there have been others skipped
        for (std::size_t j = 1u; j < i; ++j) {
          auto innerInterpolatedPoint = interpolatePoint(
              newSecond[0], interpolatedPoint, physics::ParametricValue(0.),
              secondParametricEdgePoints[j], parametricInterpolatingDistance);
        ...
        if (newOtherSecond.size() > 0u) {
          std::size_t j = 0u;
          for (; (j < i) && (j < newOtherSecond.size()) &&
                 (j < secondOtherLane->size());
               ++j) {
          ...
          if ((j < newOtherSecond.size()) && (j < secondOtherLane->size())) {
            ...
            newOtherSecond[j] = interpolatePoint(
                secondOtherLane->at(j - 1u), secondOtherLane->at(j),
                secondParametricEdgePoints[i - 1],
                parametricInterpolatingDistance, secondParametricEdgePoints[i]);
          }
          for (; ((j + 1u) < newOtherSecond.size()) &&
                 (j < secondOtherLane->size());
               j++) {
...
void makeTransitionFromFirstEdgeContinuous(point::ENUEdge &first,
  ...
  // if actually something has to be calculated, we reverse the edge and let the
  // other function do the job
  ...
  performMakeTransitionToSecondEdgeContinuous(invertedSecondEdge,
                                              invertedFirstEdge);
...
void makeTransitionToSecondBorderContinuous(ENUBorder const &first,
                                            ENUBorder &second) {
  performMakeTransitionToSecondEdgeContinuous(first.left, second.left,
                                              &second.right);
  performMakeTransitionToSecondEdgeContinuous(first.right, second.right,
                                              &second.left);
}

void makeTransitionFromFirstBorderContinuous(ENUBorder &first,
                                             ENUBorder const &second) {
  if (areEdgesContinuous(first.left, second.left) &&
      areEdgesContinuous(first.right, second.right)) {
    return;
  }
  // if actually something has to be calculated, we reverse the borders and let
  // the other function do the job
  ENUBorder invertedFirstBorder;
  invertedFirstBorder.left = first.left;
  invertedFirstBorder.right = first.right;
  std::reverse(invertedFirstBorder.left.begin(),
               invertedFirstBorder.left.end());
  std::reverse(invertedFirstBorder.right.begin(),
               invertedFirstBorder.right.end());
  ENUBorder invertedSecondBorder;
  invertedSecondBorder.left = second.left;
  invertedSecondBorder.right = second.right;
  std::reverse(invertedSecondBorder.left.begin(),
               invertedSecondBorder.left.end());
  std::reverse(invertedSecondBorder.right.begin(),
               invertedSecondBorder.right.end());
  makeTransitionToSecondBorderContinuous(invertedSecondBorder,
                                         invertedFirstBorder);
