//
// Copyright 2020 Inceptio Technology. All Rights Reserved.
//
// File: automldiffopen_m841FDlI.h
//
// Code generated for Simulink model 'CNHTC_VEHICLE_MODEL_PureSL'.
//
// Model version                  : 1.92
// Simulink Coder version         : 9.2 (R2019b) 18-Jul-2019
// C/C++ source code generated on : Thu Jul  9 18:47:09 2020
//
#ifndef SHARE_automldiffopen_m841FDlI
#define SHARE_automldiffopen_m841FDlI
#include "rtwtypes.h"

extern void automldiffopen_m841FDlI(const real_T u[3], real_T bw1, real_T bd,
  real_T bw2, real_T Ndiff, real_T shaftSwitch, real_T Jd, real_T Jw1, real_T
  Jw2, const real_T x[2], real_T y[3], real_T xdot[2]);

#endif

//
// File trailer for generated code.
//
// [EOF]
//
